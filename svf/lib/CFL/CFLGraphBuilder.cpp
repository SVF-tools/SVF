...
CFLGraph* CFLGraphBuilder::build(std::string fileName, GrammarBase* grammar, BuildDirection direction)
{
    ...
    if (isDot) return buildFromDot(fileName, grammar, direction);

    bool isJson = (fileName.rfind(".json") == fileName.length() - std::string(".json").length());
    if (isJson) return buildFromJson(fileName, grammar, direction);
...
CFLGraph* CFLGraphBuilder::buildFromText(std::string fileName, GrammarBase* grammar, BuildDirection direction)
{
    ...
    while (getline(inputFile, line))
    {
        ...
        if (labelToKindMap.find(label) != labelToKindMap.end()) cflGraph->addCFLEdge(src, dst, labelToKindMap[label]);
        else
        {
    ...
    for (auto it = graph->begin(); it != graph->end(); it++)
    {
        ...
        for (ConstraintEdge* edge : node->getOutEdges())
        {
            ...
            if (NormalGepCGEdge::classof(edge))
            {
                ...
                cflGraph->addCFLEdge(cflGraph->getGNode(edge->getSrcID()), cflGraph->getGNode(edge->getDstID()),
                                     edgeLabel);
                std::string label = kindToLabelMap[edge->getEdgeKind()];
                label.append("bar"); // for example Gep_i should be Gepbar_i, not Gep_ibar
                cflGraph->addCFLEdge(cflGraph->getGNode(edge->getDstID()), cflGraph->getGNode(edge->getSrcID()),
                                     CFGrammar::getAttributedKind(attr, labelToKindMap[label]));
...
CFLGraph* AliasCFLGraphBuilder::buildBiPEGgraph(ConstraintGraph* graph, Kind startKind, GrammarBase* grammar,
                                                SVFIR* pag)
{
    cflGraph = new CFLGraph(startKind);
    ...
    for (auto it = graph->begin(); it != graph->end(); it++)
    {
        ...
        for (ConstraintEdge* edge : node->getOutEdges())
        {
            ...
            {
                if (pag->isNullPtr(edge->getSrcID())) continue;
                ...
                /// Add Copy Edge
                cflGraph->addCFLEdge(cflGraph->getGNode(edge->getSrcID()), cflGraph->getGNode(CFLDerefNode->getId()),
                                     ConstraintEdge::Copy);
            ...
            else if (edge->getEdgeKind() == ConstraintEdge::Load)
            {
                ...
                /// Add Copy Edge
                cflGraph->addCFLEdge(cflGraph->getGNode(CFLDerefNode->getId()), cflGraph->getGNode(edge->getDstID()),
                                     ConstraintEdge::Copy);
                std::string label = kindToLabelMap[ConstraintEdge::Copy];
                label.append("bar");
...
void AliasCFLGraphBuilder::AliasCFLGraphBuilder::connectVGep(CFLGraph* cflGraph, ConstraintGraph* graph,
                                                             ConstraintNode* src, ConstraintNode* dst, u32_t level,
                                                             SVFIR* pag)
{
    if (level == 0) return;
    level -= 1;
...
void AliasCFLGraphBuilder::addBiCFLEdge(CFLGraph* cflGraph, ConstraintNode* src, ConstraintNode* dst,
                                        CFGrammar::Kind kind)
{
...
void AliasCFLGraphBuilder::addBiGepCFLEdge(CFLGraph* cflGraph, ConstraintNode* src, ConstraintNode* dst,
                                           CFGrammar::Attribute attri)
{
    ...
    cflGraph->addCFLEdge(cflGraph->getGNode(dst->getId()), cflGraph->getGNode(src->getId()),
                         CFGrammar::getAttributedKind(attri, labelToKindMap[label]));
    return;
}
...
CFLGraph* VFCFLGraphBuilder::buildBigraph(SVFG* graph, Kind startKind, GrammarBase* grammar)
{
    ...
    for (auto it = graph->begin(); it != graph->end(); it++)
    {
        ...
        for (VFGEdge* edge : node->getOutEdges())
        {
            ...
            if (edge->getEdgeKind() == VFGEdge::IntraDirectVF || edge->getEdgeKind() == VFGEdge::IntraIndirectVF ||
                edge->getEdgeKind() == VFGEdge::TheadMHPIndirectVF)
            {
                edgeLabel = 0;
                cflGraph->addCFLEdge(cflGraph->getGNode(edge->getSrcID()), cflGraph->getGNode(edge->getDstID()),
                                     edgeLabel);
