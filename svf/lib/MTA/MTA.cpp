...
LockAnalysis* MTA::computeLocksets(TCT* tct)
{
    ...
    lsa->analyze(PAG::getPAG()->getICFG(),
...
MHP* MTA::computeMHP(TCT* tct)
{
    ...
    MHP* mhp = new MHP(tct);
    mhp->analyze(PAG::getPAG()->getICFG(),
                 const_cast<CallGraph*>(PAG::getPAG()->getCallGraph()));
...
// objects it points to) and containment (a base object -> its field
// sub-objects). The containment step is essential for field sensitivity: a
// field-sensitive access resolves to a GepObjVar that is NOT reachable from its
// base by points-to edges, so without it a race on a (non-zero-offset) struct
// field -- or on an object reached through a struct's pointer field -- would be
// screened out of the escape set as "not shared".
PointsTo MTA::getPointsToClosure(AndersenBase* pta, const PointsTo& pts)
{
    ...
    while (!worklist.empty())
    {
        ...
        if (pag->getBaseObject(obj) !=
            nullptr) // containment (object nodes only)
            for (NodeID field :
                 pta->getAllFieldsObjVars(pta->getBaseObjVarID(obj)))
...
// only when it is multiforked (more than one dynamic instance).
bool MTA::occurrencesRace(MHP* mhp, const RaceOccurrence& first,
                          const RaceOccurrence& second)
{
    if (first.tid != second.tid)
        return first.interleav.test(second.tid) &&
               second.interleav.test(first.tid);
    return mhp->getTCT()->getTCTNode(first.tid)->isMultiforked();
}

// Record one order-normalised racing statement pair.
void MTA::commitRacePair(std::set<RacePair>& out, const RaceOccurrence& first,
                         const RaceOccurrence& second)
{
...
// occurrences sharing the race predicate's inputs into classes, then pair
// classes.
std::set<const SVFStmt*> MTA::detectRace(SVFIR* svfIr, AndersenBase* pta,
                                         ...
                                         std::set<RacePair>& outRacePairs)
{
    ...
    if (ThreadCallGraph* tcg = SVFUtil::dyn_cast<ThreadCallGraph>(callGraph))
    {
        ...
        for (auto it = tcg->forksitesBegin(), eit = tcg->forksitesEnd();
             it != eit; ++it)
    ...
    // "same bucket") as collected, so the points-to set is consumed straight
    // into the buckets and never stored per occurrence.
        ...
        for (auto bbIt : *fun)
            for (const ICFGNode* node : bbIt.second->getICFGNodeList())
            {
                if (!mhp->hasThreadStmtSet(node))
                    continue; // screen 1: concurrent?
                for (const SVFStmt* stmt : svfIr->getSVFStmtList(node))
                {
                    NodeID accessedPtr;
                    bool isStore;
                    if (const LoadStmt* load =
                            SVFUtil::dyn_cast<LoadStmt>(stmt))
                    {
                    ...
                    for (const CxtThreadStmt& threadStmt :
                         mhp->getThreadStmtSet(node))
                        occurrences.push_back(
                            {stmt, node, isStore, threadStmt.getTid(),
    ...
    // interleaving, isStore, lock sig) race the same partners, so collapse into
    // a class and judge C2/C3/C4 once per class pair -- O(classes^2) not
    // O(occ^2).
    for (const auto& objectAndOccs : objectToOccurrences)
    {
        ...
        for (size_t occIdx : objectAndOccs.second)
        {
            ...
            RaceKey key{occ.tid, occ.isStore, occ.interleav,
                        occ.locked ? occ.node : nullptr};
        ...
        for (size_t firstIdx = 0; firstIdx < classes.size(); ++firstIdx)
            for (size_t secondIdx = firstIdx; secondIdx < classes.size();
                 ++secondIdx)
            {
                ...
                if (!occurrencesRace(mhp, firstRep, secondRep))
                    continue; // C3
                // C4 + emit. Lock relation is uniform per class (the reps
                // decide it), so emit a statement-COVERING set -- every racy
                // member as an endpoint.
                if (firstIdx != secondIdx)
                {
                    if (firstClass.locked && secondClass.locked &&
                        lockAnalysis->isProtectedByCommonLock(firstRep.node,
                                                              secondRep.node))
                        continue;
                    ...
                    for (size_t memberIdx : secondClass.members)
                        commitRacePair(outRacePairs,
                                       occurrences[firstClass.members[0]],
                                       occurrences[memberIdx]);
                }
                else
                {
                    const std::vector<size_t>& members = firstClass.members;
                    // Multiforked self-race: every member self-races a
                    // concurrent instance of itself, independent of any
                    // cross-race below.
                    for (size_t memberIdx : members)
                        if (!firstClass.locked ||
                            ...
                            commitRacePair(outRacePairs, occurrences[memberIdx],
                                           occurrences[memberIdx]);
                    // Cross-race needs >=2 members: two distinct occurrences to
                    // pair.
                    if (members.size() >= 2 &&
                        (!firstClass.locked ||
                         !lockAnalysis->isProtectedByCommonLock(
                             occurrences[members[0]].node,
                             occurrences[members[1]].node)))
                        for (size_t pos = 0; pos < members.size(); ++pos)
                            commitRacePair(
                                outRacePairs, occurrences[members[pos]],
...
void MTA::reportRaces()
{
    ...
    // Shared equivalence-class detector (the same one the slicing pipeline
    // uses), run over the Andersen pre-analysis with this MTA's MHP/lock
    // results.
    std::set<RacePair> racePairs;
    detectRace(pag, pta, mhp, lsa, callGraph, racePairs);

    for (const RacePair& rp : racePairs)
        outs() << SVFUtil::bugMsg1("race pair(")
               << " stmt1: " << rp.stmt1->toString()
...
void timePhase(const char* name, const std::function<void()>& fn)
{
    ...
    if (ms >= 1000.0)
        SVFUtil::outs() << " (" << std::fixed << std::setprecision(2)
                        << (ms / 1000.0) << " s)";
    SVFUtil::outs() << "\n";
}
...
void reportOriginalStats(SVFIR* svfIr)
{
    ...
    for (ICFG::iterator it = svfIr->getICFG()->begin(),
                        eit = svfIr->getICFG()->end();
         it != eit; ++it)
        icfgNodeCount++;

    size_t functionCount = 0;
    for (auto it = svfIr->getCallGraph()->begin(),
              eit = svfIr->getCallGraph()->end();
         it != eit; ++it)
        functionCount++;

    size_t pagStmtCount = 0;
    for (PAG::iterator it = svfIr->getPAG()->begin(),
                       eit = svfIr->getPAG()->end();
         it != eit; ++it)
        pagStmtCount++;
...
SlicedMTA::~SlicedMTA()
{
    // Cleanup in reverse order of creation. Release the VFG_pre (and its
    // MemSSA), the FSAM, the sliced/base TCTs -- all of which reference the
    // SVFIR -- before releasing the SVFIR itself. The remaining unique_ptr
    // members (mhp / lockAnalysis / sliced* / slicers / views) auto-destroy
    // after this body.
    vfgPreBuilder.reset();
    mtaFSMPTA.reset();
    slicedTCT.reset();
    tct.reset();
    SVFIR::releaseSVFIR();
    // The Andersen pre-analysis is a shared singleton (reused by VFG_pre and
    // the main FSMPTA, neither of which frees it); release it once here.
...
BVDataPTAImpl* SlicedMTA::getMainPTA() const
{
    ...
    // BVDataPTAImpl, so the downstream race detector queries it
    // polymorphically.
...
void SlicedMTA::buildVFGPre()
{
    timePhase("Build thread-aware VFG_pre", [&]() {
        ...
        if (ThreadCallGraph* tcg =
                SVFUtil::dyn_cast<ThreadCallGraph>(preAnder->getCallGraph()))
        {
            tcg->updateCallGraph(preAnder);
            tcg->updateJoinEdge(preAnder);
        }
        vfgPreBuilder =
            std::make_unique<MTASVFGBuilder>(mhp.get(), lockAnalysis.get());
        vfgPre = vfgPreBuilder->buildPTROnlySVFG(preAnder);
        SVFUtil::outs() << "[VFG_pre] thread-aware SVFG: "
                        << vfgPre->getSVFGNodeNum() << " nodes, "
...
bool SlicedMTA::runPreAnalysis(const ResolveIndirectCalls& resolveIndirectCalls)
{
    ...
    // Steensgaard's unification, so fewer spurious MHP/races and a smaller
    // slice). The same Andersen instance (a singleton) is reused for the
    // thread-aware VFG_pre and the main FSMPTA, so the whole pipeline shares
    // one pre-analysis.
    timePhase("Andersen's pointer analysis", [&]() {
        ...
        // Materialise resolved indirect calls into the PAG (LLVM-dependent
        // step, injected by the caller), then update the ICFG with the resolved
        // calls.
    ...
    // Step 2: Build Thread Create Tree (the caller forces -max-cxt to 0 around
    // the whole pre-analysis when slicing; see runOnModule).
    ...
    timePhase("Mark truncation-merged multiforked threads", [&]() {
        ...
        for (const auto& deepPair : deepTct)
            if (const ICFGNode* forkSite =
                    deepPair.second->getCxtThread().getThread())
            {
        ...
        for (const auto& prePair : *tct)
        {
            const ICFGNode* forkSite =
                prePair.second->getCxtThread().getThread();
            if (forkSite == nullptr)
                continue;
            Map<const ICFGNode*, u32_t>::const_iterator fIt =
                forkSiteInstances.find(forkSite);
    ...
    timePhase("Detect Race Statements", [&]() {
        // Shared equivalence-class detector (the same one MTA::reportRaces
        // uses).
        vulnerableStatements =
            MTA::detectRace(svfIr, preAnder, mhp.get(), lockAnalysis.get(),
                            preAnder->getCallGraph(), racePairs);
    });
    SVFUtil::outs() << "Found " << vulnerableStatements.size()
                    << " vulnerable statements\n";
    SVFUtil::outs() << "Found " << racePairs.size() << " race pairs\n";

    // Step 6: build the thread-aware VFG once (substrate for slicing + main
    // FS).
    buildVFGPre();
...
bool SlicedMTA::runMTASlicingAndAnalysis()
{
    ...
    if (Options::SlicingSingle())
    {
        ...
        // combining synchronization + data + call dependence, shared by both
        // the ILA and the FSPTA stages. Computed once here; reused in PTA
        // slicing.
        SVFUtil::outs() << "[Slicing Mode] Single unified slice (V_Single) for "
                           "ILA + FSPTA\n";
    ...
    else
    {
        ...
        // ILA slicing sources = [INIT] race statements + [THREAD-VF] sources.
        // Keep a candidate edge's query (see
        // MTASVFGBuilder::getThreadVFQueryMap) only if both endpoints survive
        // the FSPTA slice -- i.e. the edge is in ThreadVF(VFG'_pre). Closure
        // computed here (pre<->pre) and reused by PTA slicing.
        std::set<const ICFGNode*> threadVFSources;
        if (vfgPreBuilder)
        {
            const std::set<const SVFGNode*>& retained =
                multiStageSlicer->getRetainedSVFGNodes(vulnerableStatements);
            // Millions of query entries are scanned; use a constant-time lookup
            // view of the retained nodes instead of two ordered-set lookups
            // each.
            const Set<const SVFGNode*> retainedView(retained.begin(),
                                                    retained.end());
            for (const auto& entry : vfgPreBuilder->getThreadVFQueryMap())
            {
                const MTASVFGBuilder::ThreadVFEdge& edge = entry.first;
                if (retainedView.count(edge.first) &&
                    retainedView.count(edge.second))
                {
                    ...
                    threadVFSources.insert(entry.second.begin(),
                                           entry.second.end());
                }
            }
        }
        SVFUtil::outs()
            << "[THREAD-VF] " << threadVFSources.size()
            << " ILA slicing sources from VFG_pre value-flow construction\n";
        timePhase("MTA Slicing", [&]() {
            mtaSlicedNodes = multiStageSlicer->runILASlicing(
                vulnerableStatements, threadVFSources);
        });
    ...
    timePhase("Sliced Thread Create Tree", [&]() {
        ...
        SVFUtil::outs() << "[SlicedTCT] Using max context length: "
                        << maxContextLen << " (from -max-cxt)\n";
        // Reuse the shared pre-analysis (Andersen) for the sliced TCT.
        slicedTCT =
            std::make_unique<SlicedTCT>(preAnder, slicedView, maxContextLen);
        if (dumpDot)
    ...
    timePhase("Sliced Interleaving and Lock Analysis", [&]() {
        slicedMhp = std::make_unique<MHP>(slicedTCT.get());
        slicedMhp->analyze(slicedView->getICFG(),
                           slicedView->getThreadCallGraph());
        slicedLockAnalysis = std::make_unique<LockAnalysis>(slicedTCT.get());
        slicedLockAnalysis->analyze(slicedView->getICFG(),
                                    slicedView->getThreadCallGraph());
    });
...
bool SlicedMTA::runPTASlicingAndAnalysis()
{
    ...
    if (!hasThreadFunctions)
    {
        SVFUtil::outs() << "[SKIP] No thread functions found in pre-analysis, "
                           "skipping PTA slicing\n";
        return true;
    }
    if (racePairs.empty())
    {
    ...
    if (Options::SlicingSingle())
    {
        // Single-pass baseline: reuse the unified V_Single computed in MTA
        // slicing (no separate data-dependence slice); FSPTA runs on the same
        // slice as ILA.
        SVFUtil::outs()
            << "[Slicing Mode] Reusing unified slice (V_Single) for FSPTA\n";
        ptaSlicedNodes = singleSlicedNodes;
        SVFUtil::outs() << "PTA reuses unified slice: " << ptaSlicedNodes.size()
                        << " nodes\n";
    }
    else
    {
        SVFUtil::outs() << "Using " << vulnerableStatements.size()
                        << " vulnerable statements from pre-analysis\n";
        SVFUtil::outs() << "Using " << racePairs.size()
                        << " race pairs from pre-analysis\n";

        // Stage 2 of the multi-stage slicer built during MTA slicing (it
        // memoised the shared data-dependence closure over VFG_pre); construct
        // only if absent (defensive: the ILA stage normally created it).
        if (!multiStageSlicer)
            multiStageSlicer =
                std::
                    make_unique<MultiStageSlicer>(svfIr, preAnder, mhp.get(),
                                                  lockAnalysis.get(), vfgPre /* paper-faithful data dependence over the thread-aware VFG */);

        timePhase("PTA Slicing", [&]() {
            ptaSlicedNodes =
                multiStageSlicer->runPTASlicing(vulnerableStatements);
        });
        SVFUtil::outs() << "PTA sliced to " << ptaSlicedNodes.size()
                        << " nodes\n";
    }

    // Step 4: Build PTA SlicedSVFIRView for pointer analysis. Its control flow
    // is never walked (FSMPTA uses only isKeptNode), so skip bridged-edge
    // construction.
    ...
    // VFG_pre and the slicers: the pre- and main graphs never coexist in
    // memory.
    ...
    // Always rebuild the thread-aware value flow from the SLICED ILA: the
    // sliced MHP/lock analysis is context-sensitive, whereas the pre-analysis
    // VFG_pre was built context-insensitively (max-cxt forced to 0 for
    // slicing). Reusing VFG_pre would decide the interference (thread-aware
    // value-flow) edges from a context-insensitive ILA and over-approximate the
    // FSAM points-to, so a fresh context-sensitive SVFG is required to preserve
    // the result. [THREAD-VF] seeding keeps the queried interference witnesses
    // in the slice.
    ...
    timePhase("Flow-Sensitive FSAM Analysis", [&]() {
        // The sliced SVFG view: membership from the FSPTA ICFG slice; the SVFG
        // itself is built inside the solver and bound afterwards (for dumping).
        slicedSVFGView =
            std::make_unique<SlicedSVFGView>(ptaSlicedView->getICFG());
...
// Build a lock analysis over the WHOLE ICFG (every node kept => real control
// flow, no bridged edges). Used for the final detection's lock signature so the
// sliced run reproduces the whole-program lock relation exactly (query
// preservation).
LockAnalysis* SlicedMTA::buildFullLockAnalysis()
{
    ...
    fullLockTCT = std::make_unique<SlicedTCT>(preAnder, fullLockView.get(),
                                              slicedMaxContextLen());
    fullLockAnalysis = std::make_unique<LockAnalysis>(fullLockTCT.get());
    const SlicedSVFIRView* flv = fullLockView.get();
    fullLockAnalysis->analyze(flv->getICFG(), flv->getThreadCallGraph());
    return fullLockAnalysis.get();
}
...
bool SlicedMTA::runFinalRaceDetection()
{
    ...
    {
        SVFUtil::outs() << "\n=== Bug Report ===\n";
        SVFUtil::outs() << "Found " << detectedPairs.size()
                        << " race pair(s) in sliced graph\n";
    }
    else
    {
...
void SlicedMTA::runWholeProgramDetection()
{
    ...
    timePhase("Whole-program Sliced TCT/MHP/Lock", [&]() {
        slicedTCT = std::make_unique<SlicedTCT>(preAnder, ptaSlicedView.get(),
                                                slicedMaxContextLen());
        slicedMhp = std::make_unique<MHP>(slicedTCT.get());
                            ...
                            const ResolveIndirectCalls& resolveIndirectCalls)
{
    ...
    SVFUtil::outs() << "[Config] Slicing: "
                    << (Options::EnableSlicing() ? "enabled" : "disabled")
                    << "\n";
    ...
    {
        if (!runMTASlicingAndAnalysis())
            return;
        if (!runPTASlicingAndAnalysis())
            return;
        if (!runFinalRaceDetection())
            return;
    }
    else
    {
...
// baseline), driven by the sliced/FSAM analyses. hasThreadFunctions is a
// generic helper on the base MTA detector; detectRacePairsOnSlicedGraph is the
// pipeline's sliced-graph screen and stays on SlicedMTA.
//===----------------------------------------------------------------------===//
    ...
    BVDataPTAImpl* slicedPTA, MHP* slicedMHP, LockAnalysis* slicedLockAnalysis)
{
    ...
    // only kept nodes, so the sliced and whole runs invoke the identical
    // detector.
