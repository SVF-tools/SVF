...
{
    return eqVarToValMap(_varToVal, rhs.getVarToVal()) && eqVarToValMap(_addrToVal, rhs.getLocToVal());
}
...
bool RelExeState::operator<(const RelExeState& rhs) const
{
    return lessThanVarToValMap(_varToVal, rhs.getVarToVal()) || lessThanVarToValMap(_addrToVal, rhs.getLocToVal());
}
...
bool RelExeState::eqVarToValMap(const VarToValMap& lhs, const VarToValMap& rhs) const
{
    ...
    for (const auto& item : lhs)
    {
        ...
        if (it == rhs.end() || !eq(item.second, it->second)) return false;
    }
    return true;
}
...
bool RelExeState::lessThanVarToValMap(const VarToValMap& lhs, const VarToValMap& rhs) const
{
    ...
    for (const auto& item : lhs)
    {
        ...
        // judge from expr id
        if (!eq(item.second, it->second)) return item.second.id() < it->second.id();
    }
    return false;
}
