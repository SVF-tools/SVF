...
AbstractState RelationSolver::bilateral(const AbstractState& domain, const Z3Expr& phi, u32_t descend_check)
{
    /// init variables
    ...
    while (lower != upper)
    {
        ...
        else /// unknown or unsat
        {
            ...
            if (checkRes == z3::unknown)
            {
                ...
                if (solver.reason_unknown() == "timeout") return upper;
            }
...
AbstractState RelationSolver::RSY(const AbstractState& domain, const Z3Expr& phi)
{
    ...
    while (1)
    {
        ...
        else /// unknown or unsat
        {
            ...
            {
                /// for timeout reason return upper
                if (solver.reason_unknown() == "timeout") return domain.top();
            }
            break;
        }
    }
...
AbstractState RelationSolver::abstract_consequence(const AbstractState& lower, const AbstractState& upper,
                                                   const AbstractState& domain) const
{
    ...
    for (auto it = domain.getVarToVal().begin(); it != domain.getVarToVal().end(); ++it)
...
Z3Expr RelationSolver::gamma_hat(const AbstractState& exeState) const
{
    ...
    for (auto& item : exeState.getVarToVal())
    {
        ...
        res = (res && v >= (int)interval.lb().getNumeral() && v <= (int)interval.ub().getNumeral()).simplify();
    }
    return res;
}

Z3Expr RelationSolver::gamma_hat(const AbstractState& alpha, const AbstractState& exeState) const
{
    Z3Expr res(Z3Expr::getContext().bool_val(true));
    for (auto& item : exeState.getVarToVal())
    {
...
Z3Expr RelationSolver::gamma_hat(u32_t id, const AbstractState& exeState) const
{
    ...
    Z3Expr res =
        (v >= (int)it->second.getInterval().lb().getNumeral() && v <= (int)it->second.getInterval().ub().getNumeral());
    return res;
}

AbstractState RelationSolver::beta(const Map<u32_t, s32_t>& sigma, const AbstractState& exeState) const
{
    AbstractState res;
    for (const auto& item : exeState.getVarToVal())
    {
        res[item.first] = IntervalValue(sigma.at(item.first), sigma.at(item.first));
    }
    return res;
}
...
AbstractState RelationSolver::BS(const AbstractState& domain, const Z3Expr& phi)
{
    ...
    for (const auto& item : domain.getVarToVal())
    {
        ...
        if (interval.lb().is_minus_infinity()) updateMap(low_values, item.first, -infinity);
        else
            updateMap(low_values, item.first, interval.lb().getIntNumeral());
        if (interval.ub().is_plus_infinity()) updateMap(high_values, item.first, infinity);
        else
            updateMap(high_values, item.first, interval.ub().getIntNumeral());
        if (item.first > bias) bias = item.first + 1;
    }
    for (const auto& item : domain.getVarToVal())
    {
        /// init objects -x
        ...
        if (interval.ub().is_plus_infinity()) updateMap(low_values, reverse_key, -infinity);
        else
            updateMap(low_values, reverse_key, -interval.ub().getIntNumeral());
        if (interval.lb().is_minus_infinity()) updateMap(high_values, reverse_key, infinity);
        else
    ...
    for (const auto& item : ret)
    {
        if (item.first >= bias)
        {
            if (!retInv.inVarToValTable(item.first - bias)) retInv[item.first - bias] = IntervalValue::top();

            if (item.second == (infinity))
                retInv[item.first - bias] =
                    IntervalValue(BoundedInt::minus_infinity(), retInv[item.first - bias].getInterval().ub());
            else
                retInv[item.first - bias] =
                    IntervalValue(float(-item.second), retInv[item.first - bias].getInterval().ub());
        }
        else
        {
            if (item.second == (infinity))
                retInv[item.first] = IntervalValue(retInv[item.first].getInterval().lb(), BoundedInt::plus_infinity());
            else
...
Map<u32_t, s32_t> RelationSolver::BoxedOptSolver(const Z3Expr& phi, Map<u32_t, s32_t>& ret,
                                                 Map<u32_t, s32_t>& low_values, Map<u32_t, s32_t>& high_values)
{
...
void RelationSolver::decide_cpa_ext(const Z3Expr& phi, Map<u32_t, Z3Expr>& L_phi, Map<u32_t, s32_t>& mid_values,
                                    Map<u32_t, s32_t>& ret, Map<u32_t, s32_t>& low_values,
                                    Map<u32_t, s32_t>& high_values)
{
    while (1)
    {
        Z3Expr join_expr(Z3Expr::getContext().bool_val(false));
        for (const auto& item : L_phi) join_expr = (join_expr || item.second);
        ...
        {
            solver.pop();
            for (const auto& item : L_phi) high_values.at(item.first) = mid_values.at(item.first) - 1;
            return;
        }
    }
}