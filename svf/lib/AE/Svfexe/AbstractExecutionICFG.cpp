...
void AbstractExecutionICFG::initExtAPI()
{
...
void AbstractExecutionICFG::markRecursiveFuns()
{
...
/// handle global node
void AbstractExecutionICFG::handleGlobalNode()
{
    IntervalExeState es;
...
bool AbstractExecutionICFG::hasCmpBranchES(const CmpStmt* cmpStmt, s64_t succ, IntervalExeState& es)
{
...
bool AbstractExecutionICFG::hasSwitchBranchES(const SVFVar* var, s64_t succ, IntervalExeState& es)
{
    IntervalExeState new_es = es;
...
bool AbstractExecutionICFG::hasBranchES(const IntraCFGEdge* intraEdge, IntervalExeState& es)
{
...
void AbstractExecutionICFG::handleCallSite(const ICFGNode* node)
{
...
void AbstractExecutionICFG::handleCycle(const ICFGWTOCycle *cycle)
{
    // Get execution states from in edges
...
void AbstractExecutionICFG::handleSVFStatement(const SVFStmt *stmt)
{
    if (const AddrStmt *addr = SVFUtil::dyn_cast<AddrStmt>(stmt))
    {
        _svfir2ExeState->translateAddr(addr);
    ...
    {
        std::cout << _stat->getBlockTrace() << ", CMP: " << cmp->toString() << std::endl;
...
void AbstractExecutionICFG::SkipRecursiveCall(const CallICFGNode *callNode)
{
    const SVFFunction *callfun = SVFUtil::getCallee(callNode->getCallSite());
    const RetICFGNode *retNode = callNode->getRetICFGNode();
    if (retNode->getSVFStmts().size() > 0)
...
void AbstractExecutionICFG::SkipRecursiveFunc(const SVFFunction *func)
{
