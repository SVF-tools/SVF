...
const std::string FormalINSVFGNode::toString() const
{
    ...
    rawstr << getMRVer()->getMR()->getMRID() << "V_" << getMRVer()->getSSAVersion() << " = ENCHI(MR_"
...
const std::string ActualINSVFGNode::toString() const
{
    std::string str;
    std::stringstream rawstr(str);
    rawstr << "ActualINSVFGNode ID: " << getId() << " at callsite: " << getCallSite()->getCallSite()->toString()
           << " {fun: " << getFun()->getName() << "}";
...
const std::string ActualOUTSVFGNode::toString() const
{
    std::string str;
    std::stringstream rawstr(str);
    rawstr << "ActualOUTSVFGNode ID: " << getId() << " at callsite: " << getCallSite()->getCallSite()->toString()
           << " {fun: " << getFun()->getName() << "}";
    rawstr << getMRVer()->getMR()->getMRID() << "V_" << getMRVer()->getSSAVersion() << " = CSCHI(MR_"
           << getMRVer()->getMR()->getMRID() << "V_" << getMRVer()->getSSAVersion() << ")\n";
    rawstr << getMRVer()->getMR()->dumpStr() << "\n";
    rawstr << "CS[" << getCallSite()->getCallSite()->getSourceLoc() << "]";
    return rawstr.str();
...
const std::string MSSAPHISVFGNode::toString() const
{
    ...
    rawstr << "MR_" << getResVer()->getMR()->getMRID() << "V_" << getResVer()->getSSAVersion() << " = PHI(";
...
const std::string InterMSSAPHISVFGNode::toString() const
{
    std::string str;
    std::stringstream rawstr(str);
    if (isFormalINPHI()) rawstr << "FormalINPHISVFGNode ID: " << getId() << " {fun: " << getFun()->getName() << "}";
    else
        rawstr << "ActualOUTPHISVFGNode ID: " << getId() << " at callsite: " << getCallSite()->getCallSite()->toString()
               << " {fun: " << getFun()->getName() << "}";
    rawstr << MSSAPHISVFGNode::toString();
    return rawstr.str();
}
...
FormalOUTSVFGNode::FormalOUTSVFGNode(NodeID id, const MRVer* mrVer, const FunExitICFGNode* funExit)
    : MRSVFGNode(id, FPOUT)
{
...
void SVFG::buildSVFG()
{
    ...
    else
    {
        ...
        stat->indVFEdgeEnd();
        if (!Options::WriteSVFG().empty()) writeToFile(Options::WriteSVFG());
    }
}
...
void SVFG::addSVFGNodesForAddrTakenVars()
{
    ...
    for (MemSSA::BBToPhiSetMap::iterator it = mssa->getBBToPhiSetMap().begin(), eit = mssa->getBBToPhiSetMap().end();
         it != eit; ++it)
    {
        for (PHISet::iterator pi = it->second.begin(), epi = it->second.end(); pi != epi; ++pi)
        {
            ...
            addIntraMSSAPHISVFGNode(pag->getICFG()->getICFGNode(inst), phi->opVerBegin(), phi->opVerEnd(),
                                    phi->getResVer(), totalVFGNode++);
        }
    }
         ...
         it != eit; ++it)
    {
        for (CHISet::iterator pi = it->second.begin(), epi = it->second.end(); pi != epi; ++pi)
        {
            ...
            addFormalINSVFGNode(pag->getICFG()->getFunEntryICFGNode(chi->getFunction()), chi->getResVer(),
                                totalVFGNode++);
        }
    }
...
void SVFG::connectIndirectSVFGEdges()
{
    ...
    for (iterator it = begin(), eit = end(); it != eit; ++it)
    {
        ...
        else if (const FormalINSVFGNode* formalIn = SVFUtil::dyn_cast<FormalINSVFGNode>(node))
        {
            ...
            for (CallGraphEdge::CallInstSet::iterator it = callInstSet.begin(), eit = callInstSet.end(); it != eit;
                 ++it)
            {
                const CallICFGNode* cs = *it;
                if (!mssa->hasMU(cs)) continue;
        ...
        else if (const FormalOUTSVFGNode* formalOut = SVFUtil::dyn_cast<FormalOUTSVFGNode>(node))
        {
            ...
            {
                const CallICFGNode* cs = *it;
                if (!mssa->hasCHI(cs)) continue;
                ActualOUTSVFGNodeSet& actualOuts = getActualOUTSVFGNodes(cs);
                for (ActualOUTSVFGNodeSet::iterator ait = actualOuts.begin(), aeit = actualOuts.end(); ait != aeit;
                     ++ait)
                {
        ...
        {
            for (MemSSA::PHI::OPVers::const_iterator it = phiNode->opVerBegin(), eit = phiNode->opVerEnd(); it != eit;
                 it++)
            {
...
void SVFG::connectFromGlobalToProgEntry()
{
         ...
         storeIt != storeEit; ++storeIt)
    {
        if (const StoreSVFGNode* store = SVFUtil::dyn_cast<StoreSVFGNode>(*storeIt))
        {
            ...
            for (FormalINSVFGNodeSet::iterator fiIt = formalIns.begin(), fiEit = formalIns.end(); fiIt != fiEit; ++fiIt)
            {
                NodeID formalInID = *fiIt;
                NodeBS formalInPts = ((FormalINSVFGNode*)getSVFGNode(formalInID))->getPointsTo();

                formalInPts &= storePts;
                if (formalInPts.empty()) continue;
 ...
 */
void SVFG::getInterVFEdgesForIndirectCallSite(const CallICFGNode* callICFGNode, const SVFFunction* callee,
                                              SVFGEdgeSetTy& edges)
{
    ...
    if (hasFuncEntryChi(callee) && hasCallSiteMu(callICFGNode))
    {
        ...
        for (SVFG::ActualINSVFGNodeSet::iterator ai_it = actualInNodes.begin(), ai_eit = actualInNodes.end();
             ai_it != ai_eit; ++ai_it)
        {
    ...
    if (hasFuncRetMu(callee) && hasCallSiteChi(callICFGNode))
    {
        ...
        for (SVFG::ActualOUTSVFGNodeSet::iterator ao_it = actualOutNodes.begin(), ao_eit = actualOutNodes.end();
             ao_it != ao_eit; ++ao_it)
        {
...
void SVFG::connectCallerAndCallee(const CallICFGNode* cs, const SVFFunction* callee, SVFGEdgeSetTy& edges)
{
    ...
    if (hasFuncEntryChi(callee) && hasCallSiteMu(cs))
    {
             ...
             ai_it != ai_eit; ++ai_it)
        {
            ...
            for (SVFG::FormalINSVFGNodeSet::iterator fi_it = formalInNodes.begin(), fi_eit = formalInNodes.end();
                 fi_it != fi_eit; ++fi_it)
            {
    ...
    if (hasFuncRetMu(callee) && hasCallSiteChi(cs))
    {
        ...
        for (SVFG::FormalOUTSVFGNodeSet::iterator fo_it = formalOutNodes.begin(), fo_eit = formalOutNodes.end();
             fo_it != fo_eit; ++fo_it)
        {
...
const SVFFunction* SVFG::isFunEntrySVFGNode(const SVFGNode* node) const
{
    ...
    {
        if (mphi->isFormalINPHI()) return mphi->getFun();
    }
    return nullptr;
}
...
const CallICFGNode* SVFG::isCallSiteRetSVFGNode(const SVFGNode* node) const
{
    ...
    {
        if (phi->isActualRetPHI()) return phi->getCallSite();
    }
    ...
    {
        if (mphi->isActualOUTPHI()) return mphi->getCallSite();
    }
    return nullptr;
}
...
template <> struct DOTGraphTraits<SVFG*> : public DOTGraphTraits<SVFIR*>
{
    ...
    template <class EdgeIter> static std::string getEdgeAttributes(NodeType*, EdgeIter EI, SVFG*)
    {
