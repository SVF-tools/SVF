...
bool AccessPath::isConstantOffset() const
{
    ...
    {
        if (SVFUtil::isa<SVFConstantInt>(it.first->getValue()) == false) return false;
    }
    return true;
}
...
/// For a pointer type (e.g., t1 is PointerType), we will retrieve the pointee type and times the offset, i.e.,
...
APOffset AccessPath::computeConstantOffset() const
{
    ...
    if (idxOperandPairs.size() == 0) return getConstantStructFldIdx();
    for (int i = idxOperandPairs.size() - 1; i >= 0; i--)
    {
        const SVFValue* value = idxOperandPairs[i].first->getValue();
        const SVFType* type = idxOperandPairs[i].second;
        ...
        if (SVFUtil::isa<SVFPointerType>(type)) totalConstOffset += op->getSExtValue() * getElementNum(gepPointeeType);
        else
        {
...
AccessPath AccessPath::operator+(const AccessPath& rhs) const
{
    ...
    for (auto& p : ap.getIdxOperandPairVec()) ap.addOffsetVarAndGepTypePair(p.first, p.second);
...
{
    if (fldIdx != rhs.fldIdx) return (fldIdx < rhs.fldIdx);
    else
    {
        const IdxOperandPairs& pairVec = getIdxOperandPairVec();
        const IdxOperandPairs& rhsPairVec = rhs.getIdxOperandPairVec();
        if (pairVec.size() != rhsPairVec.size()) return (pairVec.size() < rhsPairVec.size());
        else
        {
...
SVF::AccessPath::LSRelation AccessPath::checkRelation(const AccessPath& LHS, const AccessPath& RHS)
{
    ...
    {
        if (lhsLocations == rhsLocations) return Same;
