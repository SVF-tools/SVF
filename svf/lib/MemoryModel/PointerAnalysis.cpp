...
void PointerAnalysis::initialize()
{
    ...
    if (Options::CallGraphDotGraph())
        getPTACallGraph()->dump("pta_callgraph_initial");
}
        ...
        if(ObjVar* objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii)))
        {
            ...
            if(obj->isFunction())
            {
                ...
                if(0 == getIndCallMap()[cs].count(callee))
                {
                    ...
                    //PTACallGraphNode* callgraphNode = ptaCallGraph->getOrInsertFunction(cs.getCaller());
                    //callgraphNode->addCalledFunction(cs,ptaCallGraph->getOrInsertFunction(callee));
                }
            }
        }
    }
