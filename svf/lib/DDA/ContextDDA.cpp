...
bool ContextDDA::testIndCallReachability(CxtLocDPItem& dpm, const CallGraphNode* callee, const CallICFGNode* cs)
{
...
CallSiteID ContextDDA::getCSIDAtCall(CxtLocDPItem&, const SVFGEdge* edge)
{
    ...
    const CallGraphNode* callee = edge->getDstNode()->getFun();
...
CallSiteID ContextDDA::getCSIDAtRet(CxtLocDPItem&, const SVFGEdge* edge)
{
    ...
    const CallICFGNode* cbn = getSVFG()->getCallSite(svfg_csId);
    const CallGraphNode* callee = edge->getSrcNode()->getFun();
