...
const SVFValue* ThreadAPI::getLockVal(const ICFGNode* inst) const
{
    ...
    assert((isTDAcquire(call->getCallSite()) || isTDRelease(call->getCallSite())) &&
...
const SVFValue* ThreadAPI::getJoinedThread(const ICFGNode* inst) const
{
    ...
    {
        if (SVFUtil::isa<LoadStmt>(stmt)) return stmt->getSrcNode()->getValue();
    }
    if (SVFUtil::isa<SVFArgument>(join)) return join;
...
void ThreadAPI::performAPIStat(SVFModule* module)
{
    ...
    for (SVFModule::const_iterator it = module->begin(), eit = module->end(); it != eit; ++it)
    {
        for (SVFFunction::const_iterator bit = (*it)->begin(), ebit = (*it)->end(); bit != ebit; ++bit)
        {
            ...
            for (SVFBasicBlock::const_iterator ii = bb->begin(), eii = bb->end(); ii != eii; ++ii)
            {
                ...
                if (!SVFUtil::isCallSite(svfInst)) continue;
    ...
    for (Map<std::string, u32_t>::iterator it = tdAPIStatMap.begin(), eit = tdAPIStatMap.end(); it != eit; ++it)
    {
