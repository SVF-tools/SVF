...
{
    pta = p;
    assert((pta->getAnalysisTy() != PointerAnalysis::Default_PTA) && "please specify a pointer analysis");
...
void MemSSA::createMUCHI(const SVFFunction& fun)
{
    ...
    for (BBList::const_iterator iter = reachableBBs.begin(), eiter = reachableBBs.end(); iter != eiter; ++iter)
    {
        ...
        for (SVFBasicBlock::const_iterator it = bb->begin(), eit = bb->end(); it != eit; ++it)
        {
            ...
            if (mrGen->hasSVFStmtList(inst))
            {
                ...
                for (SVFStmtList::const_iterator bit = pagEdgeList.begin(), ebit = pagEdgeList.end(); bit != ebit;
                     ++bit)
                {
            ...
            if (isNonInstricCallSite(inst))
            {
                ...
                if (mrGen->hasRefMRSet(cs)) AddCallSiteMU(cs, mrGen->getCallSiteRefMRSet(cs));

                if (mrGen->hasModMRSet(cs)) AddCallSiteCHI(cs, mrGen->getCallSiteModMRSet(cs));
            }
        }
    }
    ...
    for (MRSet::iterator iter = usedRegs.begin(), eiter = usedRegs.end(); iter != eiter; ++iter)
    {
...
void MemSSA::SSARenameBB(const SVFBasicBlock& bb)
{
    ...
    if (hasPHISet(&bb)) RenamePhiRes(getPHISet(&bb), memRegs);
    ...
    for (SVFBasicBlock::const_iterator it = bb.begin(), eit = bb.end(); it != eit; ++it)
    {
        ...
        if (mrGen->hasSVFStmtList(inst))
        {
            ...
            {
                const PAGEdge* inst = *bit;
                if (const LoadStmt* load = SVFUtil::dyn_cast<LoadStmt>(inst)) RenameMuSet(getMUSet(load));
        ...
        {
            const CallICFGNode* cs = pag->getICFG()->getCallICFGNode(inst);
            if (mrGen->hasRefMRSet(cs)) RenameMuSet(getMUSet(cs));

            if (mrGen->hasModMRSet(cs)) RenameChiSet(getCHISet(cs), memRegs);
        }
    ...
    for (const SVFBasicBlock* succ : bb.getSuccessors())
    {
        ...
        if (hasPHISet(succ)) RenamePhiOps(getPHISet(succ), pos, memRegs);
    }
...
{
    assert(0 != mr2CounterMap.count(mr) && "did not find initial version in map? ");
    assert(0 != mr2VerStackMap.count(mr) && "did not find initial stack in map? ");
...
void MemSSA::destroy()
{
    ...
    for (LoadToMUSetMap::iterator iter = load2MuSetMap.begin(), eiter = load2MuSetMap.end(); iter != eiter; ++iter)
    {
        for (MUSet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
            delete *it;
        }
    }

    for (StoreToChiSetMap::iterator iter = store2ChiSetMap.begin(), eiter = store2ChiSetMap.end(); iter != eiter;
         ++iter)
    {
        for (CHISet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
            delete *it;
        }
    }

    for (CallSiteToMUSetMap::iterator iter = callsiteToMuSetMap.begin(), eiter = callsiteToMuSetMap.end();
         iter != eiter; ++iter)
    {
        for (MUSet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
    ...
    for (CallSiteToCHISetMap::iterator iter = callsiteToChiSetMap.begin(), eiter = callsiteToChiSetMap.end();
         iter != eiter; ++iter)
    {
        for (CHISet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
    ...
    for (FunToEntryChiSetMap::iterator iter = funToEntryChiSetMap.begin(), eiter = funToEntryChiSetMap.end();
         iter != eiter; ++iter)
    {
        for (CHISet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
    ...
    for (FunToReturnMuSetMap::iterator iter = funToReturnMuSetMap.begin(), eiter = funToReturnMuSetMap.end();
         iter != eiter; ++iter)
    {
        for (MUSet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
    ...
    for (BBToPhiSetMap::iterator iter = bb2PhiSetMap.begin(), eiter = bb2PhiSetMap.end(); iter != eiter; ++iter)
    {
        for (PHISet::iterator it = iter->second.begin(), eit = iter->second.end(); it != eit; ++it)
        {
            delete *it;
        }
    }
...
{
    if (pta->printStat()) stat->performStat();
}
...
void MemSSA::dumpMSSA(OutStream& Out)
{
    ...
    for (SVFModule::iterator fit = pta->getModule()->begin(), efit = pta->getModule()->end(); fit != efit; ++fit)
    {
        const SVFFunction* fun = *fit;
        if (Options::MSSAFun() != "" && Options::MSSAFun() != fun->getName()) continue;
            ...
            for (SVFBasicBlock::const_iterator it = bb->begin(), eit = bb->end(); it != eit; ++it)
            {
                ...
                if (isAppCall || isHeapAllocExtCall(inst))
                {
                    ...
                    if (hasMU(cs))
                    {
                        ...
                        for (MUSet::iterator mit = getMUSet(cs).begin(), emit = getMUSet(cs).end(); mit != emit; ++mit)
                        {
                            (*mit)->dump();
                        }
                    }
                    ...
                    if (hasCHI(cs))
                    {
                        for (CHISet::iterator cit = getCHISet(cs).begin(), ecit = getCHISet(cs).end(); cit != ecit;
                             ++cit)
                        {
                            (*cit)->dump();
                        }
