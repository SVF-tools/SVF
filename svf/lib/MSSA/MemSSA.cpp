...
void MemSSA::dumpMSSA(OutStream& Out)
{
    ...
    for (SVFModule::const_callgraphnode_iterator fit = pta->getModule()->callgraphnode_begin(), efit = pta->getModule()->callgraphnode_end();
            fit != efit; ++fit)
    {
        const SVFFunction* fun = (*fit)->getFunction();
