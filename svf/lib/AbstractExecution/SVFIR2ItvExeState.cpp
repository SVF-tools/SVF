...
{
    if (gep->getOffsetVarIterTypePairVec().empty())
        return std::make_pair(gep->getConstantFieldIdx(), gep->getConstantFieldIdx());

    s32_t totalOffsetLb = 0;
    s32_t totalOffsetUb = 0;
    for (int i = gep->getOffsetVarIterTypePairVec().size() - 1; i >= 0; i--)
    {
        const SVFValue *value = gep->getOffsetVarIterTypePairVec()[i].first->getValue();
        const SVFType *type = gep->getOffsetVarIterTypePairVec()[i].second;
