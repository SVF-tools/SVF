...
public:
    ...
    typedef Map<const PAGEdge*, const CallGraphNode*> PAGEdgeToFunMap;
    typedef OrderedSet<NodeBS, SVFUtil::equalNodeBS> PointsToList;
    typedef Map<const CallGraphNode*, NodeBS> FunToPointsToMap;
    typedef Map<const CallGraphNode*, PointsToList> FunToPointsTosMap;
    typedef OrderedMap<NodeBS, NodeBS, SVFUtil::equalNodeBS> PtsToRepPtsSetMap;

    /// Map a function to its region set
    typedef Map<const CallGraphNode*, MRSet> FunToMRsMap;
    ...
    typedef Map<const CallGraphNode*, NodeBS> FunToNodeBSMap;
...
private:
    ...
    bool isNonLocalObject(NodeID id, const CallGraphNode* curFun) const;
...
protected:
    ...
    void createMR(const CallGraphNode* fun, const NodeBS& cpts);
    ...
    virtual inline void getAliasMemRegions(MRSet& aliasMRs, const NodeBS& cpts, const CallGraphNode* fun)
    {
    ...
    virtual inline void getMRsForLoad(MRSet& aliasMRs, const NodeBS& cpts, const CallGraphNode*)
    {
    ...
    virtual inline void getMRsForCallSiteRef(MRSet& aliasMRs, const NodeBS& cpts, const CallGraphNode*)
    {
        const MemRegion* mr = getMR(cpts);
        aliasMRs.insert(mr);
    }
    ...
    virtual bool handleCallsiteModRef(NodeBS& mod, NodeBS& ref, const CallICFGNode* cs, const CallGraphNode* fun);


    /// Add cpts to store/load
    //@{
    inline void addCPtsToStore(NodeBS& cpts, const StoreStmt *st, const CallGraphNode* fun)
    {
        storesToPointsToMap[st] = cpts;
        funToPointsToMap[fun].insert(cpts);
        addModSideEffectOfFunction(fun,cpts);
    }
    inline void addCPtsToLoad(NodeBS& cpts, const LoadStmt *ld, const CallGraphNode* fun)
    {
    ...
    inline bool hasCPtsList(const CallGraphNode* fun) const
    {
        return funToPointsToMap.find(fun)!=funToPointsToMap.end();
    }
    inline PointsToList& getPointsToList(const CallGraphNode* fun)
    {
    ...
    void addRefSideEffectOfFunction(const CallGraphNode* fun, const NodeBS& refs);
    /// Add indirect def an memory object in the function
    void addModSideEffectOfFunction(const CallGraphNode* fun, const NodeBS& mods);
    /// Add indirect uses an memory object in the function
    ...
    inline const NodeBS& getRefSideEffectOfFunction(const CallGraphNode* fun)
    {
        return funToRefsMap[fun];
    }
    /// Get indirect mods of a function
    inline const NodeBS& getModSideEffectOfFunction(const CallGraphNode* fun)
    {
...
public:
    ...
    const CallGraphNode* getFunction(const PAGEdge* pagEdge) const
    {
    ...
    //@{
    inline MRSet& getFunMRSet(const CallGraphNode* fun)
    {
