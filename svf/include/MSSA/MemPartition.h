...
protected:
    ...
    virtual void getMRsForLoad(MRSet& aliasMRs, const NodeBS& cpts, const CallGraphNode* fun);

    /// Get memory regions to be inserted at a load statement.
    virtual void getMRsForCallSiteRef(MRSet& aliasMRs, const NodeBS& cpts, const CallGraphNode* fun);
private:
    /// Create memory regions for each points-to target.
    void createDistinctMR(const CallGraphNode* func, const NodeBS& cpts);
...
public:
    ...
    typedef Map<const CallGraphNode*, PtsToSubPtsMap> FunToPtsMap;
    typedef Map<const CallGraphNode*, PointsToList> FunToInterMap;
...
protected:
    ...
    void createDisjointMR(const CallGraphNode* func, const NodeBS& cpts);
...
private:
    inline PtsToSubPtsMap& getPtsSubSetMap(const CallGraphNode* func)
    {
        return funcToPtsMap[func];
    }

    inline PointsToList& getIntersList(const CallGraphNode* func)
    {
        return funcToInterMap[func];
    }

    inline const PtsToSubPtsMap& getPtsSubSetMap(const CallGraphNode* func) const
    {
