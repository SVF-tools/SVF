...
template <typename Key, typename KeySet, typename Data, typename DataSet> class MutableDFPTData;
...
public:
    ...
    /// Constructor
    explicit MutableDiffPTData(bool reversePT = true, PTDataTy ty = PTDataTy::Diff)
...
public:
    ...
    virtual inline bool hasDFInSet(LocID loc, const Key& var) const override
    {
        ...
        if (it == dfInPtsMap.end()) return false;
    ...
    virtual inline bool hasDFOutSet(LocID loc, const Key& var) const override
    {
        ...
        if (it == dfOutPtsMap.end()) return false;
        const PtsMap& ptsMap = it->second;
        return (ptsMap.find(var) != ptsMap.end());
    }
    ...
    virtual inline bool updateAllDFOutFromIn(LocID loc, const Key& singleton, bool strongUpdates) override
    {
        ...
        if (this->hasDFInSet(loc))
        {
            ...
            for (typename PtsMap::const_iterator ptsIt = ptsMap.begin(), ptsEit = ptsMap.end(); ptsIt != ptsEit;
                 ++ptsIt)
            {
