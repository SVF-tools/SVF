...
private:
    ...
    extractBBS(const SVFFunction *func,
               Map<const BasicBlockNode*, std::vector<const BasicBlockNode*>> &res);

    /// extract nodes between two nodes in pdom tree
    void
    extractNodesBetweenPdomNodes(const BasicBlockNode*succ, const BasicBlockNode*LCA,
                                 std::vector<const BasicBlockNode*> &tgtNodes);


    void dfsNodesBetweenPdomNodes(const BasicBlockNode*cur,
    ...
    s64_t getBBSuccessorBranchID(const BasicBlockNode*BB, const BasicBlockNode*Succ);


    /// update map
    inline void updateMap(const BasicBlockNode*pred, const BasicBlockNode*bb, s32_t pos)
    {
...
private:
    CDG *_controlDG;
    Map<const BasicBlockNode*, Map<const BasicBlockNode*, Set<s32_t>>> _svfcontrolMap; ///< map a basicblock to its controlling BBs (position, set of BBs)
    Map<const BasicBlockNode*, Map<const BasicBlockNode*, Set<s32_t>>> _svfdependentOnMap; ///< map a basicblock to its dependent on BBs (position, set of BBs)
