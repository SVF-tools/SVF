...
public:
    /// Constructor
    CxtProc(const CallStrCxt& c, const CallGraphNode* f) :
        cxt(c), fun(f)
    {
    }
    /// Copy constructor
    ...
    inline const CallGraphNode* getProc() const
    {
        return fun;
    }
    /// Return current context
...
public:
    /// Constructor
    CxtThreadProc(NodeID t, const CallStrCxt& c, const CallGraphNode* f) :CxtProc(c,f),tid(t)
    {
    }
    /// Copy constructor
...
template <> struct std::hash<SVF::CxtProc>
{
    ...
    {
        std::hash<SVF::CallGraphNode*> h;
        SVF::CallGraphNode* fun = const_cast<SVF::CallGraphNode*> (cs.getProc());
        return h(fun);
    }
};
