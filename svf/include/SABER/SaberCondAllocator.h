...
public:
    ...
    typedef Map<const SVFFunction*, BasicBlockSet>
        FunToExitBBsMap; ///< map a function to all its basic blocks calling program exit
    typedef Map<const SVFBasicBlock*, Condition>
        BBToCondMap; ///< map a basic block to its condition during control-flow guard computation
    ...
    {
        u32_t vmrss, vmsize;
        if (SVFUtil::getMemoryUsageKB(&vmrss, &vmsize)) return std::to_string(vmsize) + "KB";
        else
    ...
    virtual Condition ComputeInterCallVFGGuard(const SVFBasicBlock* src, const SVFBasicBlock* dst,
                                               const SVFBasicBlock* callBB);
    virtual Condition ComputeInterRetVFGGuard(const SVFBasicBlock* src, const SVFBasicBlock* dst,
    ...
    virtual Condition getPHIComplementCond(const SVFBasicBlock* BB1, const SVFBasicBlock* BB2,
    ...
    inline bool setCFCond(const SVFBasicBlock* bb, const Condition& cond)
    {
        ...
        if (it != bbToCondMap.end() && isEquivalentBranchCond(it->second, cond)) return false;
