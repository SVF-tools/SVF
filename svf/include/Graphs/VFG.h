...
public:
    ...
    typedef Map<const CallGraphNode*, VFGNodeSet > FunToVFGNodesMapTy;
...
public:
    ...
    ///@{
    inline VFGNodeSet& getVFGNodes(const CallGraphNode *fun)
    {
        return funToVFGNodesMap[fun];
    }
    inline bool hasVFGNodes(const CallGraphNode *fun) const
    {
        return funToVFGNodesMap.find(fun) != funToVFGNodesMap.end();
    }
    inline bool VFGNodes(const CallGraphNode *fun) const
    {
        return funToVFGNodesMap.find(fun) != funToVFGNodesMap.end();
    }
    inline VFGNodeSet::const_iterator getVFGNodeBegin(const CallGraphNode *fun) const
    {
    ...
    inline VFGNodeSet::const_iterator getVFGNodeEnd(const CallGraphNode *fun) const
    {
        FunToVFGNodesMapTy::const_iterator it = funToVFGNodesMap.find(fun);
        assert(it != funToVFGNodesMap.end() && "this function does not have any VFGNode");
...
protected:
    ...
    void addVFGInterEdges(const CallICFGNode* cs, const CallGraphNode* callee);
    ...
    virtual inline void addVFGNode(VFGNode* vfgNode, ICFGNode* icfgNode)
    {
        ...
        if(const CallGraphNode* cgn = icfgNode->getFun())
            funToVFGNodesMap[cgn].insert(vfgNode);
        else
    ...
    inline void addFormalParmVFGNode(const PAGNode* fparm, const CallGraphNode* fun, CallPESet& callPEs)
    {
    ...
    inline void addFormalRetVFGNode(const PAGNode* uniqueFunRet, const CallGraphNode* fun, RetPESet& retPEs)
    {
