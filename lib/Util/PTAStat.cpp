...
const char *PTAStat::NumOfObjsHasVarStruct =
    "VarStructObj"; ///< SVFIR object node has struct (maybe nested with array)
const char *PTAStat::NumOfObjsHasVarArray =
    "VarArrayObj"; ///< SVFIR object node has array (maybe nested with struct)
const char *PTAStat::NumOfObjsHasConstStruct =
    "ConstStructObj"; ///< SVFIR object node has const struct (maybe nested with
                      ///< array)
const char *PTAStat::NumOfObjsHasConstArray =
    "ConstArrayObj"; ///< SVFIR object node has const array (maybe nested with
                     ///< struct)
const char *PTAStat::NumOfNonPtrObjs =
    "NonPtrObj"; ///< SVFIR object node which is non pointer type object (do not
                 ///< have pts)
const char *PTAStat::NumOfConstantObjs =
    "ConstantObj"; ///< SVFIR object node which is purely scalar
...
const char *PTAStat::NumOfProcessedAddrs =
    "AddrProcessed"; ///< SVFIR addr processed edge
const char *PTAStat::NumOfProcessedLoads =
    "LoadProcessed"; ///< SVFIR load processed edge
const char *PTAStat::NumOfProcessedStores =
    "StoreProcessed"; ///< SVFIR store processed edge
const char *PTAStat::NumOfProcessedCopys =
    "CopyProcessed"; ///< SVFIR copy processed edge
const char *PTAStat::NumOfProcessedGeps =
    "GepProcessed"; ///< SVFIR gep processed edge
...
const char *PTAStat::NumOfPointers =
    "Pointers"; ///< SVFIR value node, each of them maps to a llvm value
const char *PTAStat::NumOfGepFieldPointers =
    "DYFieldPtrs"; ///< SVFIR gep value node (field value, dynamically created
                   ///< dummy node)

const char *PTAStat::NumOfMemObjects =
    "MemObjects"; ///< SVFIR object node, each of them maps to a llvm value
const char *PTAStat::NumOfGepFieldObjects =
    "DYFieldObjs"; ///< SVFIR gep object node (field obj, dynamically created
                   ///< dummy node)
...
PTAStat::PTAStat(PointerAnalysis *p) : startTime(0), endTime(0), pta(p) {
  assert((Options::ClockType == ClockType::Wall ||
          Options::ClockType == ClockType::CPU) &&
...
void PTAStat::callgraphStat() {
  ...
  PointerAnalysis::CallGraphSCC *callgraphSCC =
      new PointerAnalysis::CallGraphSCC(graph);
  callgraphSCC->find();
...
void PTAStat::printStat(string statname) {
  ...
  SVFUtil::outs()
      << "-------------------------------------------------------\n";
  for (TIMEStatMap::iterator it = timeStatMap.begin(), eit = timeStatMap.end();
       it != eit; ++it) {
    // format out put with width 20 space
    SVFUtil::outs() << std::setw(field_width) << it->first << it->second
                    << "\n";
  }
  for (NUMStatMap::iterator it = PTNumStatMap.begin(), eit = PTNumStatMap.end();
       it != eit; ++it) {
    // format out put with width 20 space
    SVFUtil::outs() << std::setw(field_width) << it->first << it->second
                    << "\n";
...
void PTAStat::bitcastInstStat()
{
    ...
    for (SVFModule::llvm_const_iterator funIter = module->llvmFunBegin(),
funEiter = module->llvmFunEnd(); funIter != funEiter; ++funIter)
    {
                ...
                bbIt != bbEit; ++bbIt)
        {
            ...
            for (BasicBlock::const_iterator instIt = bb.begin(), instEit =
bb.end(); instIt != instEit; ++instIt)
            {
                const Instruction& inst = *instIt;
                if (const BitCastInst* bitcast =
