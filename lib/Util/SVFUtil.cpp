...
void SVFUtil::dumpPointsToList(const PointsToList &ptl) {
  outs() << "{";
  for (PointsToList::const_iterator ii = ptl.begin(), ie = ptl.end(); ii != ie;
       ii++) {
    auto bs = *ii;
    dumpSet(bs);
  }
...
std::string SVFUtil::getSourceLoc(const Value *val) {
  ...
  if (const Instruction *inst = SVFUtil::dyn_cast<Instruction>(val)) {
    ...
    rawstr << "Can only get source location for instruction, argument, global "
              "var, function or constant data.";
  }
  rawstr << " }";
