...
void ExeState::initValVar(const ValVar *valVar, Z3Expr &e) {
  ...
  if (const Type *type = valVar->getType()) {
    if (type->isIntegerTy() || type->isFloatingPointTy() ||
        type->isPointerTy() || type->isFunctionTy() || type->isStructTy() ||
        type->isArrayTy() || type->isVoidTy() || type->isLabelTy() ||
        type->isMetadataTy())
    ...
    if (svfir->getNullPtr() == valVar->getId())
      ...
      e = getContext().int_const(rawstr.str().c_str());
    assert(SVFUtil::isa<DummyValVar>(valVar) &&
           "not a DummValVar if it has no type?");
  }
}
...
void ExeState::initObjVar(const ObjVar *objVar, Z3Expr &e) {
  ...
  if (objVar->hasValue()) {
    ...
    /// constant data
    if (obj->isConstantData() || obj->isConstantArray() ||
        obj->isConstantStruct()) {
      if (const ConstantInt *consInt =
              SVFUtil::dyn_cast<ConstantInt>(obj->getValue())) {
        if (consInt == llvm::ConstantInt::getTrue(
      ...
      } else if (const llvm::ConstantFP *consFP =
                     SVFUtil::dyn_cast<llvm::ConstantFP>(obj->getValue()))
        e = getContext().int_val(
            static_cast<u32_t>(consFP->getValue().convertToFloat()));
    ...
    assert(SVFUtil::isa<DummyObjVar>(objVar) &&
           "it should either be a blackhole or constant dummy if this obj has "
           "no value?");
    e = getContext().int_val(getVirtualMemAddress(objVar->getId()));
  }
}
...
bool ExeState::operator==(const ExeState &rhs) const {
  return eq(pathConstraint, rhs.getPathConstraint()) &&
         eqVarToValMap(varToVal, rhs.getVarToVal()) &&
...
bool ExeState::operator<(const ExeState &rhs) const {
  ...
  if (lessThanVarToValMap(varToVal, rhs.getVarToVal()) ||
      lessThanVarToValMap(locToVal, rhs.getLocToVal()))
    return true;
  return false;
}

bool ExeState::eqVarToValMap(const VarToValMap &lhs,
...
bool ExeState::lessThanVarToValMap(const VarToValMap &lhs,
                                   const VarToValMap &rhs) const {
  if (lhs.size() != rhs.size())
...
void ExeState::store(const Z3Expr &loc, const Z3Expr &value) {
  ...
  assert(isVirtualMemAddress(virAddr) &&
