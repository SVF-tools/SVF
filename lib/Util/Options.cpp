...
const llvm::cl::opt<BVDataPTAImpl::PTBackingType> Options::ptDataBacking(
    "ptd", llvm::cl::init(BVDataPTAImpl::PTBackingType::Persistent),
    llvm::cl::desc("Overarching points-to data structure"),
    llvm::cl::values(
        clEnumValN(BVDataPTAImpl::PTBackingType::Mutable, "mutable",
                   "points-to set per pointer"),
        clEnumValN(BVDataPTAImpl::PTBackingType::Persistent, "persistent",
                   "points-to set ID per pointer, operations hash-consed")));

const llvm::cl::opt<unsigned> Options::FsTimeLimit(
    "fs-time-limit", llvm::cl::init(0),
    llvm::cl::desc("time limit for main phase of flow-sensitive analyses"));

const llvm::cl::opt<unsigned> Options::VersioningThreads(
    "versioning-threads", llvm::cl::init(1),
    llvm::cl::desc("number of threads to use in the versioning phase of "
                   "versioned flow-sensitive analysis"));

const llvm::cl::opt<unsigned> Options::AnderTimeLimit(
    "ander-time-limit", llvm::cl::init(0),
    llvm::cl::desc("time limit for Andersen's analyses (ignored when "
                   "-fs-time-limit set)"));
...
const llvm::cl::opt<unsigned> Options::MaxStepInWrapper(
    "max-step", llvm::cl::init(10),
    llvm::cl::desc("Maximum steps when traversing on SVFG to identify a memory "
                   "allocation wrapper"));
...
const llvm::cl::opt<bool> Options::ShowSVFIRValue(
    "show-ir-value", llvm::cl::init(true),
...
const llvm::cl::opt<bool> Options::ClusterAnder(
    "cluster-ander", llvm::cl::init(false),
    llvm::cl::desc(
        "Stage Andersen's with Steensgard's and cluster based on that"));

const llvm::cl::opt<bool> Options::ClusterFs(
    "cluster-fs", llvm::cl::init(false),
    llvm::cl::desc("Cluster for FS/VFS with auxiliary Andersen's"));

const llvm::cl::opt<bool> Options::PlainMappingFs(
    "plain-mapping-fs", llvm::cl::init(false),
    llvm::cl::desc("Use an explicitly (not null) plain mapping for FS"));

const llvm::cl::opt<PointsTo::Type> Options::PtType(
    "pt-type", llvm::cl::init(PointsTo::Type::SBV),
    llvm::cl::desc("points-to set data structure to use in all analyses"),
    llvm::cl::values(
        clEnumValN(PointsTo::Type::SBV, "sbv", "sparse bit-vector"),
        clEnumValN(PointsTo::Type::CBV, "cbv",
                   "core bit-vector (dynamic bit-vector without leading and "
                   "trailing 0s)"),
        clEnumValN(PointsTo::Type::BV, "bv",
                   "bit-vector (dynamic bit-vector without trailing 0s)")));

const llvm::cl::opt<enum hclust_fast_methods> Options::ClusterMethod(
    "cluster-method", llvm::cl::init(HCLUST_METHOD_SVF_BEST),
    llvm::cl::desc("hierarchical clustering method for objects"),
    // TODO: maybe add descriptions.
    llvm::cl::values(
        clEnumValN(HCLUST_METHOD_SINGLE, "single",
                   "single linkage; minimum spanning tree algorithm"),
        clEnumValN(HCLUST_METHOD_COMPLETE, "complete",
                   "complete linkage; nearest-neighbour-chain algorithm"),
        clEnumValN(
            HCLUST_METHOD_AVERAGE, "average",
            "unweighted average linkage; nearest-neighbour-chain algorithm"),
        clEnumValN(HCLUST_METHOD_SVF_BEST, "best",
                   "try all linkage criteria; choose best")));

const llvm::cl::opt<bool> Options::RegionedClustering(
    // Use cluster to "gather" the options closer together, even if it sounds a
    // little worse.
    "cluster-regioned", llvm::cl::init(true),
    llvm::cl::desc("cluster regions separately"));

const llvm::cl::opt<bool> Options::RegionAlign(
    "cluster-region-aligned", llvm::cl::init(true),
    llvm::cl::desc("align each region's identifiers to the native word size"));

const llvm::cl::opt<bool> Options::PredictPtOcc(
    "cluster-predict-occ", llvm::cl::init(false),
    llvm::cl::desc("try to predict which points-to sets are more important in "
                   "staged analysis"));
...
const llvm::cl::opt<MemSSA::MemPartition> Options::MemPar(
    "mem-par", llvm::cl::init(MemSSA::MemPartition::IntraDisjoint),
    llvm::cl::desc(
        "Memory region partiion strategies (e.g., for SVFG construction)"),
    llvm::cl::values(
        clEnumValN(MemSSA::MemPartition::Distinct, "distinct",
                   "memory region per each object"),
        clEnumValN(MemSSA::MemPartition::IntraDisjoint, "intra-disjoint",
                   "memory regions partioned based on each function"),
        clEnumValN(MemSSA::MemPartition::InterDisjoint, "inter-disjoint",
                   "memory regions partioned across functions")));
...
const llvm::cl::opt<bool>
    Options::PrintValidRes("mhp-validation", llvm::cl::init(false),
                           llvm::cl::desc("Print MHP Validation Results"));
// LockResultValidator.cpp
const llvm::cl::opt<bool>
    Options::LockValid("lock-validation", llvm::cl::init(false),
...
const llvm::cl::opt<bool> Options::ModelArrays(
    "model-arrays", llvm::cl::init(false),
...
const llvm::cl::opt<bool> Options::SABERFULLSVFG(
    "saber-full-svfg", llvm::cl::init(false),
    llvm::cl::desc("When using SABER for bug detection pass, enable full svfg "
                   "on top of the pointer-only one"));
...
llvm::cl::bits<PointerAnalysis::PTATY> Options::PASelected(
    ...
    llvm::cl::values(
        ...
        clEnumValN(PointerAnalysis::TypeCPP_WPA, "type",
                   "Type-based fast analysis for Callgraph, SVFIR and CHA")));
...
const llvm::cl::opt<bool> Options::ShowHiddenNode(
    "show-hidden-nodes", llvm::cl::init(false),
    llvm::cl::desc(
