...
NodeID NodeIDAllocator::allocateObjectId(void) {
  ...
  if (strategy == Strategy::DENSE) {
    // We allocate objects from 0(-ish, considering the special nodes) to # of
    // objects.
    id = numObjects;
...
NodeID NodeIDAllocator::allocateGepObjectId(NodeID base, u32_t offset,
  ...
  if (strategy == Strategy::DENSE) {
    ...
    // For a gep id, base id is set at lower bits, and offset is set at higher
    // bits e.g., 1100050 denotes base=50 and offset=10 The offset is 10, not
    // 11, because we add 1 to the offset to ensure that the high bits are never
    // 0. For example, we do not want the gep id to be 50 when the base is 50
    // and the offset is 0.
    NodeID gepMultiplier = pow(
        10,
        ceil(log10(numSymbols > maxFieldLimit ? numSymbols : maxFieldLimit)));
    id = (offset + 1) * gepMultiplier + base;
    assert(id > numSymbols && "NodeIDAllocator::allocateGepObjectId: GEP "
                              "allocation clashing with other nodes");
  } else {
    assert(false && "NodeIDAllocator::allocateGepObjectId: unimplemented node "
                    "allocation strategy");
  }
...
const std::string NodeIDAllocator::Clusterer::DistanceMatrixTime =
    "DistanceMatrixTime";
const std::string NodeIDAllocator::Clusterer::FastClusterTime =
    "FastClusterTime";
const std::string NodeIDAllocator::Clusterer::DendrogramTraversalTime =
    "DendrogramTravTime";
...
const std::string NodeIDAllocator::Clusterer::TheoreticalNumWords =
    "TheoreticalWords";
const std::string NodeIDAllocator::Clusterer::OriginalBvNumWords =
    "OriginalBvWords";
const std::string NodeIDAllocator::Clusterer::OriginalSbvNumWords =
    "OriginalSbvWords";
...
const std::string NodeIDAllocator::Clusterer::NumGtIntRegions =
    "NumGtIntRegions";
...
const std::string NodeIDAllocator::Clusterer::NumNonTrivialRegionObjects =
    "NumNonTrivObj";

std::vector<NodeID> NodeIDAllocator::Clusterer::cluster(
    BVDataPTAImpl *pta, const std::vector<std::pair<NodeID, unsigned>> keys,
    std::vector<std::pair<hclust_fast_methods, std::vector<NodeID>>>
        &candidates,
  ...
  assert(Options::NodeAllocStrat == Strategy::DENSE &&
         "Clusterer::cluster: only dense allocation clustering currently "
         "supported");
  ...
  // Pair of nodes to their (minimum) distance and the number of occurrences of
  // that distance.
  ...
  // Set needs to be ordered because getDistanceMatrix, in its n^2 iteration,
  // expects sets to be ordered (we are building a condensed matrix, not a full
  // matrix, so it matters). In getDistanceMatrix, doing regionReverseMapping
  // for oi and oj, where oi < oj, and getting a result moi > moj gives
  // incorrect results. In the condensed matrix, [b][a] where b >= a, is
  // incorrect.
  std::vector<OrderedSet<NodeID>> regionsObjects(numRegions);
  for (NodeID o = 0; o < numObjects; ++o)
    regionsObjects[objectsRegion[o]].insert(o);

  // Size of the return node mapping. It is potentially larger than the number
  // of objects because we align each region to NATIVE_INT_SIZE. size_t
  // numMappings = 0;
  ...
  std::vector<std::vector<std::pair<const PointsTo *, unsigned>>>
      regionsPointsTos(numRegions);
  for (const Map<PointsTo, unsigned>::value_type &ptocc : pointsToSets) {
    ...
    if (pt.empty())
      continue;
    // Guaranteed that begin() != end() because of the continue above. All
    // objects in pt will be relevant to the same region.
    unsigned region = objectsRegion[*(pt.begin())];
    // In our "graph", objects in the same points-to set have an edge between
    // them, so they are all in the same connected component/region.
  ...
  for (const hclust_fast_methods method : methods) {
    ...
    for (unsigned region = 0; region < numRegions; ++region) {
      ...
      if (Options::RegionAlign) {
        allocCounter =
            ((allocCounter + NATIVE_INT_SIZE - 1) / NATIVE_INT_SIZE) *
            NATIVE_INT_SIZE;
      }

      if (regionNumObjects > largestRegion)
        largestRegion = regionNumObjects;
    ...
    // Though we "update" these in the loop, they will be the same every
    // iteration.
    overallStats[NumGtIntRegions] = std::to_string(numGtIntRegions);
    overallStats[LargestRegion] = std::to_string(largestRegion);
    overallStats[NumNonTrivialRegionObjects] =
        std::to_string(nonTrivialRegionObjects);
  }
  ...
  overallStats[DendrogramTraversalTime] =
      std::to_string(dendrogramTraversalTime);
  ...
  overallStats[TotalTime] =
      std::to_string(distanceMatrixTime + dendrogramTraversalTime +
                     fastClusterTime + regioningTime + evalTime);

  overallStats[BestCandidate] =
      SVFUtil::hclustMethodToString(bestMapping.first);
  printStats(evalSubtitle + ": overall", overallStats);

  return bestMapping.second;
}

std::vector<NodeID> NodeIDAllocator::Clusterer::getReverseNodeMapping(
    const std::vector<NodeID> &nodeMapping) {
  // nodeMapping.size() may not be big enough because we leave some gaps, but
  // it's a start.
  ...
  for (size_t i = 0; i < nodeMapping.size(); ++i) {
    ...
    if (mapsTo >= reverseNodeMapping.size())
      reverseNodeMapping.resize(mapsTo + 1, UINT_MAX);
...
size_t NodeIDAllocator::Clusterer::condensedIndex(size_t n, size_t i,
                                                  size_t j) {
...
unsigned NodeIDAllocator::Clusterer::requiredBits(const size_t n) {
  if (n == 0)
    return 0;
  // Ceiling of number of bits amongst each native integer gives needed native
  // ints, so we then multiply again by the number of bits in each native int.
  return ((n - 1) / NATIVE_INT_SIZE + 1) * NATIVE_INT_SIZE;
}

double *NodeIDAllocator::Clusterer::getDistanceMatrix(
    const std::vector<std::pair<const PointsTo *, unsigned>> pointsToSets,
  ...
  for (size_t i = 0; i < condensedSize; ++i)
    distMatrix[i] = numObjects * numObjects;
    ...
    for (size_t i = 0; i < ptsVec.size(); ++i) {
      ...
      for (size_t j = i + 1; j < ptsVec.size(); ++j) {
        ...
        double &existingDistance =
            distMatrix[condensedIndex(numObjects, moi->second, moj->second)];

        // Subtract extra occurrenceEpsilon to make upcoming logic simpler.
        // When existingDistance is never whole, it is always between two
        // distances.
        if (distance < existingDistance)
          existingDistance = distance - occurrenceEpsilon;

        if (distance == std::ceil(existingDistance)) {
          // We have something like distance == x, existingDistance == x - e,
          // for some e < 1 (potentially even set during this iteration). So,
          // the new distance is an occurrence the existingDistance being
          // tracked, it just had some reductions because of multiple
          // occurences. If there is not room within this distance to reduce
          // more (increase priority), just ignore it. TODO: maybe warn?
          if (existingDistance - occ * occurrenceEpsilon >
              std::floor(existingDistance)) {
...
void NodeIDAllocator::Clusterer::traverseDendrogram(
    std::vector<NodeID> &nodeMap, const int *dendrogram,
  ...
  if (left < 0) {
    ...
    traverseDendrogram(nodeMap, dendrogram, numObjects, allocCounter, visited,
                       left, regionNodeMap);
  }
...
std::vector<NodeID>
NodeIDAllocator::Clusterer::regionObjects(const Map<NodeID, Set<NodeID>> &graph,
                                          size_t numObjects,
                                          size_t &numLabels) {
  ...
  for (const Map<NodeID, Set<NodeID>>::value_type &oos : graph) {
    ...
    while (!bfsQueue.empty()) {
      ...
      for (const NodeID neighbour : neighboursIt->second)
        bfsQueue.push(neighbour);
    }
  }

  // The remaining objects have no relation with others: they get their own
  // label.
...
void NodeIDAllocator::Clusterer::evaluate(
    const std::vector<NodeID> &nodeMap,
...
// Work out which of the mappings we generated looks best.
std::pair<hclust_fast_methods, std::vector<NodeID>>
NodeIDAllocator::Clusterer::determineBestMapping(
    const std::vector<std::pair<hclust_fast_methods, std::vector<NodeID>>>
        &candidates,
    Map<PointsTo, unsigned> pointsToSets, const std::string &evalSubtitle,
    double &evalTime) {
  ...
  if (evalSubtitle != "" || Options::ClusterMethod == HCLUST_METHOD_SVF_BEST) {
    for (const std::pair<hclust_fast_methods, std::vector<NodeID>> &candidate :
         candidates) {
      Map<std::string, std::string> candidateStats;
      hclust_fast_methods candidateMethod = candidate.first;
      std::string candidateMethodName =
          SVFUtil::hclustMethodToString(candidateMethod);
      ...
      printStats(evalSubtitle + ": candidate " + candidateMethodName,
                 candidateStats);

      size_t candidateWords = 0;
      if (Options::PtType == PointsTo::SBV)
        candidateWords = std::stoull(candidateStats[NewSbvNumWords]);
...
void NodeIDAllocator::Clusterer::printStats(
    std::string subtitle, Map<std::string, std::string> &stats) {
  // When not in order, it is too hard to compare original/new SBV/BV words, so
  // this array forces an order.
