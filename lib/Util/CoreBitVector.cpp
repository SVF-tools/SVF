...
bool CoreBitVector::test(u32_t bit) const {
  if (bit < offset || bit >= offset + words.size() * WordSize)
    return false;
...
bool CoreBitVector::operator==(const CoreBitVector &rhs) const {
  ...
  while (lhsSetIndex < words.size() && rhsSetIndex < rhs.words.size()) {
    ...
    if (lhsSetIndex * WordSize + offset !=
            rhsSetIndex * WordSize + rhs.offset ||
...
bool CoreBitVector::operator|=(const CoreBitVector &rhs) {
  ...
  if (finalBit() < rhs.finalBit())
    extendForward(rhs.finalBit());
  if (offset > rhs.offset)
    extendBackward(rhs.offset);
...
bool CoreBitVector::operator&=(const CoreBitVector &rhs) {
  ...
  Word oldWord;
  for (; thisIndex < words.size() && rhsIndex < rhs.words.size();
       ++thisIndex, ++rhsIndex) {
    if (!changed)
      oldWord = words[thisIndex];
    words[thisIndex] &= rhs.words[rhsIndex];
    if (!changed)
      changed = oldWord != words[thisIndex];
  }

  // Clear the remaining bits with no rhs analogue.
  for (; thisIndex < words.size(); ++thisIndex) {
    if (!changed && words[thisIndex] != 0)
      changed = true;
    words[thisIndex] = 0;
  }
...
void CoreBitVector::intersectWithComplement(const CoreBitVector &lhs,
...
void CoreBitVector::extendBackward(u32_t bit) {
  ...
  // Check if bit can already be included in this BV or if it's extendForward's
  // problem.
  if (newOffset >= offset)
    return;
...
void CoreBitVector::extendForward(u32_t bit) {
  ...
  u32_t wordsToAdd =
      1 + (bitsWord - words.size() * WordSize - offset) / WordSize;
...
CoreBitVector::CoreBitVectorIterator::CoreBitVectorIterator(
    const CoreBitVector *cbv, bool end)
    : cbv(cbv), bit(0) {
  ...
  if (wordIt != cbv->words.end() && !(cbv->words[0] & (Word)0b1))
    ++(*this);
}

const CoreBitVector::CoreBitVectorIterator &
CoreBitVector::CoreBitVectorIterator::operator++(void) {
  ...
  if (bit == WordSize || (*wordIt >> bit) == 0) {
    bit = 0;
    ++wordIt;
    while (wordIt != cbv->words.end() && *wordIt == 0)
      ++wordIt;
  }

  // Find set bit if we're not at the end.
  if (wordIt != cbv->words.end()) {
    while (bit != WordSize && (*wordIt & ((Word)0b1 << bit)) == 0)
      ++bit;
  }
...
const CoreBitVector::CoreBitVectorIterator
CoreBitVector::CoreBitVectorIterator::operator++(int) {
  assert(!atEnd() && "CoreBitVectorIterator::++(pre): incrementing past end!");
...
bool CoreBitVector::CoreBitVectorIterator::operator==(
    const CoreBitVectorIterator &rhs) const {
  assert(cbv == rhs.cbv &&
