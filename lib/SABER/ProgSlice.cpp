    ...
    for (SVFGNode::const_iterator it = node->OutEdgeBegin(),
      ...
      if (inBackwardSlice(succ)) {
        ...
        if (edge->isCallVFGEdge()) {
          vfCond = ComputeInterCallVFGGuard(nodeBB, succBB,
                                            getCallSite(edge)->getParent());
        } else if (edge->isRetVFGEdge()) {
          vfCond = ComputeInterRetVFGGuard(nodeBB, succBB,
                                           getRetSite(edge)->getParent());
        } else
...
std::string ProgSlice::evalFinalCond() const {
  ...
  for (NodeBS::iterator it = elems.begin(), eit = elems.end(); it != eit;
       ++it) {
  ...
  for (Set<std::string>::iterator iter = locations.begin(),
                                  eiter = locations.end();
       iter != eiter; ++iter) {
    rawstr << "\t\t  --> (" << *iter << ") \n";
  }
