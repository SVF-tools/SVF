...
void LeakChecker::initSrcs() {
  ...
  for (SVFIR::CSToRetMap::iterator it = pag->getCallSiteRets().begin(),
    ...
    /// about its leaks for example instruction `int* p = malloc(size)` is in a
    /// dead function, then program won't allocate this memory for example a
    /// customized malloc `int p = malloc()` returns an integer value, then
    /// program treat it as a system malloc
      ...
      if (isSinkLikeFun(fun)) {
        ...
        for (SVFIR::SVFVarList::const_iterator ait = arglist.begin(),
          ...
          if (pagNode->isPointer()) {
            ...
            // For any multi-level pointer e.g., XFree(void** pagNode) that
            // passed into a ExtAPI::EFT_FREE_MULTILEVEL function (e.g., XFree),
            // we will add the DstNode of a load edge, i.e., dummy = *pagNode
            SVFStmt::SVFStmtSetTy &loads =
                const_cast<PAGNode *>(pagNode)->getOutgoingEdges(SVFStmt::Load);
...
void LeakChecker::validateSuccessTests(const SVFGNode *source,
  ...
  if (fun->getName() == "SAFEMALLOC") {
    ...
    writeWrnMsg("\t can not validate, check function not found, please put it "
                "at the right place!!");
    return;
  }
  ...
  if (success) {
    outs() << sucMsg("\t SUCCESS :") << funName
           ...
           << SVFUtil::value2String(getSrcCSID(source)->getCallSite())
