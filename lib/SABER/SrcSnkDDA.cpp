...
void SrcSnkDDA::analyze(SVFModule *module) {
  ...
  for (SVFGNodeSetIter iter = sourcesBegin(), eiter = sourcesEnd();
    ...
    /// if we touch a global, then we assume the client uses this memory until
    /// the program exits.
    if (getCurSlice()->isReachGlobal()) {
      ...
      for (SVFGNodeSetIter sit = getCurSlice()->sinksBegin(),
...
bool SrcSnkDDA::isInAWrapper(const SVFGNode *src, CallSiteSet &csIdSet) {
  ...
  while (!worklist.empty()) {
    ...
    if (visited.test(node->getId()) == 0)
      ...
      continue;
    // reaching maximum steps when traversing on SVFG to identify a memory
    // allocation wrapper
    if (step++ > Options::MaxStepInWrapper)
      return false;

    for (SVFGNode::const_iterator it = node->OutEdgeBegin(),
                                  eit = node->OutEdgeEnd();
         it != eit; ++it) {
      const SVFGEdge *edge = (*it);
      // assert(edge->isDirectVFGEdge() && "the edge should always be direct
      // VF");
      ...
      // (2) an intra indirect edge, we only track if the succ SVFGNode is a
      // load, which means we only track one level store-load pair . (3) do not
      // track for all other interprocedural edges.
      else {
...
void SrcSnkDDA::annotateSlice(ProgSlice *slice) {
  ...
  for (SVFGNodeSetIter it = slice->sinksBegin(), eit = slice->sinksEnd();
       it != eit; ++it)
    getSVFG()->getStat()->addToSinks(*it);
  for (SVFGNodeSetIter it = slice->forwardSliceBegin(),
                       eit = slice->forwardSliceEnd();
       it != eit; ++it)
    getSVFG()->getStat()->addToForwardSlice(*it);
  for (SVFGNodeSetIter it = slice->backwardSliceBegin(),
                       eit = slice->backwardSliceEnd();
       it != eit; ++it)
