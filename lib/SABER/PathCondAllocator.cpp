...
PathCondAllocator::PathCondAllocator()
    : condMgr(BDDExprManager::getBDDExprMgr()) {}
/*!
...
void PathCondAllocator::allocateForBB(const BasicBlock &bb) {
  ...
  if (succ_number > 1) {
    ...
    for (succ_const_iterator succ_it = succ_begin(&bb);
      ...
      // decide whether each bit of succ_index is 1 or 0, if (three successor)
      // succ_index is 000 then use C1^C2^C3 if 001 use C1^C2^negC3
...
PathCondAllocator::Condition *
PathCondAllocator::getEvalBrCond(const BasicBlock *bb, const BasicBlock *succ) {
...
void PathCondAllocator::setBranchCond(const BasicBlock *bb,
  ...
  /// FIXME: llvm getNumSuccessors allows duplicated block in the successors, it
  /// makes this assertion fail In this case we may waste a condition
  /// allocation, because the overwrite of the previous cond
  // assert(condPosMap.find(pos) == condPosMap.end() && "this branch has already
  // been set ");
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::evaluateTestNullLikeExpr(const BranchStmt *branchStmt,
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::evaluateProgExit(const BranchStmt *branchStmt,
                                    const BasicBlock *succ) {
  const BasicBlock *succ1 = branchStmt->getSuccessor(0)->getBB();
...
PathCondAllocator::evaluateLoopExitBranch(const BasicBlock *bb,
  ...
  if (loopInfo->isLoopHeader(const_cast<BasicBlock *>(bb))) {
    ...
    /// if the dst dominate all other loop exit bbs, then dst can certainly be
    /// reached
    bool allPDT = true;
 ...
 */
PathCondAllocator::Condition *
PathCondAllocator::evaluateBranchCond(const BasicBlock *bb,
                                      const BasicBlock *succ) {
  if (getBBSuccessorNum(bb) == 1) {
  ...
  if (ICFGNode *icfgNode = getICFG()->getICFGNode(bb->getTerminator())) {
    for (const auto &svfStmt : icfgNode->getSVFStmts()) {
      if (const BranchStmt *branchStmt =
              SVFUtil::dyn_cast<BranchStmt>(svfStmt)) {
        if (branchStmt->getNumSuccessors() == 2) {
          ...
          Condition *evalTestNullLike =
              evaluateTestNullLikeExpr(branchStmt, succ);
          if (evalTestNullLike)
 ...
 *                       4. br i1 %tobool, label %if.end, label %if.then, !dbg
 * !161 There is an indirect edge 1->2 with value %0
 */
bool PathCondAllocator::isTestContainsNullAndTheValue(
...
PathCondAllocator::ComputeIntraVFGGuard(const BasicBlock *srcBB,
  ...
  while (!worklist.empty()) {
    ...
    for (succ_const_iterator succ_it = succ_begin(bb); succ_it != succ_end(bb);
         succ_it++) {
      const BasicBlock *succ = *succ_it;
