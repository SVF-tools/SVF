...
llvm::StringRef vtblLabelBeforeDemangle("_ZTV");

// label for virtual functions
llvm::StringRef vfunPreLabel("_Z");

llvm::StringRef clsName("class.");
llvm::StringRef structName("struct.");
...
{
    return !SVFUtil::isa<GlobalVariable>(val) &&
           val->getName().startswith(vtblLabelBeforeDemangle);
}
...
{
    return cs->getArgOperand(cs->paramHasAttr(0, llvm::Attribute::StructRet));
}
...
std::string LLVMUtil::getClassNameFromType(const Type* ty)
{
    ...
    if (const PointerType* ptrType = SVFUtil::dyn_cast<PointerType>(ty))
    {
        ...
        {
            llvm::StringRef elemTypeName = elemType->getStructName();
            if (elemTypeName.startswith(clsName))
            {
                className = elemTypeName.substr(clsName.size()).str();
            }
            else if (elemTypeName.startswith(structName))
            {
                className = elemTypeName.substr(structName.size()).str();
            }
        }
    }
    return className;
...
bool LLVMUtil::isConstructor(const Function* F)
{
    ...
    if (funcName.startswith(vfunPreLabel))
    {
        return false;
    }
    struct cppUtil::DemangledName dname = cppUtil::demangle(funcName.str());
    ...
    return dname.className == dname.funcName;
}
...
std::string LLVMUtil::getClassNameOfThisPtr(const CallBase* inst)
{
    ...
    if (thisPtrClassName.size() == 0)
    {
        ...
        thisPtrClassName = LLVMUtil::getClassNameFromType(thisPtr->getType());
    }
