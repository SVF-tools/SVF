...
void DDAStat::performStatPerQuery(NodeID ptr) {
  ...
  PointsTo pts;
  if (flowDDA) {
    for (FlowDDA::LocToDPMVecMap::const_iterator
             it = flowDDA->getLocToDPMVecMap().begin(),
    ...
    for (ContextDDA::LocToDPMVecMap::const_iterator
             it = contextDDA->getLocToDPMVecMap().begin(),
...
void DDAStat::performStat() {
  ...
  for (SVFIR::const_iterator nodeIt = SVFIR::getPAG()->begin(),
                             nodeEit = SVFIR::getPAG()->end();
  ...
  PTNumStatMap["NumOfStore"] =
      SVFIR::getPAG()->getPTASVFStmtSet(SVFStmt::Store).size();
...
void DDAStat::printStatPerQuery(NodeID ptr, const PointsTo &pts) {
  ...
  if (timeStatMap.empty() == false && NumPerQueryStatMap.empty() == false) {
    ...
    for (NUMStatMap::iterator it = NumPerQueryStatMap.begin(),
                              eit = NumPerQueryStatMap.end();
         it != eit; ++it) {
      // format out put with width 20 space
      SVFUtil::outs() << std::setw(field_width) << it->first << it->second
                      << "\n";
