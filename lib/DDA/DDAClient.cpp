...
OrderedNodeSet &FunptrDDAClient::collectCandidateQueries(SVFIR *p) {
  setPAG(p);
  for (SVFIR::CallSiteToFunPtrMap::const_iterator
           it = pag->getIndirectCallsites().begin(),
...
OrderedNodeSet &AliasDDAClient::collectCandidateQueries(SVFIR *pag) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = geps.begin(), eiter = geps.end();
       iter != eiter; ++iter) {
...
void AliasDDAClient::performStat(PointerAnalysis *pta) {
  ...
  for (PAGNodeSet::const_iterator lit = loadSrcNodes.begin();
       lit != loadSrcNodes.end(); lit++) {
    for (PAGNodeSet::const_iterator sit = storeDstNodes.begin();
         sit != storeDstNodes.end(); sit++) {
      ...
      if (node1->hasValue() && node2->hasValue()) {
        ...
        outs() << "\n=================================================\n";
        outs() << "Alias Query for ("
               << SVFUtil::value2String(node1->getValue()) << ",";
        outs() << SVFUtil::value2String(node2->getValue()) << ") \n";
        outs() << "[NodeID:" << node1->getId() << ", NodeID:" << node2->getId()
               << " " << result << "]\n";
        outs() << "=================================================\n";
      }
    }
