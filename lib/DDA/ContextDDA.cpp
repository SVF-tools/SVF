    ...
    if (isBlkObjOrConstantObj(ptd.get_id()))
      ...
      if (gepStmt->isVariantFieldGep()) {
        ...
        CxtVar var(ptd.get_cond(),
                   getGepObjVar(ptd.get_id(), gepStmt->getLocationSet()));
        tmpDstPts.set(var);
      }
    }
  }
  ...
  if (mem->isHeap()) {
    if (!mem->getValue()) {
      ...
      if (GepObjVar *gepobj = SVFUtil::dyn_cast<GepObjVar>(pnode)) {
        assert(
            SVFUtil::isa<DummyObjVar>(_pag->getGNode(gepobj->getBaseNode())) &&
            "emtpy refVal in a gep object whose base is a non-dummy object");
      } else {
        assert((SVFUtil::isa<DummyObjVar>(pnode) ||
                SVFUtil::isa<DummyValVar>(pnode)) &&
               "empty refVal in non-dummy object");
      }
      return true;
    } else if (const Instruction *mallocSite =
                   SVFUtil::dyn_cast<Instruction>(mem->getValue())) {
