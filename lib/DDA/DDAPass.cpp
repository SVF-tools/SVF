  ...
  if (!Options::UserInputQuery.empty()) {
    ...
    /// allow user specify queries
      ...
      if (Options::UserInputQuery != "all") {
        ...
        stringstream ss(Options::UserInputQuery); // Insert the user input
    ...
    for (; edgeIt != edgeEit; ++edgeIt) {
      ...
      if (edge->isCallVFGEdge() || edge->isRetVFGEdge()) {
        if (this->edgeInSVFGSCC(svfgSCC, edge)) {
          ...
          if (srcFun && dstFun) {
            NodeID src =
                pta->getPTACallGraph()->getCallGraphNode(srcFun)->getId();
            NodeID dst =
                pta->getPTACallGraph()->getCallGraphNode(dstFun)->getId();
