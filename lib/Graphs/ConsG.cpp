...
void ConstraintGraph::buildCG() {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = ngeps.begin(),
    ...
    if (edge->isVariantFieldGep())
      ...
      addNormalGepCGEdge(edge->getRHSVarID(), edge->getLHSVarID(),
                         edge->getLocationSet());
  }
...
NormalGepCGEdge *ConstraintGraph::addNormalGepCGEdge(NodeID src, NodeID dst,
  ...
  NormalGepCGEdge *edge =
      new NormalGepCGEdge(srcNode, dstNode, ls, edgeIndex++);
  bool added = directEdgeSet.insert(edge).second;
  assert(added && "not added??");
...
void ConstraintGraph::reTargetDstOfEdge(ConstraintEdge *edge,
  ...
  } else if (VariantGepCGEdge *gep =
                 SVFUtil::dyn_cast<VariantGepCGEdge>(edge)) {
    removeDirectEdge(gep);
...
bool ConstraintGraph::moveOutEdgesToRepNode(ConstraintNode *node,
  ...
  for (ConstraintNode::const_iterator it = node->OutEdgeBegin(),
                                      eit = node->OutEdgeEnd();
       it != eit; ++it) {
...
void ConstraintGraph::print() {
  ...
  for (ConstraintEdge::ConstraintEdgeSetTy::iterator iter = directs.begin(),
    ...
    } else if (NormalGepCGEdge *ngep =
                   SVFUtil::dyn_cast<NormalGepCGEdge>(*iter)) {
      outs() << ngep->getSrcID() << " -- NormalGep ("
             << ngep->getConstantFieldIdx() << ") --> " << ngep->getDstID()
             << "\n";
    } else if (VariantGepCGEdge *vgep =
                   SVFUtil::dyn_cast<VariantGepCGEdge>(*iter)) {
...
/// instruction
static std::string
getNodeLabel(NodeType *n, ConstraintGraph *) {
