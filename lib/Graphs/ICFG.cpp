...
FunEntryICFGNode::FunEntryICFGNode(NodeID id, const SVFFunction *f)
...
const std::string IntraCFGEdge::toString() const {
  ...
  if (getCondition() == nullptr)
    rawstr << "IntraCFGEdge: [ICFGNode" << getDstID() << " <-- ICFGNode"
           << getSrcID() << "]\t";
  else
    rawstr << "IntraCFGEdge: [ICFGNode" << getDstID() << " <-- ICFGNode"
           << getSrcID() << "] (branchCondition:" << *getCondition()
           << ") (succCondValue: " << getSuccessorCondValue() << ") \t";
...
const std::string CallCFGEdge::toString() const {
  ...
  rawstr << "CallCFGEdge "
         << " [ICFGNode";
  rawstr << getDstID() << " <-- ICFGNode" << getSrcID()
         << "]\t CallSite: " << *cs << "\t";
  return rawstr.str();
}
...
void ICFG::updateCallGraph(PTACallGraph *callgraph) {
  ...
  for (; iter != eiter; iter++) {
    ...
    for (PTACallGraph::FunctionSet::const_iterator func_iter =
      ...
      /// if this is an external function (no function body), connect
      /// calleeEntryNode to calleeExitNode
      if (isExtCall(callee))
        ...
        /// Remove callBlockNode to retBlockNode intraICFGEdge since we found at
        /// least one inter procedural edge
        if (ICFGEdge *edge = hasIntraICFGEdge(callBlockNode, retBlockNode,
                                              ICFGEdge::IntraCF))
          removeICFGEdge(edge);
      }
    }
