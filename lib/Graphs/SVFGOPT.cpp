...
SVFGEdge *SVFGOPT::addRetIndirectSVFGEdge(NodeID srcId, NodeID dstId,
                                          CallSiteID csid, const NodeBS &cpts) {
  if (Options::ContextInsensitive)
    return addIntraIndirectVFEdge(srcId, dstId, cpts);
  else
...
void SVFGOPT::handleInterValueFlow() {
  ...
  for (SVFGNodeSet::const_iterator it = candidates.begin(),
                                   eit = candidates.end();
       it != eit; ++it) {
    SVFGNode *node = *it;
  ...
  for (SVFGNodeSet::iterator it = nodesToBeDeleted.begin(),
                             eit = nodesToBeDeleted.end();
       it != eit; ++it) {
    SVFGNode *node = *it;
...
void SVFGOPT::replaceFParamARetWithPHI(PHISVFGNode *phi, SVFGNode *svfgNode) {
  ...
  for (SVFGNode::const_iterator it = svfgNode->OutEdgeBegin(),
                                eit = svfgNode->OutEdgeEnd();
       it != eit; ++it) {
    ...
    else if (const RetDirSVFGEdge *retEdge =
  ...
  if (FormalParmSVFGNode *fp =
          SVFUtil::dyn_cast<FormalParmSVFGNode>(svfgNode)) {
    for (SVFGNode::iterator it = svfgNode->InEdgeBegin(),
                            eit = svfgNode->InEdgeEnd();
         it != eit; ++it) {
      ActualParmSVFGNode *ap =
          SVFUtil::cast<ActualParmSVFGNode>((*it)->getSrcNode());
    ...
    for (SVFGNode::iterator it = svfgNode->InEdgeBegin(),
                            eit = svfgNode->InEdgeEnd();
         it != eit; ++it) {
      FormalRetSVFGNode *fr =
          SVFUtil::cast<FormalRetSVFGNode>((*it)->getSrcNode());
...
void SVFGOPT::retargetEdgesOfAInFOut(SVFGNode *node) {
  ...
  for (; it != eit; ++it) {
    ...
    SVFGNode *dstNode = outEdge->getDstNode();
    if (const CallIndSVFGEdge *callEdge =
    ...
    else if (const RetIndSVFGEdge *retEdge =
...
bool SVFGOPT::canBeRemoved(const SVFGNode *node) {
  if (SVFUtil::isa<ActualParmSVFGNode>(node) ||
    ...
    } else if (const FormalOUTSVFGNode *fo =
...
bool SVFGOPT::checkSelfCycleEdges(const MSSAPHISVFGNode *node) {
  ...
  for (; inEdgeIt != inEdgeEit; ++inEdgeIt) {
    ...
    if (preEdge->getSrcID() == preEdge->getDstID()) {
      if (keepAllSelfCycle) {
        hasSelfCycle = true;
        break; /// There's no need to check other edge if we do not remove self
               /// cycle
      } else if (keepContextSelfCycle &&
                 (SVFUtil::isa<CallIndSVFGEdge>(preEdge) ||
                  SVFUtil::isa<RetIndSVFGEdge>(preEdge))) {
        hasSelfCycle = true;
        continue; /// Continue checking and remove other self cycle which are
                  /// NOT context-related
      } else {
  ...
  for (; inEdgeIt != inEdgeEit; ++inEdgeIt) {
    ...
    for (; outEdgeIt != outEdgeEit; ++outEdgeIt) {
      ...
      if (srcNode->getId() != dstNode->getId() &&
        ...
        /// if no new edge is added, the number of dst node's incoming edges may
        /// be decreased. try to analyze it again.
        addIntoWorklist(dstNode);
      }
    }

    if (added == false) {
      /// if no new edge is added, the number of src node's outgoing edges may
      /// be decreased. try to analyze it again.
...
bool SVFGOPT::addNewSVFGEdge(NodeID srcId, NodeID dstId,
  ...
  const IndirectSVFGEdge *succIndEdge =
      SVFUtil::cast<IndirectSVFGEdge>(succEdge);
  ...
  if (const CallIndSVFGEdge *preCallEdge =
          SVFUtil::dyn_cast<CallIndSVFGEdge>(preEdge)) {
    return addCallIndirectSVFGEdge(srcId, dstId, preCallEdge->getCallSiteId(),
                                   intersection);
  } else if (const CallIndSVFGEdge *succCallEdge =
                 SVFUtil::dyn_cast<CallIndSVFGEdge>(succEdge)) {
    return addCallIndirectSVFGEdge(srcId, dstId, succCallEdge->getCallSiteId(),
                                   intersection);
  } else if (const RetIndSVFGEdge *preRetEdge =
                 SVFUtil::dyn_cast<RetIndSVFGEdge>(preEdge)) {
    return addRetIndirectSVFGEdge(srcId, dstId, preRetEdge->getCallSiteId(),
                                  intersection);
  } else if (const RetIndSVFGEdge *succRetEdge =
                 SVFUtil::dyn_cast<RetIndSVFGEdge>(succEdge)) {
    return addRetIndirectSVFGEdge(srcId, dstId, succRetEdge->getCallSiteId(),
                                  intersection);
  } else {
