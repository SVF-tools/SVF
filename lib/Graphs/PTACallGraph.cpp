...
bool PTACallGraphNode::isReachableFromProgEntry() const {
  ...
  while (nodeStack.empty() == false) {
    ...
    for (const_iterator it = node->InEdgeBegin(), eit = node->InEdgeEnd();
         it != eit; ++it) {
...
void PTACallGraph::addDirectCallGraphEdge(const CallICFGNode *cs,
  ...
  if (!hasGraphEdge(caller, callee, PTACallGraphEdge::CallRetEdge, csId)) {
    PTACallGraphEdge *edge = new PTACallGraphEdge(
        caller, callee, PTACallGraphEdge::CallRetEdge, csId);
...
bool PTACallGraph::isReachableBetweenFunctions(const SVFFunction *srcFn,
  ...
  while (nodeStack.empty() == false) {
    ...
    for (CallGraphEdgeConstIter it = node->InEdgeBegin(),
                                eit = node->InEdgeEnd();
         it != eit; ++it) {
      PTACallGraphEdge *edge = *it;
      if (visitedNodes.test_and_set(edge->getSrcID()))
...
void PTACallGraph::view() { llvm::ViewGraph(this, "Call Graph"); }
