...
static bool checkArgTypes(CallSite cs, const Function *fn) {
  ...
  u32_t arg_size =
      (fn->arg_size() > cs.arg_size()) ? cs.arg_size() : fn->arg_size();
  if (arg_size > 1) {
...
void CHNode::getVirtualFunctions(u32_t idx,
  ...
  for (vector<FuncVector>::const_iterator it = virtualFunctionVectors.begin(),
                                          eit = virtualFunctionVectors.end();
       it != eit; ++it) {
    if ((*it).size() > idx)
...
void CHGraph::printCH() {
  for (CHGraph::const_iterator it = this->begin(), eit = this->end(); it != eit;
       ++it) {
...
template <> struct DOTGraphTraits<CHGraph *> : public DefaultDOTGraphTraits {
  ...
  static std::string getGraphName(CHGraph *) { return "Class Hierarchy Graph"; }
