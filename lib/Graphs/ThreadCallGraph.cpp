...
void ThreadCallGraph::updateCallGraph(PointerAnalysis *pta) {
  ...
  // Fork sites
  for (CallSiteSet::const_iterator it = forksitesBegin(), eit = forksitesEnd();
       it != eit; ++it) {
    const Value *forkedval = tdAPI->getForkedFun((*it)->getCallSite());
    if (SVFUtil::dyn_cast<Function>(forkedval) == nullptr) {
      SVFIR *pag = pta->getPAG();
      const NodeBS targets =
          pta->getPts(pag->getValueNode(forkedval)).toNodeBS();
      for (NodeBS::iterator ii = targets.begin(), ie = targets.end(); ii != ie;
           ii++) {
        if (ObjVar *objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii))) {
          ...
          if (obj->isFunction()) {
            ...
            const SVFFunction *svfCallee =
                LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(callee);
  ...
  // parallel_for sites
  for (CallSiteSet::const_iterator it = parForSitesBegin(),
                                   eit = parForSitesEnd();
       it != eit; ++it) {
...
void ThreadCallGraph::updateJoinEdge(PointerAnalysis *pta) {
  ...
  for (CallSiteSet::const_iterator it = joinsitesBegin(), eit = joinsitesEnd();
       it != eit; ++it) {
    const Value *jointhread = tdAPI->getJoinedThread((*it)->getCallSite());
    // find its corresponding fork sites first
    CallSiteSet forkset;
    for (CallSiteSet::const_iterator it = forksitesBegin(),
                                     eit = forksitesEnd();
         it != eit; ++it) {
 ...
 */
void ThreadCallGraph::addIndirectForkEdge(const CallICFGNode *cs,
...
void ThreadCallGraph::addDirectJoinEdge(const CallICFGNode *cs,
  ...
  for (CallSiteSet::const_iterator it = forkset.begin(), eit = forkset.end();
       it != eit; ++it) {

    const Function *threadRoutineFun =
...
void ThreadCallGraph::addDirectParForEdge(const CallICFGNode *cs) {
  ...
  PTACallGraphNode *caller = getCallGraphNode(cs->getCaller());
  const Function *taskFunc = SVFUtil::dyn_cast<Function>(
 ...
 */
void ThreadCallGraph::addIndirectParForEdge(const CallICFGNode *cs,
                                            const SVFFunction *calleefun) {
