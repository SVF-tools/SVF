...
void OfflineConsG::buildOfflineCG() {
  ...
  for (ConstraintEdge::ConstraintEdgeSetTy::iterator it = LoadCGEdgeSet.begin(),
  ...
  for (StoreEdges::iterator it = stores.begin(), eit = stores.end(); it != eit;
       ++it) {
    removeStoreEdge(*it);
  }
...
NodeID OfflineConsG::solveRep(OSCC *oscc, NodeID rep) {
  if (isaRef(rep)) {
    ...
    for (NodeBS::iterator subIt = subNodes.begin(), subEit = subNodes.end();
         subIt != subEit; ++subIt) {
      if (isaRef(*subIt)) {
        rep = *subIt;
        break;
