...
const NodeBS MRSVFGNode::getDefSVFVars() const { return getPointsTo(); }
...
const std::string FormalINSVFGNode::toString() const {
  ...
  rawstr << "FormalINSVFGNode ID: " << getId()
         << " {fun: " << getFun()->getName() << "}";
  rawstr << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << " = ENCHI(MR_"
         << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << ")\n";
...
const std::string FormalOUTSVFGNode::toString() const {
  ...
  rawstr << "FormalOUTSVFGNode ID: " << getId()
         << " {fun: " << getFun()->getName() << "}";
  rawstr << "RETMU(" << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << ")\n";
  rawstr << getMRVer()->getMR()->dumpStr() << "\n";
  return rawstr.str();
}
...
const std::string ActualINSVFGNode::toString() const {
  ...
  rawstr << "ActualINSVFGNode ID: " << getId()
         ...
         << " {fun: " << getFun()->getName() << "}";
  rawstr << "CSMU(" << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << ")\n";
  rawstr << getMRVer()->getMR()->dumpStr() << "\n";
...
const std::string ActualOUTSVFGNode::toString() const {
  ...
  rawstr << "ActualOUTSVFGNode ID: " << getId()
         ...
         << " {fun: " << getFun()->getName() << "}";
  rawstr << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << " = CSCHI(MR_"
         << getMRVer()->getMR()->getMRID() << "V_"
         << getMRVer()->getSSAVersion() << ")\n";
  rawstr << getMRVer()->getMR()->dumpStr() << "\n";
  rawstr << "CS[" << getSourceLoc(getCallSite()->getCallSite()) << "]";
...
const std::string MSSAPHISVFGNode::toString() const {
  ...
  rawstr << "MSSAPHISVFGNode ID: " << getId()
         << " {fun: " << getFun()->getName() << "}";
  rawstr << "MR_" << getResVer()->getMR()->getMRID() << "V_"
         << getResVer()->getSSAVersion() << " = PHI(";
...
FormalOUTSVFGNode::FormalOUTSVFGNode(NodeID id, const MRVer *mrVer,
                                     const FunExitICFGNode *funExit)
    : MRSVFGNode(id, FPOUT) {
...
void SVFG::addSVFGNodesForAddrTakenVars() {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = stores.begin(),
                                       eiter = stores.end();
       iter != eiter; ++iter) {
    StoreStmt *store = SVFUtil::cast<StoreStmt>(*iter);
    const StmtSVFGNode *sNode = getStmtVFGNode(store);
    for (CHISet::iterator pi = mssa->getCHISet(store).begin(),
                          epi = mssa->getCHISet(store).end();
         pi != epi; ++pi)
  ...
  for (MemSSA::BBToPhiSetMap::iterator it = mssa->getBBToPhiSetMap().begin(),
                                       eit = mssa->getBBToPhiSetMap().end();
       it != eit; ++it) {
    for (PHISet::iterator pi = it->second.begin(), epi = it->second.end();
         pi != epi; ++pi) {
      MemSSA::PHI *phi = *pi;
      addIntraMSSAPHISVFGNode(
          pag->getICFG()->getICFGNode(&(phi->getBasicBlock()->front())),
          phi->opVerBegin(), phi->opVerEnd(), phi->getResVer(), totalVFGNode++);
    }
  }
  /// initialize memory SSA entry chi nodes
  for (MemSSA::FunToEntryChiSetMap::iterator
           it = mssa->getFunToEntryChiSetMap().begin(),
           eit = mssa->getFunToEntryChiSetMap().end();
       it != eit; ++it) {
    for (CHISet::iterator pi = it->second.begin(), epi = it->second.end();
         pi != epi; ++pi) {
      const MemSSA::ENTRYCHI *chi = SVFUtil::cast<ENTRYCHI>(*pi);
      addFormalINSVFGNode(
          pag->getICFG()->getFunEntryICFGNode(chi->getFunction()),
          chi->getResVer(), totalVFGNode++);
    }
  }
  /// initialize memory SSA return mu nodes
  for (MemSSA::FunToReturnMuSetMap::iterator
           it = mssa->getFunToRetMuSetMap().begin(),
           eit = mssa->getFunToRetMuSetMap().end();
       it != eit; ++it) {
    for (MUSet::iterator pi = it->second.begin(), epi = it->second.end();
         pi != epi; ++pi) {
      const MemSSA::RETMU *mu = SVFUtil::cast<RETMU>(*pi);
      addFormalOUTSVFGNode(
          pag->getICFG()->getFunExitICFGNode(mu->getFunction()), mu->getMRVer(),
          totalVFGNode++);
    }
  }
  /// initialize memory SSA callsite mu nodes
  for (MemSSA::CallSiteToMUSetMap::iterator
           it = mssa->getCallSiteToMuSetMap().begin(),
  ...
  for (MemSSA::CallSiteToCHISetMap::iterator
           it = mssa->getCallSiteToChiSetMap().begin(),
           eit = mssa->getCallSiteToChiSetMap().end();
       it != eit; ++it) {
    for (CHISet::iterator pi = it->second.begin(), epi = it->second.end();
         pi != epi; ++pi) {
      const MemSSA::CALLCHI *chi = SVFUtil::cast<CALLCHI>(*pi);
      addActualOUTSVFGNode(chi->getCallSite(), chi->getResVer(),
                           totalVFGNode++);
    }
  }
}
...
void SVFG::connectIndirectSVFGEdges() {
  ...
  for (iterator it = begin(), eit = end(); it != eit; ++it) {
    ...
    if (const LoadSVFGNode *loadNode = SVFUtil::dyn_cast<LoadSVFGNode>(node)) {
      ...
      for (PTACallGraphEdge::CallInstSet::iterator it = callInstSet.begin(),
                                                   eit = callInstSet.end();
           it != eit; ++it) {
        ...
        for (ActualINSVFGNodeSet::iterator ait = actualIns.begin(),
                                           aeit = actualIns.end();
             ait != aeit; ++ait) {
          const ActualINSVFGNode *actualIn =
              SVFUtil::cast<ActualINSVFGNode>(getSVFGNode(*ait));
          addInterIndirectVFCallEdge(actualIn, formalIn,
      ...
      /// There's no need to connect actual out node to its definition site in
      /// the same function.
    } else if (const MSSAPHISVFGNode *phiNode =
                   SVFUtil::dyn_cast<MSSAPHISVFGNode>(node)) {
      for (MemSSA::PHI::OPVers::const_iterator it = phiNode->opVerBegin(),
...
void SVFG::connectFromGlobalToProgEntry() {
  ...
  for (GlobalVFGNodeSet::const_iterator storeIt = globalVFGNodes.begin(),
                                        storeEit = globalVFGNodes.end();
       storeIt != storeEit; ++storeIt) {
    if (const StoreSVFGNode *store =
            SVFUtil::dyn_cast<StoreSVFGNode>(*storeIt)) {
      /// connect this store to main function entry
      const NodeBS &storePts =
          mssa->getPTA()->getPts(store->getPAGDstNodeID()).toNodeBS();

      for (FormalINSVFGNodeSet::iterator fiIt = formalIns.begin(),
                                         fiEit = formalIns.end();
           fiIt != fiEit; ++fiIt) {
        NodeID formalInID = *fiIt;
        NodeBS formalInPts =
            ((FormalINSVFGNode *)getSVFGNode(formalInID))->getPointsTo();
...
SVFGEdge *SVFG::addIntraIndirectVFEdge(NodeID srcId, NodeID dstId,
  ...
  if (SVFGEdge *edge =
    ...
    return (SVFUtil::cast<IndirectSVFGEdge>(edge)->addPointsTo(cpts) ? edge
                                                                     : nullptr);
  } else {
...
SVFGEdge *SVFG::addThreadMHPIndirectVFEdge(NodeID srcId, NodeID dstId,
  ...
  if (SVFGEdge *edge =
    ...
    ThreadMHPIndSVFGEdge *indirectEdge =
        new ThreadMHPIndSVFGEdge(srcNode, dstNode);
    indirectEdge->addPointsTo(cpts);
    return (addSVFGEdge(indirectEdge) ? indirectEdge : nullptr);
  }
}
...
SVFGEdge *SVFG::addCallIndirectVFEdge(NodeID srcId, NodeID dstId,
                                      const NodeBS &cpts, CallSiteID csId) {
  SVFGNode *srcNode = getSVFGNode(srcId);
  SVFGNode *dstNode = getSVFGNode(dstId);
  if (SVFGEdge *edge =
    ...
    assert(SVFUtil::isa<CallIndSVFGEdge>(edge) &&
           "this should be a indirect value flow edge!");
    return (SVFUtil::cast<CallIndSVFGEdge>(edge)->addPointsTo(cpts) ? edge
                                                                    : nullptr);
  } else {
...
SVFGEdge *SVFG::addRetIndirectVFEdge(NodeID srcId, NodeID dstId,
  ...
  if (SVFGEdge *edge =
    ...
    assert(SVFUtil::isa<RetIndSVFGEdge>(edge) &&
           "this should be a indirect value flow edge!");
    return (SVFUtil::cast<RetIndSVFGEdge>(edge)->addPointsTo(cpts) ? edge
                                                                   : nullptr);
  } else {
...
std::set<const SVFGNode *> SVFG::fromValue(const llvm::Value *value) const {
  ...
  for (const PAGEdge *pagEdge : pag->getValueEdges(value)) {
    PAGEdgeToStmtVFGNodeMapTy::const_iterator it =
        PAGEdgeToStmtVFGNodeMap.find(pagEdge);
 ...
 * return edges.
 */
void SVFG::getInterVFEdgesForIndirectCallSite(const CallICFGNode *callBlockNode,
                                              const SVFFunction *callee,
                                              SVFGEdgeSetTy &edges) {
  CallSiteID csId = getCallSiteID(callBlockNode, callee);
  RetICFGNode *retBlockNode =
      pag->getICFG()->getRetICFGNode(callBlockNode->getCallSite());
  ...
  if (pag->hasCallSiteArgsMap(callBlockNode) && pag->hasFunArgsList(callee)) {
    ...
    for (; funArgIt != funArgEit && csArgIt != csArgEit;
      ...
      if (fun_arg->isPointer() && cs_arg->isPointer())
        getInterVFEdgeAtIndCSFromAPToFP(cs_arg, fun_arg, callBlockNode, csId,
                                        edges);
    }
  ...
  if (hasFuncEntryChi(callee) && hasCallSiteMu(callBlockNode)) {
    ...
    for (SVFG::ActualINSVFGNodeSet::iterator ai_it = actualInNodes.begin(),
                                             ai_eit = actualInNodes.end();
         ai_it != ai_eit; ++ai_it) {
      ActualINSVFGNode *actualIn =
  ...
  if (hasFuncRetMu(callee) && hasCallSiteChi(callBlockNode)) {
    ...
    for (SVFG::ActualOUTSVFGNodeSet::iterator ao_it = actualOutNodes.begin(),
                                              ao_eit = actualOutNodes.end();
         ao_it != ao_eit; ++ao_it) {
      ActualOUTSVFGNode *actualOut =
...
void SVFG::connectCallerAndCallee(const CallICFGNode *cs,
  ...
  if (hasFuncRetMu(callee) && hasCallSiteChi(cs)) {
    ...
    for (SVFG::FormalOUTSVFGNodeSet::iterator fo_it = formalOutNodes.begin(),
                                              fo_eit = formalOutNodes.end();
         fo_it != fo_eit; ++fo_it) {
      const FormalOUTSVFGNode *formalOut =
...
const SVFFunction *SVFG::isFunEntrySVFGNode(const SVFGNode *node) const {
  if (const FormalParmSVFGNode *fp =
          SVFUtil::dyn_cast<FormalParmSVFGNode>(node)) {
    return fp->getFun();
  } else if (const InterPHISVFGNode *phi =
                 SVFUtil::dyn_cast<InterPHISVFGNode>(node)) {
    if (phi->isFormalParmPHI())
      return phi->getFun();
  } else if (const FormalINSVFGNode *fi =
                 SVFUtil::dyn_cast<FormalINSVFGNode>(node)) {
    return fi->getFun();
  } else if (const InterMSSAPHISVFGNode *mphi =
...
const CallICFGNode *SVFG::isCallSiteRetSVFGNode(const SVFGNode *node) const {
  ...
  } else if (const ActualOUTSVFGNode *ao =
 ...
 */
namespace llvm {
template <> struct DOTGraphTraits<SVFG *> : public DOTGraphTraits<SVFIR *> {
...
std::string
getNodeLabel(NodeType *node, SVFG *graph) {
  if (isSimple())
