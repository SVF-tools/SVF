...
const NodeBS BranchVFGNode::getDefSVFVars() const { return NodeBS(); }
...
void VFG::addVFGNodes() {
  ...
  // construction to be noted for black hole pointer it has already has address
  // edge connected, and its definition will be set when processing addr SVFIR
  // edge.
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = addrs.begin(),
                                       eiter = addrs.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = copys.begin(),
                                       eiter = copys.end();
       iter != eiter; ++iter) {
    const CopyStmt *edge = SVFUtil::cast<CopyStmt>(*iter);
    assert(!isPhiCopyEdge(edge) &&
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = ngeps.begin(),
                                       eiter = ngeps.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = loads.begin(),
                                       eiter = loads.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = forks.begin(),
                                       eiter = forks.end();
       iter != eiter; ++iter) {
  ...
  for (SVFIR::CSToArgsListMap::iterator it = pag->getCallSiteArgsMap().begin(),
    ...
    for (SVFIR::SVFVarList::iterator pit = it->second.begin(),
                                     epit = it->second.end();
         pit != epit; ++pit) {
  ...
  for (SVFIR::CSToRetMap::iterator it = pag->getCallSiteRets().begin(),
    ...
    /// they are in the formal of AddrVFGNode if the external function returns
    /// an allocated memory if fun has body, it may also exist in isExtCall,
    /// e.g., xmalloc() in bzip2, spec2000.
  ...
  for (SVFIR::FunToArgsListMap::iterator it = pag->getFunArgsMap().begin(),
                                         eit = pag->getFunArgsMap().end();
       it != eit; ++it) {
    ...
    for (SVFIR::SVFVarList::iterator pit = it->second.begin(),
      ...
      if (isInterestedPAGNode(param) == false ||
      ...
      if (param->hasIncomingEdges(SVFStmt::Call)) {
        for (SVFStmt::SVFStmtSetTy::const_iterator
                 cit = param->getIncomingEdgesBegin(SVFStmt::Call),
                 ecit = param->getIncomingEdgesEnd(SVFStmt::Call);
             cit != ecit; ++cit) {
    ...
    if (func->getLLVMFun()->getFunctionType()->isVarArg()) {
      ...
      if (varParam->hasIncomingEdges(SVFStmt::Call)) {
        for (SVFStmt::SVFStmtSetTy::const_iterator
                 cit = varParam->getIncomingEdgesBegin(SVFStmt::Call),
                 ecit = varParam->getIncomingEdgesEnd(SVFStmt::Call);
             cit != ecit; ++cit) {
          CallPE *callPE = SVFUtil::cast<CallPE>(*cit);
          if (isInterestedPAGNode(callPE->getRHSVar()))
            callPEs.insert(callPE);
  ...
  for (SVFIR::FunToRetMap::iterator it = pag->getFunRets().begin(),
                                    eit = pag->getFunRets().end();
       it != eit; ++it) {
    const SVFFunction *func = it->first;
    ...
    if (uniqueFunRetNode->hasOutgoingEdges(SVFStmt::Ret)) {
      for (SVFStmt::SVFStmtSetTy::const_iterator
               cit = uniqueFunRetNode->getOutgoingEdgesBegin(SVFStmt::Ret),
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = phis.begin(), eiter = phis.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = selects.begin(),
                                       eiter = selects.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = binaryops.begin(),
                                       eiter = binaryops.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = unaryops.begin(),
                                       eiter = unaryops.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = brs.begin(), eiter = brs.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = cmps.begin(), eiter = cmps.end();
       iter != eiter; ++iter) {
...
void VFG::connectDirectVFGEdges() {
  ...
  for (iterator it = begin(), eit = end(); it != eit; ++it) {
    ...
    if (StmtVFGNode *stmtNode = SVFUtil::dyn_cast<StmtVFGNode>(node)) {
      /// do not handle AddrSVFG node, as it is already the source of a
      /// definition
      if (SVFUtil::isa<AddrVFGNode>(stmtNode))
        continue;
      /// for all other cases, like copy/gep/load/ret, connect the RHS pointer
      /// to its def
      ...
      for (PHIVFGNode::OPVers::const_iterator it = phiNode->opVerBegin(),
                                              eit = phiNode->opVerEnd();
           it != eit; it++) {
        if (it->second->isConstantData() == false)
          addIntraDirectVFEdge(getDef(it->second), nodeId);
      }
    } else if (BinaryOPVFGNode *binaryNode =
                   SVFUtil::dyn_cast<BinaryOPVFGNode>(node)) {
      for (BinaryOPVFGNode::OPVers::const_iterator
               it = binaryNode->opVerBegin(),
               eit = binaryNode->opVerEnd();
           it != eit; it++) {
        if (it->second->isConstantData() == false)
          addIntraDirectVFEdge(getDef(it->second), nodeId);
      }
    } else if (UnaryOPVFGNode *unaryNode =
                   SVFUtil::dyn_cast<UnaryOPVFGNode>(node)) {
      for (UnaryOPVFGNode::OPVers::const_iterator it = unaryNode->opVerBegin(),
      ...
      for (CmpVFGNode::OPVers::const_iterator it = cmpNode->opVerBegin(),
                                              eit = cmpNode->opVerEnd();
           it != eit; it++) {
        if (it->second->isConstantData() == false)
          addIntraDirectVFEdge(getDef(it->second), nodeId);
      }
    } else if (ActualParmVFGNode *actualParm =
      ...
      for (RetPESet::const_iterator it = calleeRet->retPEBegin(),
        ...
        CallICFGNode *callBlockNode =
            pag->getICFG()->getCallICFGNode(retBlockNode->getCallSite());
        addInterEdgeFromFRToAR(
            calleeRet, callsiteRev,
            getCallSiteID(callBlockNode, calleeRet->getFun()));
      }
    }
    /// Do not process FormalRetVFGNode, as they are connected by copy within
    /// callee We assume one procedure only has unique return
  }
  ...
  if (Options::EnableThreadCallGraph) {
    ...
    for (SVFStmt::SVFStmtSetTy::iterator iter = forks.begin(),
      ...
      TDForkPE *forkedge = SVFUtil::cast<TDForkPE>(*iter);
      ActualParmVFGNode *acutalParm =
          getActualParmVFGNode(forkedge->getRHSVar(), forkedge->getCallSite());
      FormalParmVFGNode *formalParm =
          getFormalParmVFGNode(forkedge->getLHSVar());
    ...
    for (SVFStmt::SVFStmtSetTy::iterator iter = joins.begin(),
                                         eiter = joins.end();
         iter != eiter; ++iter) {
...
VFGEdge *VFG::getIntraVFGEdge(const VFGNode *src, const VFGNode *dst,
                              VFGEdge::VFGEdgeK kind) {
  return hasIntraVFGEdge(const_cast<VFGNode *>(src), const_cast<VFGNode *>(dst),
                         kind);
}
 ...
 */
void VFG::view() { llvm::ViewGraph(this, "Value Flow Graph"); }

void VFG::updateCallGraph(PointerAnalysis *pta) {
  VFGEdgeSetTy vfEdgesAtIndCallSite;
  PointerAnalysis::CallEdgeMap::const_iterator iter =
      pta->getIndCallMap().begin();
  PointerAnalysis::CallEdgeMap::const_iterator eiter =
      pta->getIndCallMap().end();
  for (; iter != eiter; iter++) {
...
void VFG::connectCallerAndCallee(const CallICFGNode *callBlockNode,
  ...
  RetICFGNode *retBlockNode =
      icfg->getRetICFGNode(callBlockNode->getCallSite());
    ...
    if (callee->getLLVMFun()->isVarArg()) {
      ...
      if (varFunArgNode->isPointer()) {
        for (; csArgIt != csArgEit; csArgIt++) {
          const PAGNode *cs_arg = *csArgIt;
          if (cs_arg->isPointer())
            connectAParamAndFParam(cs_arg, varFunArgNode, callBlockNode, csId,
                                   edges);
        }
      }
    }
...
const Value *StmtVFGNode::getValue() const { return getPAGEdge()->getValue(); }

const Value *CmpVFGNode::getValue() const { return getRes()->getValue(); }

const Value *BinaryOPVFGNode::getValue() const { return getRes()->getValue(); }

const Value *PHIVFGNode::getValue() const { return getRes()->getValue(); }

const Value *ArgumentVFGNode::getValue() const { return param->getValue(); }

/*!
 * GraphTraits specialization
 */
namespace llvm {
template <> struct DOTGraphTraits<VFG *> : public DOTGraphTraits<SVFIR *> {
  ...
  static std::string getSimpleNodeLabel(NodeType *node, VFG *) {
    ...
    } else if (FormalParmVFGNode *fp =
                   SVFUtil::dyn_cast<FormalParmVFGNode>(node)) {
      rawstr << fp->toString();
    } else if (ActualParmVFGNode *ap =
                   SVFUtil::dyn_cast<ActualParmVFGNode>(node)) {
      rawstr << ap->toString();
    } else if (ActualRetVFGNode *ar =
                   SVFUtil::dyn_cast<ActualRetVFGNode>(node)) {
      rawstr << ar->toString();
    } else if (FormalRetVFGNode *fr =
                   SVFUtil::dyn_cast<FormalRetVFGNode>(node)) {
      rawstr << fr->toString();
    } else if (SVFUtil::isa<NullPtrVFGNode>(node)) {
      rawstr << "NullPtr";
    } else if (BinaryOPVFGNode *bop =
                   SVFUtil::dyn_cast<BinaryOPVFGNode>(node)) {
      rawstr << bop->toString();
