    ...
    if (visited.find(inst) == visited.end()) {
      ...
      for (InstVec::const_iterator nit = nextInsts.begin(),
        ...
        if (const BranchInst *br = SVFUtil::dyn_cast<BranchInst>(inst)) {
          ...
          if (br->isConditional())
            icfg->addConditionalIntraEdge(srcNode, dstNode, br->getCondition(),
                                          1 - branchID);
          else
          ...
          const ConstantInt *condVal =
              const_cast<SwitchInst *>(si)->findCaseDest(
                  const_cast<BasicBlock *>(succ->getParent()));
          /// default case is set to -1;
          s32_t val = condVal ? condVal->getSExtValue() : -1;
          icfg->addConditionalIntraEdge(srcNode, dstNode, si->getCondition(),
                                        val);
        } else
          icfg->addIntraEdge(srcNode, dstNode);
...
/*
 * Add the global initialization statements immediately after the function entry
 * of main
 */
