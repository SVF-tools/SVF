...
SVFIR *SVFIRBuilder::build(SVFModule *svfModule) {
  ...
  // If the SVFIR has been built before, then we return the unique SVFIR of the
  // program
  ...
  for (SVFModule::iterator fit = svfModule->begin(), efit = svfModule->end();
    ...
    if (!fun.isDeclaration()) {
      ...
      if (fun.getLLVMFun()->doesNotReturn() == false &&
          fun.getLLVMFun()->getReturnType()->isVoidTy() == false)
        pag->addFunRet(&fun, pag->getGNode(pag->getReturnNode(&fun)));

      /// To be noted, we do not record arguments which are in declared function
      /// without body
      /// TODO: what about external functions with SVFIR imported by
      /// commandline?
      for (Function::arg_iterator I = fun.getLLVMFun()->arg_begin(),
        ...
        // it is (1) too conservative, and (2) make FormalParmVFGNode defined at
        // blackhole address PAGEdge. if(SVFUtil::ArgInNoCallerFunction(&*I)) {
    ...
    for (Function::iterator bit = fun.getLLVMFun()->begin(),
                            ebit = fun.getLLVMFun()->end();
         bit != ebit; ++bit) {
      BasicBlock &bb = *bit;
      for (BasicBlock::iterator it = bb.begin(), eit = bb.end(); it != eit;
           ++it) {
        Instruction &inst = *it;
...
/*
    ...
    Handling single value types, for constant index, including pointer, integer,
   etc e.g. field_idx = getelementptr i8, %i8* %p, i64 -4 We can obtain the
   field index by inferring the byteoffset if %p is casted from a pointer to a
   struct For another example, the following can be an array access. e.g.
   field_idx = getelementptr i8, %struct_type %p, i64 1

*/
u32_t SVFIRBuilder::inferFieldIdxFromByteOffset(const llvm::GEPOperator *gepOp,
                                                DataLayout *dl, LocationSet &ls,
                                                s32_t idx) {
  return 0;
}
...
bool SVFIRBuilder::computeGepOffset(const User *V, LocationSet &ls) {
  ...
  const llvm::GEPOperator *gepOp =
      SVFUtil::dyn_cast<const llvm::GEPOperator>(V);
  DataLayout *dataLayout =
      getDataLayout(LLVMModuleSet::getLLVMModuleSet()->getMainLLVMModule());
  llvm::APInt byteOffset(
      dataLayout->getIndexSizeInBits(gepOp->getPointerAddressSpace()), 0, true);
  if (gepOp && dataLayout &&
  ...
  for (bridge_gep_iterator gi = bridge_gep_begin(*V), ge = bridge_gep_end(*V);
    ...
    // but we can distinguish different field of an array of struct, e.g.
    // s[1].f1 is differet from s[0].f2
    if (SVFUtil::isa<ArrayType>(gepTy)) {
      if (!op)
        continue;
      s32_t idx = op->getSExtValue();
      u32_t offset =
          SymbolTableInfo::SymbolInfo()->getFlattenedElemIdx(gepTy, idx);
      ...
      s32_t idx = op->getSExtValue();
      u32_t offset =
          SymbolTableInfo::SymbolInfo()->getFlattenedElemIdx(ST, idx);
      ls.setFldIdx(ls.accumulateConstantFieldIdx() + offset);
    } else if (gepTy->isSingleValueType()) {
      // If it's a non-constant offset access
      // If its point-to target is struct or array, it's likely an array
      // accessing (%result = gep %struct.A* %a, i32 %non-const-index) If its
      // point-to target is single value (pointer arithmetic), then it's a
      // variant gep (%result = gep i8* %p, i32 %non-const-index)
      if (!op && gepTy->isPointerTy() &&
          gepTy->getPointerElementType()->isSingleValueType())
        return false;
      ...
      // inferFieldIdxFromByteOffset(geopOp,idx));
    }
  }
  return true;
}
...
void SVFIRBuilder::processCE(const Value *val) {
  if (const Constant *ref = SVFUtil::dyn_cast<Constant>(val)) {
    if (const ConstantExpr *gepce = isGepConstantExpr(ref)) {
      ...
      // must invoke pag methods here, otherwise it will be a dead recursion
      // cycle
       ...
       * multiple callsites) so bb/inst of this edge may be rewritten several
       * times, we treat it as global here.
       */
    ...
    // if we meet a int2ptr, then it points-to black hole
      ...
      // SymbolTableInfo.cpp:674 and SVFIRBuilder.cpp:183-194
      const Value *cval = getCurrentValue();
      const BasicBlock *cbb = getCurrentBB();
      setCurrentLocation(ref, nullptr);
      NodeID dst = pag->getValueNode(ref);
...
void SVFIRBuilder::InitialGlobal(const GlobalVariable *gvar, Constant *C,
                                 u32_t offset) {
  DBOUT(DPAGBuild, outs() << "global " << SVFUtil::value2String(gvar)
                          << " constant initializer: "
                          << SVFUtil::value2String(C) << "\n");
  if (C->getType()->isSingleValueType()) {
    NodeID src = getValueNode(C);
    // get the field value if it is avaiable, otherwise we create a dummy field
    // node.
    ...
    if (SVFUtil::isa<GlobalVariable>(C) || SVFUtil::isa<Function>(C)) {
      ...
      if (C->getType()->isPtrOrPtrVectorTy() && src != pag->getNullPtr())
        addCopyEdge(pag->getNullPtr(), src);
             ...
             SVFUtil::isa<ConstantStruct>(C)) {
    for (u32_t i = 0, e = C->getNumOperands(); i != e; i++) {
      u32_t off =
          SymbolTableInfo::SymbolInfo()->getFlattenedElemIdx(C->getType(), i);
    ...
    if (Options::ModelConsts) {
      if (ConstantDataSequential *seq =
        ...
        for (u32_t i = 0; i < seq->getNumElements(); i++) {
          u32_t off = SymbolTableInfo::SymbolInfo()->getFlattenedElemIdx(
              C->getType(), i);
          Constant *ct = seq->getElementAsConstant(i);
          InitialGlobal(gvar, ct, offset + off);
        }
      } else {
        assert(SVFUtil::isa<ConstantAggregateZero>(data) &&
  ...
  for (SVFModule::global_iterator I = svfModule->global_begin(),
    ...
    if (gvar->hasInitializer()) {
      ...
      DBOUT(DPAGBuild, outs() << "add global var node "
                              << SVFUtil::value2String(gvar) << "\n");
      InitialGlobal(gvar, C, 0);
    }
  }

  /// initialize global functions
  for (SVFModule::llvm_const_iterator I = svfModule->llvmFunBegin(),
                                      E = svfModule->llvmFunEnd();
       I != E; ++I) {
    const Function *fun = *I;
    NodeID idx = getValueNode(fun);
    NodeID obj = getObjectNode(fun);

    DBOUT(DPAGBuild, outs() << "add global function node "
                            << fun->getName().str() << "\n");
    setCurrentLocation(fun, nullptr);
    addAddrEdge(obj, idx);
  }

  // Handle global aliases (due to linkage of multiple bc files), e.g., @x =
  // internal alias @y. We need to add a copy from y to x.
  for (SVFModule::alias_iterator I = svfModule->alias_begin(),
                                 E = svfModule->alias_end();
       I != E; I++) {
 ...
 * Add edge V (dst) <-- O (src), V here is a value node on SVFIR, O is object
 * node on SVFIR
 */
void SVFIRBuilder::visitAllocaInst(AllocaInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process alloca  " << SVFUtil::value2String(&inst) << " \n");
...
void SVFIRBuilder::visitPHINode(PHINode &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process phi " << SVFUtil::value2String(&inst) << "  \n");
...
void SVFIRBuilder::visitLoadInst(LoadInst &inst) {
  DBOUT(DPAGBuild,
        outs() << "process load  " << SVFUtil::value2String(&inst) << " \n");
...
void SVFIRBuilder::visitStoreInst(StoreInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process store " << SVFUtil::value2String(&inst) << " \n");
...
void SVFIRBuilder::visitGetElementPtrInst(GetElementPtrInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process gep  " << SVFUtil::value2String(&inst) << " \n");
...
void SVFIRBuilder::visitCastInst(CastInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process cast  " << SVFUtil::value2String(&inst) << " \n");
  NodeID dst = getValueNode(&inst);
...
void SVFIRBuilder::visitSelectInst(SelectInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process select  " << SVFUtil::value2String(&inst) << " \n");
...
void SVFIRBuilder::visitCallSite(CallSite cs) {
  ...
  DBOUT(DPAGBuild, outs() << "process callsite "
                          << SVFUtil::value2String(cs.getInstruction())
                          << "\n");
  ...
  for (CallSite::arg_iterator itA = cs.arg_begin(), ieA = cs.arg_end();
       itA != ieA; ++itA)
    pag->addCallSiteArgs(callBlockNode, pag->getGNode(getValueNode(*itA)));

  if (!cs.getType()->isVoidTy())
    pag->addCallSiteRets(retBlockNode,
  ...
  if (callee) {
    ...
    // If the callee was not identified as a function (null F), this is
    // indirect.
    handleIndCall(cs);
  }
}
...
void SVFIRBuilder::visitReturnInst(ReturnInst &inst) {
  ...
  DBOUT(DPAGBuild,
        outs() << "process return  " << SVFUtil::value2String(&inst) << " \n");

  if (Value *src = inst.getReturnValue()) {
    const SVFFunction *F = LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(
...
/// TODO: for now, create a copy edge from %ap2 to %tmp, we assume here %tmp
/// should point to the n-th argument of the var_args
...
/// If <val> is undef or poison, ‘freeze’ returns an arbitrary, but fixed value
/// of type `ty` Otherwise, this instruction is a no-op and returns the input
/// <val> For now, we assume <val> is never a posion or undef.
...
void SVFIRBuilder::handleDirectCall(CallSite cs, const SVFFunction *F) {
  ...
  DBOUT(DPAGBuild, outs() << "handle direct call "
                          << SVFUtil::value2String(cs.getInstruction())
                          << " callee " << *F << "\n");
...
const Value *SVFIRBuilder::getBaseValueForExtArg(const Value *V) {
  ...
  if (const GetElementPtrInst *gep =
          SVFUtil::dyn_cast<GetElementPtrInst>(value)) {
    s32_t totalidx = 0;
    for (bridge_gep_iterator gi = bridge_gep_begin(gep),
                             ge = bridge_gep_end(gep);
         gi != ge; ++gi) {
      if (const ConstantInt *op =
              SVFUtil::dyn_cast<ConstantInt>(gi.getOperand()))
 ...
 * (V).
 */
const Type *SVFIRBuilder::getBaseTypeAndFlattenedFields(
    const Value *V, std::vector<LocationSet> &fields, const Value *szValue) {
  assert(V);
  ...
  /// use user-specified size for this copy operation if the size is a constaint
  /// int
  ...
  for (u32_t ei = 0; ei < numOfElems; ei++) {
    LocationSet ls(ei);
    // make a ConstantInt and create char for the content type due to byte-wise
    // copy
...
void SVFIRBuilder::addComplexConsForExt(Value *D, Value *S,
  ...
  for (u32_t index = 0; index < sz; index++) {
    const Type *dElementType =
        SymbolTableInfo::SymbolInfo()->getFlatternedElemType(
            dtype, fields[index].accumulateConstantFieldIdx());
    const Type *sElementType =
        SymbolTableInfo::SymbolInfo()->getFlatternedElemType(
            stype, fields[index].accumulateConstantFieldIdx());
  ...
  if (isHeapAllocOrStaticExtCall(cs)) {
    ...
    if (isExtCall(callee)) {
      ...
      case ExtAPI::EFT_REALLOC: {
        ...
        // if ptr is not null, then we assume a new data memory will be attached
        // to the tail of old allocated memory block.
      ...
      case ExtAPI::EFT_L_A0__A0R_A1: {
        ...
        // which copies the character c (an unsigned char) to the first n
        // characters of the string pointed to, by the argument str
        std::vector<LocationSet> dstFields;
        const Type *dtype = getBaseTypeAndFlattenedFields(
            cs.getArgument(0), dstFields, cs.getArgument(2));
        u32_t sz = dstFields.size();
        // For each field (i), add store edge *(arg0 + i) = arg1
        for (u32_t index = 0; index < sz; index++) {
          const Type *dElementType =
              SymbolTableInfo::SymbolInfo()->getFlatternedElemType(
                  dtype, dstFields[index].accumulateConstantFieldIdx());
          NodeID dField =
              getGepValVar(cs.getArgument(0), dstFields[index], dElementType);
      ...
      case ExtAPI::EFT_L_A1__FunPtr: {
        /// handling external function e.g., void *dlsym(void *handle, const
        /// char *funname);
        const Value *src = cs.getArgument(1);
        if (const GetElementPtrInst *gep =
                SVFUtil::dyn_cast<GetElementPtrInst>(src))
          src = stripConstantCasts(gep->getPointerOperand());
        if (const GlobalVariable *glob =
                SVFUtil::dyn_cast<GlobalVariable>(src)) {
          if (const ConstantDataArray *constarray =
                  SVFUtil::dyn_cast<ConstantDataArray>(
                      glob->getInitializer())) {
            if (const SVFFunction *fun =
                    getProgFunction(svfMod, constarray->getAsCString().str())) {
      ...
      case ExtAPI::EFT_FREE_MULTILEVEL: {
        // For any argument with 2-level pointer passing too a free function
        // e.g., XFree(void** p), we will need to add a load edge (dummy = *p)
        // and a store edge (*dummy = nullptr).
      ...
      case ExtAPI::EFT_STD_RB_TREE_INSERT_AND_REBALANCE: {
        ...
        // We have vArg3 points to the entry of _Rb_tree_node_base { color;
        // parent; left; right; }. Now we calculate the offset from base to
        // vArg3
        NodeID vnArg3 = pag->getValueNode(vArg3);
        s32_t offset = pag->getLocationSetFromBaseNode(vnArg3)
                           .accumulateConstantFieldIdx();

        // We get all flattened fields of base
        vector<LocationSet> fields;
        const Type *type =
            getBaseTypeAndFlattenedFields(vArg3, fields, nullptr);
        assert(fields.size() >= 4 &&
               "_Rb_tree_node_base should have at least 4 fields.\n");

        // We summarize the side effects: arg3->parent = arg1, arg3->left =
        // arg1, arg3->right = arg1 Note that arg0 is aligned with "offset".
        for (s32_t i = offset + 1; i <= offset + 3; ++i) {
          if ((u32_t)i >= fields.size())
            break;
          const Type *elementType =
              SymbolTableInfo::SymbolInfo()->getFlatternedElemType(
                  type, fields[i].accumulateConstantFieldIdx());
      ...
      case ExtAPI::EFT_STD_RB_TREE_INCREMENT: {
        ...
        s32_t offset =
            pag->getLocationSetFromBaseNode(vnArg).accumulateConstantFieldIdx();
        ...
        // We summarize the side effects: ret = arg->parent, ret = arg->left,
        // ret = arg->right Note that arg0 is aligned with "offset".
        for (s32_t i = offset + 1; i <= offset + 3; ++i) {
          if ((u32_t)i >= fields.size())
            break;
          const Type *elementType =
    ...
    if (isThreadForkCall(inst)) {
      if (const Function *forkedFun = getLLVMFunction(getForkedFun(inst))) {
        ...
        if (forkedFun->arg_size() <= 2 && forkedFun->arg_size() >= 1) {
          ...
          if (SVFUtil::isa<PointerType>(actualParm->getType()) &&
            ...
            FunEntryICFGNode *entry = pag->getICFG()->getFunEntryICFGNode(
                LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(forkedFun));
            addThreadForkEdge(pag->getValueNode(actualParm),
                              pag->getValueNode(formalParm), icfgNode, entry);
          }
        }
      } else {
      ...
      /// If forkedFun does not pass to spawnee as function type but as void
      /// pointer remember to update inter-procedural callgraph/SVFIR/SVFG etc.
      /// when indirect call targets are resolved We don't connect the callgraph
      /// here, further investigation is need to hanle mod-ref during SVFG
      /// construction.
    }
    ...
    /// create inter-procedural SVFIR edges for hare_parallel_for calls
      ...
      if (const Function *taskFunc =
        ...
        assert((taskFunc->arg_size() == 3) &&
               "Size of formal parameter of hare_parallel_for's task routine "
               "should be 3");
...
void SVFIRBuilder::sanityCheck() {
  for (SVFIR::iterator nIter = pag->begin(); nIter != pag->end(); ++nIter) {
    ...
    //      otherwise it causes the a null pointer, the aliasing relation may
    //      not be captured when loading a pointer value should make sure some
    //      value has been store into this pointer before q = load p, some value
    //      should stored into p first like store w p;
    // (3)  make sure PAGNode should not have a const expr value (pointer should
    // have unique def) (4)  look closely into addComplexConsForExt, make sure
    // program locations(e.g.,inst bb)
 ...
 * table
 */
NodeID SVFIRBuilder::getGepValVar(const Value *val, const LocationSet &ls,
                                  const Type *elementType) {
  ...
  if (gepval == UINT_MAX) {
    ...
    NodeID gepNode = pag->addGepValNode(
        curVal, val, ls, NodeIDAllocator::get()->allocateValueId(),
 ...
 * SVFIRBuilder::processCE) GepEdge   (GepConstantExpr SVFIRBuilder::processCE)
 * ConstantPointerNull  CopyEdge  (3-->2 NullPtr-->BlkPtr SVFIR::addNullPtrNode)
 *  				    AddrEdge  (0-->2 BlkObj-->BlkPtr
 * SVFIR::addNullPtrNode) GlobalVariable       AddrEdge
 * (SVFIRBuilder::visitGlobal) GepEdge   (SVFIRBuilder::getGlobalVarField)
...
void SVFIRBuilder::setCurrentBBAndValueForPAGEdge(PAGEdge *edge) {
  ...
  if (const Instruction *curInst = SVFUtil::dyn_cast<Instruction>(curVal)) {
    ...
    /// We assume every GepValVar and its GepStmt are unique across whole
    /// program
    if (!(SVFUtil::isa<GepStmt>(edge) &&
          SVFUtil::isa<GepValVar>(edge->getDstNode())))
    ...
    if ((&fun->getEntryBlock() == curBB) && isExtCall(f)) {
      /// all external function connected to a indirect call, we will put
      /// SVFStmts in the FunctionEntryICFGNode
  ...
  if (const CallPE *callPE = SVFUtil::dyn_cast<CallPE>(edge)) {
    ...
    FunEntryICFGNode *entryNode =
        const_cast<FunEntryICFGNode *>(callPE->getFunEntryICFGNode());
    if (ICFGEdge *edge = pag->getICFG()->hasInterICFGEdge(callNode, entryNode,
                                                          ICFGEdge::CallCF))
      SVFUtil::cast<CallCFGEdge>(edge)->addCallPE(callPE);
  } else if (const RetPE *retPE = SVFUtil::dyn_cast<RetPE>(edge)) {
    RetICFGNode *retNode =
        const_cast<RetICFGNode *>(retPE->getCallSite()->getRetICFGNode());
    FunExitICFGNode *exitNode =
        const_cast<FunExitICFGNode *>(retPE->getFunExitICFGNode());
    if (ICFGEdge *edge = pag->getICFG()->hasInterICFGEdge(exitNode, retNode,
                                                          ICFGEdge::RetCF))
...
void SVFIRBuilder::updateCallGraph(PTACallGraph *callgraph) {
  ...
  for (; iter != eiter; iter++) {
    ...
    for (PTACallGraph::FunctionSet::const_iterator func_iter =
      ...
      if (isExtCall(callee)) {
        setCurrentLocation(callee->getLLVMFun(),
                           &callee->getLLVMFun()->getEntryBlock());
        handleExtCall(cs, callee);
      } else {
        setCurrentLocation(callBlock->getCallSite(),
                           callBlock->getCallSite()->getParent());
        handleDirectCall(cs, callee);
      }
    }
  }
