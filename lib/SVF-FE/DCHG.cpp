  ...
  case dwarf::DW_TAG_union_type:
    ...
    if (extended) {
      ...
      for (DINode *field : fields) {
        // fields[0] gives a type which is DW_TAG_member, we want the member's
        // type (getBaseType).
...
void DCHGraph::handleDIDerivedType(const DIDerivedType *derivedType) {
  ...
  case dwarf::DW_TAG_inheritance: {
    ...
    // If the offset does not exist (for primary base), getConstantFieldIdx
    // should return 0.
...
void DCHGraph::handleTypedef(const DIType *typedefType) {
  ...
  while (typedefType != nullptr &&
    ...
    const DIDerivedType *typedefDerivedType =
...
void DCHGraph::buildVTables(const Module &module) {
  for (Module::const_global_iterator gvI = module.global_begin();
    ...
    if (gv->hasMetadata(cppUtil::ctir::vtMDName) && gv->getNumOperands() > 0) {
      DIType *type =
          SVFUtil::dyn_cast<DIType>(gv->getMetadata(cppUtil::ctir::vtMDName));
      ...
      for (unsigned nthVtbl = 0; nthVtbl < vtbls->getNumOperands(); ++nthVtbl) {
        const ConstantArray *vtbl =
            SVFUtil::dyn_cast<ConstantArray>(vtbls->getOperand(nthVtbl));
        ...
        // 3. i8* bitcast  ... (we only care when a function pointer is being
        // bitcasted).
        for (unsigned cN = 0; cN < vtbl->getNumOperands(); ++cN) {
          ...
          if (ce->getOpcode() == Instruction::BitCast) {
            // Could be a GlobalAlias which we don't care about, or a
            // virtual/thunk function.
            const Function *vfn =
...
const NodeBS &DCHGraph::cha(const DIType *type, bool firstField) {
  ...
  for (const DCHEdge *edge : node->getInEdges()) {
    // Don't care about anything but inheritance, first-field, and standard def.
    // edges.
...
void DCHGraph::flatten(const DICompositeType *type) {
  ...
  for (unsigned i = 0; i < fieldsDINA.size(); ++i) {
    if (const DIDerivedType *dt =
            SVFUtil::dyn_cast<DIDerivedType>(fieldsDINA[i])) {
  ...
  if (type->getTag() == dwarf::DW_TAG_array_type) {
    ...
    for (unsigned i = 0; i < fields.size(); ++i) {
      ...
      if (const DIDerivedType *mt =
              SVFUtil::dyn_cast<DIDerivedType>(fields[i])) {
...
void DCHGraph::buildCHG(bool extend) {
  ...
  for (const DIType *type : finder.types()) {
    ...
    } else if (const DIDerivedType *derivedType =
                   SVFUtil::dyn_cast<DIDerivedType>(type)) {
      handleDIDerivedType(derivedType);
    } else if (const DISubroutineType *subroutineType =
...
bool DCHGraph::isFieldOf(const DIType *f, const DIType *b) {
  ...
  if (b->getTag() == dwarf::DW_TAG_array_type ||
    ...
    if (const DICompositeType *arrayType =
            SVFUtil::dyn_cast<DICompositeType>(b)) {
      baseType = arrayType->getBaseType();
    } else if (const DIDerivedType *ptrType =
                   SVFUtil::dyn_cast<DIDerivedType>(b)) {
...
const DIType *DCHGraph::stripQualifiers(const DIType *t) {
  while (true) {
    ...
    // Verbose for clarity.
    if (tag == dwarf::DW_TAG_const_type || tag == dwarf::DW_TAG_atomic_type ||
...
bool DCHGraph::teq(const DIType *t1, const DIType *t2) {
  ...
  if (SVFUtil::isa<DICompositeType>(t1) && SVFUtil::isa<DICompositeType>(t2)) {
    ...
    return ct1->getName() == ct2->getName() &&
           ct1->getFile() == ct2->getFile() && ct1->getLine() == ct2->getLine();
  }
...
std::string DCHGraph::diTypeToStr(const DIType *t) {
  ...
  if (const DIBasicType *bt = SVFUtil::dyn_cast<DIBasicType>(t)) {
  ...
  } else if (const DICompositeType *ct =
                 SVFUtil::dyn_cast<DICompositeType>(t)) {
    if (ct->getTag() == dwarf::DW_TAG_class_type ||
      ...
      if (ct->getName() != "") {
        ...
        for (unsigned i = 0; i < fields.size(); ++i) {
          // fields[i] gives a type which is DW_TAG_member, we want the member's
          // type (getBaseType). It can also give a Subprogram type if the class
          // just had non-virtual functions.
          if (const DISubprogram *sp =
                  SVFUtil::dyn_cast<DISubprogram>(fields[i])) {
            ss << std::string(sp->getName());
          } else if (const DIDerivedType *mt =
                         SVFUtil::dyn_cast<DIDerivedType>(fields[i])) {
            assert(mt->getTag() == dwarf::DW_TAG_member &&
  ...
  } else if (const DISubroutineType *st =
                 SVFUtil::dyn_cast<DISubroutineType>(t)) {
...
void DCHGraph::print(void) {
  ...
  for (NodeID id : nodes) {
    ...
    SVFUtil::outs() << indent(currIndent) << id << ": "
                    << diTypeToStr(node->getType()) << " [" << node->getType()
                    << "]"
                    << "\n";
    ...
    for (const DCHEdge *edge : node->getOutEdges()) {
      ...
      SVFUtil::outs() << indent(currIndent) << "[ "
                      << diTypeToStr(node->getType()) << " ] " << arrow << " [ "
