...
PTACFInfoBuilder::~PTACFInfoBuilder() {
  for (FunToLoopInfoMap::iterator it = funToLoopInfoMap.begin(),
                                  eit = funToLoopInfoMap.end();
       it != eit; ++it) {
    if (it->second != nullptr) {
      delete it->second;
    }
  }
  for (FunToDTMap::iterator it = funToDTMap.begin(), eit = funToDTMap.end();
       it != eit; ++it) {
    if (it->second != nullptr) {
      delete it->second;
    }
  }
  for (FunToPostDTMap::iterator it = funToPDTMap.begin(),
                                eit = funToPDTMap.end();
       it != eit; ++it) {
    if (it->second != nullptr) {
      delete it->second;
