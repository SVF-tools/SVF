...
void SymbolTableBuilder::buildMemModel(SVFModule *svfModule) {
  ...
  assert(symInfo->totalSymNum++ == SymbolTableInfo::NullPtr &&
         "Something changed!");

  // Pointer #1 always represents the pointer points-to black hole.
  assert(symInfo->totalSymNum++ == SymbolTableInfo::BlkPtr &&
         "Something changed!");

  // Object #2 is black hole the object that may point to any object
  assert(symInfo->totalSymNum++ == SymbolTableInfo::BlackHole &&
         "Something changed!");
  symInfo->createBlkObj(SymbolTableInfo::BlackHole);

  // Object #3 always represents the unique constant of a program (merging all
  // constants if Options::ModelConsts is disabled)
  assert(symInfo->totalSymNum++ == SymbolTableInfo::ConstantObj &&
         "Something changed!");
  ...
  for (SVFModule::llvm_iterator F = svfModule->llvmFunBegin(),
...
void SymbolTableBuilder::collectSym(const Value *val) {
  ...
  DBOUT(DMemModel,
        outs() << "collect sym from ##" << SVFUtil::value2String(val) << " \n");
...
void SymbolTableBuilder::collectVal(const Value *val) {
  ...
  if (iter == symInfo->valSymMap.end()) {
    ...
    if (const GlobalVariable *globalVar =
...
void SymbolTableBuilder::collectObj(const Value *val) {
  ...
  if (iter == symInfo->objSymMap.end()) {
    // if the object pointed by the pointer is a constant data (e.g., i32 0) or
    // a global constant object (e.g. string) then we treat them as one
    // ConstantObj
...
ObjTypeInfo *SymbolTableBuilder::createObjTypeInfo(const Value *val) {
  ...
  if (refTy) {
    ...
    if (symInfo->isConstantObjSym(val)) {
      ...
      assert(false &&
             "Memory object must be either (1) held by a pointer-typed ref "
             "value or (2) a constant value (e.g., 10).");
      abort();
    }
  }
}
...
void SymbolTableBuilder::analyzeObjType(ObjTypeInfo *typeinfo,
  ...
  if (const StructType *ST = SVFUtil::dyn_cast<StructType>(elemTy)) {
    const std::vector<const Type *> &flattenFields =
        SymbolTableInfo::SymbolInfo()->getFlattenFieldTypes(ST);
    for (std::vector<const Type *>::const_iterator it = flattenFields.begin(),
                                                   eit = flattenFields.end();
         it != eit; ++it) {
      if ((*it)->isPointerTy())
        isPtrObj = true;
    }
 ...
 */
void SymbolTableBuilder::analyzeHeapObjType(ObjTypeInfo *typeinfo,
                                            const Value *val) {
...
void SymbolTableBuilder::initTypeInfo(ObjTypeInfo *typeinfo, const Value *val) {
  ...
  if (SVFUtil::isa<Function>(val)) {
  ...
  } else if (const AllocaInst *allocaInst =
                 SVFUtil::dyn_cast<AllocaInst>(val)) {
    typeinfo->setFlag(ObjTypeInfo::STACK_OBJ);
    analyzeObjType(typeinfo, val);
    /// This is for `alloca <ty> <NumElements>`. For example, `alloca i64 3`
    /// allocates 3 i64 on the stack (objSize=3) In most cases, `NumElements` is
    /// not specified in the instruction, which means there is only one element
    /// (objSize=1).
    if (const ConstantInt *sz =
            SVFUtil::dyn_cast<ConstantInt>(allocaInst->getArraySize()))
    ...
    objSize =
        SymbolTableInfo::SymbolInfo()->getNumOfFlattenElements(val->getType());
  } else {
