...
void CHGBuilder::buildCHGNodes(const SVFFunction *fun) {
  ...
  if (isConstructor(F) || isDestructor(F)) {
    ...
    DBOUT(DCHA,
          outs() << "\t build CHANode for class " + dname.className + "...\n");
    if (!chg->getNode(dname.className))
      createNode(dname.className);
  }
}
...
void CHGBuilder::buildCHGEdges(const SVFFunction *fun) {
  ...
  if (isConstructor(F) || isDestructor(F)) {
    for (Function::const_iterator B = F->begin(), E = F->end(); B != E; ++B) {
      for (BasicBlock::const_iterator I = B->begin(), E = B->end(); I != E;
           ++I) {
...
void CHGBuilder::connectInheritEdgeViaCall(const SVFFunction *callerfun,
                                           CallSite cs) {
  ...
  if ((isConstructor(caller) && isConstructor(callee)) ||
    ...
    if (cs.arg_size() < 1 ||
        (cs.arg_size() < 2 && cs.paramHasAttr(0, llvm::Attribute::StructRet)))
      return;
    ...
    if (csThisPtr != nullptr && samePtrTrue) {
      ...
      if (!SVFUtil::isCallSite(csThisPtr) && basename.className.size() > 0) {
...
void CHGBuilder::connectInheritEdgeViaStore(const SVFFunction *caller,
        ...
        if (bcce->getOpcode() == Instruction::GetElementPtr) {
          ...
          if (isValVtbl(gepval)) {
            ...
            if (vtblClassName.size() > 0 &&
                dname.className.compare(vtblClassName) != 0) {
...
void CHGBuilder::buildClassNameToAncestorsDescendantsMap() {
  ...
  for (CHGraph::const_iterator it = chg->begin(), eit = chg->end(); it != eit;
       ++it) {
    ...
    while (!worklist.empty()) {
      ...
      if (visitedNodes.find(curnode) == visitedNodes.end()) {
        for (CHEdge::CHEdgeSetTy::const_iterator
                 it = curnode->getOutEdges().begin(),
...
CHGBuilder::getInstancesAndDescendants(const string className) {

  CHGraph::NameToCHNodesMap::const_iterator it =
      chg->classNameToInstAndDescsMap.find(className);
    ...
    if (chg->getNode(className)->isTemplate()) {
      ...
      for (CHNodeSetTy::const_iterator it = instances.begin(),
        ...
        const CHNodeSetTy &instance_descendants =
            chg->getDescendants(node->getName());
        for (CHNodeSetTy::const_iterator dit = instance_descendants.begin(),
                                         deit = instance_descendants.end();
             dit != deit; ++dit) {
...
void CHGBuilder::analyzeVTables(const Module &M) {
  for (Module::const_global_iterator I = M.global_begin(), E = M.global_end();
    ...
    const GlobalValue *globalvalue =
        SVFUtil::dyn_cast<const GlobalValue>(&(*I));
    if (isValVtbl(globalvalue) && globalvalue->getNumOperands() > 0) {
      const ConstantStruct *vtblStruct =
          SVFUtil::dyn_cast<ConstantStruct>(globalvalue->getOperand(0));
      assert(vtblStruct && "Initializer of a vtable not a struct?");
...
void CHGBuilder::buildVirtualFunctionToIDMap() {
  ...
  for (CHGraph::const_iterator nit = chg->begin(), neit = chg->end();
       nit != neit; ++nit) {
    ...
    for (CHGraph::CHNodeSetTy::iterator it = group.begin(), eit = group.end();
         it != eit; ++it) {
...
void CHGBuilder::buildCSToCHAVtblsAndVfnsMap() {
  ...
  for (SymbolTableInfo::CallSiteSet::const_iterator
           it = SymbolTableInfo::SymbolInfo()->getCallSiteSet().begin(),
           eit = SymbolTableInfo::SymbolInfo()->getCallSiteSet().end();
       it != eit; ++it) {
    CallSite cs = *it;
...
const CHGraph::CHNodeSetTy &CHGBuilder::getCSClasses(CallSite cs) {
  ...
  CHGraph::CallSiteToCHNodesMap::const_iterator it =
      chg->csToClassesMap.find(cs);
  if (it != chg->csToClassesMap.end()) {
    ...
    if (const CHNode *thisNode = chg->getNode(thisPtrClassName)) {
      const CHGraph::CHNodeSetTy &instAndDesces =
          getInstancesAndDescendants(thisPtrClassName);
      chg->csToClassesMap[cs].insert(thisNode);
      for (CHGraph::CHNodeSetTy::const_iterator it = instAndDesces.begin(),
                                                eit = instAndDesces.end();
           it != eit; ++it)
