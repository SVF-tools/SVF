...
void LLVMModuleSet::loadModules(const std::vector<std::string> &moduleNameVec) {
  ...
  // We should use only one LLVMContext object for multiple modules in the same
  // thread. No such problem if only one module is processed by SVF.
  // ------------------------------------------------------------------
...
void LLVMModuleSet::initialize() {
  ...
  for (Module &mod : modules) {
    ...
    /// GlobalAlias
    for (Module::alias_iterator it = mod.alias_begin(), eit = mod.alias_end();
         it != eit; ++it) {
...
void LLVMModuleSet::addSVFMain() {
  ...
  if (orgMain && getModuleNum() > 0 && init_funcs.size() > 0) {
    ...
    for (auto &init : init_funcs) {
      auto target = M.getOrInsertFunction(init->getName(),
...
void LLVMModuleSet::buildFunToFunMap() {
  ...
  for (Set<Function *>::iterator it = funDefs.begin(), eit = funDefs.end();
       it != eit; ++it) {
    Function *fdef = *it;
  ...
  for (Set<Function *>::iterator it = funDecls.begin(), eit = funDecls.end();
       it != eit; ++it) {
    Function *fdecl = *it;
