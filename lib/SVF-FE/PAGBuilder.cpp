...
void PAGBuilder::visitCallSite(CallSite cs)
{
    ...
    for(auto itA = cs->arg_begin(), ieA = cs->arg_end(); itA!=ieA; ++itA)
        pag->addCallSiteArgs(callBlockNode,pag->getPAGNode(getValueNode(*itA)));

    if(!cs->getType()->isVoidTy())
        pag->addCallSiteRets(retBlockNode,pag->getPAGNode(getValueNode(cs)));
...
void PAGBuilder::handleDirectCall(CallSite cs, const SVFFunction *F)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "handle direct call " << *cs << " callee " << *F << "\n");
    ...
    else
    {
        if(isExtCall(callee))
        {
            ...
            case ExtAPI::EFT_REALLOC:
            {
                ...
                if(SVFUtil::isa<ConstantPointerNull>(cs->getArgOperand(0)))
                {
                    NodeID val = getValueNode(inst);
                    NodeID obj = getObjectNode(inst);
                    addAddrEdge(obj, val);
            ...
            case ExtAPI::EFT_L_A8:
            {
                ...
                Value *src= cs->getArgOperand(arg_pos);
            ...
            case ExtAPI::EFT_L_A0__A0R_A1:
            {
                ...
                if(SVFUtil::isa<PointerType>(inst->getType()))
                    addCopyEdge(getValueNode(cs->getArgOperand(0)), getValueNode(inst));
                break;
            }
            case ExtAPI::EFT_L_A0__A0R_A1R:
            {
                addComplexConsForExt(cs->getArgOperand(0), cs->getArgOperand(1));
            ...
            case ExtAPI::EFT_A1R_A0R:
                addComplexConsForExt(cs->getArgOperand(1), cs->getArgOperand(0));
                break;
            case ExtAPI::EFT_A3R_A1R_NS:
                //These func. are never used to copy structs, so the size is 1.
                addComplexConsForExt(cs->getArgOperand(3), cs->getArgOperand(1), 1);
                break;
            case ExtAPI::EFT_A1R_A0:
            {
                NodeID vnD= getValueNode(cs->getArgOperand(1));
                NodeID vnS= getValueNode(cs->getArgOperand(0));
                if(vnD && vnS)
            ...
            {
                NodeID vnD= getValueNode(cs->getArgOperand(2));
                NodeID vnS= getValueNode(cs->getArgOperand(1));
                if(vnD && vnS)
                    addStoreEdge(vnS,vnD);
                break;
            }
            ...
            {
                NodeID vnD= getValueNode(cs->getArgOperand(4));
                NodeID vnS= getValueNode(cs->getArgOperand(1));
                if(vnD && vnS)
                    addStoreEdge(vnS,vnD);
                break;
			...
			case ExtAPI::EFT_L_A0__A1_A0:
			{
				...
				/// dst = load base
				/// store src base
				if (const LoadInst *load = SVFUtil::dyn_cast<LoadInst>(cs->getArgOperand(0))) {
					addStoreEdge(getValueNode(cs->getArgOperand(1)), getValueNode(load->getPointerOperand()));
					if (SVFUtil::isa<PointerType>(inst->getType()))
...
//				}
            ...
            {
                Value *vArg1 = cs->getArgOperand(1);
                Value *vArg3 = cs->getArgOperand(3);
            ...
            {
                Value *vSrc = cs->getArgOperand(0);
                Value *vDst = cs->getArgOperand(1);
            ...
            case ExtAPI::CPP_EFT_A0R_A1:
            {
                ...
                {
                    NodeID vnD = pag->getValueNode(cs->getArgOperand(0));
                    NodeID vnS = pag->getValueNode(cs->getArgOperand(1));
                    addStoreEdge(vnS, vnD);
                }
                break;
            }
            case ExtAPI::CPP_EFT_A0R_A1R:
            {
                ...
                if (symTable->getModelConstants())
                {
                    NodeID vnD = getValueNode(cs->getArgOperand(0));
                    NodeID vnS = getValueNode(cs->getArgOperand(1));
            ...
            {
                Value *vArg0 = cs->getArgOperand(0);
                Value *retVal = cs;
...
{
    const CallBlockNode* cbn = pag->getICFG()->getCallBlockNode(cs);
