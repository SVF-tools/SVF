...
SVFIR *MemSSA::getPAG() { return pta->getPAG(); }
...
void MemSSA::createMUCHI(const SVFFunction &fun) {
  ...
  for (BBList::const_iterator iter = reachableBBs.begin(),
...
void MemSSA::insertPHI(const SVFFunction &fun) {
  ...
  for (MRSet::iterator iter = usedRegs.begin(), eiter = usedRegs.end();
    ...
    while (!bbs.empty()) {
      ...
      bbs.pop_back();
      DominanceFrontierBase::const_iterator it =
          df->find(const_cast<BasicBlock *>(bb));
      if (it == df->end()) {
...
void MemSSA::destroy() {
  ...
  for (LoadToMUSetMap::iterator iter = load2MuSetMap.begin(),
                                eiter = load2MuSetMap.end();
       iter != eiter; ++iter) {
    for (MUSet::iterator it = iter->second.begin(), eit = iter->second.end();
         it != eit; ++it) {
      delete *it;
    }
  }

  for (StoreToChiSetMap::iterator iter = store2ChiSetMap.begin(),
                                  eiter = store2ChiSetMap.end();
       iter != eiter; ++iter) {
    for (CHISet::iterator it = iter->second.begin(), eit = iter->second.end();
         it != eit; ++it) {
      delete *it;
    }
  }
  ...
  for (BBToPhiSetMap::iterator iter = bb2PhiSetMap.begin(),
                               eiter = bb2PhiSetMap.end();
       iter != eiter; ++iter) {
    for (PHISet::iterator it = iter->second.begin(), eit = iter->second.end();
         it != eit; ++it) {
      delete *it;
    }
  }
...
void MemSSA::dumpMSSA(OutStream &Out) {
  ...
  for (SVFModule::iterator fit = pta->getModule()->begin(),
                           efit = pta->getModule()->end();
       fit != efit; ++fit) {
    ...
    if (hasFuncEntryChi(fun)) {
      ...
      for (CHISet::iterator chi_it = entry_chis.begin();
           chi_it != entry_chis.end(); chi_it++) {
        (*chi_it)->dump();
      }
    }
    ...
    for (Function::iterator bit = fun->getLLVMFun()->begin(),
      ...
      for (PHISet::iterator pi = phiSet.begin(), epi = phiSet.end(); pi != epi;
           ++pi) {
        (*pi)->dump();
      }
      ...
      for (BasicBlock::iterator it = bb.begin(), eit = bb.end(); it != eit;
        ...
        if (isAppCall || isHeapAllocExtCall(&inst)) {
          ...
          if (hasMU(cs)) {
            ...
            for (MUSet::iterator mit = getMUSet(cs).begin(),
                                 emit = getMUSet(cs).end();
                 mit != emit; ++mit) {
              (*mit)->dump();
            }
          }
          ...
          if (hasCHI(cs)) {
            for (CHISet::iterator cit = getCHISet(cs).begin(),
                                  ecit = getCHISet(cs).end();
                 cit != ecit; ++cit) {
              (*cit)->dump();
            }
            Out << "\n";
          ...
          for (SVFStmtList::const_iterator bit = pagEdgeList.begin(),
            ...
            if (const LoadStmt *load = SVFUtil::dyn_cast<LoadStmt>(edge)) {
              ...
              for (MUSet::iterator it = muSet.begin(), eit = muSet.end();
                   it != eit; ++it) {
          ...
          for (SVFStmtList::const_iterator bit = pagEdgeList.begin(),
            ...
            if (const StoreStmt *store = SVFUtil::dyn_cast<StoreStmt>(edge)) {
              ...
              for (CHISet::iterator it = chiSet.begin(), eit = chiSet.end();
                   it != eit; ++it) {
                has_chi = true;
                (*it)->dump();
              }
    ...
    if (hasReturnMu(fun)) {
      ...
      for (MUSet::iterator mu_it = return_mus.begin();
           mu_it != return_mus.end(); mu_it++) {
        (*mu_it)->dump();
      }
    }
  }
