 ...
 */
void DistinctMRG::getMRsForLoad(MRSet &mrs, const NodeBS &pts,
                                const SVFFunction *) {
 ...
 */
void DistinctMRG::getMRsForCallSiteRef(MRSet &aliasMRs, const NodeBS &cpts,
...
void IntraDisjointMRG::partitionMRs() {
  for (FunToPointsTosMap::iterator it = getFunToPointsToList().begin(),
    ...
    for (PointsToList::const_iterator interIt = inters.begin(),
                                      interEit = inters.end();
...
void IntraDisjointMRG::computeIntersections(const NodeBS &cpts,
  ...
  if (inters.find(cpts) != inters.end()) {
    ...
    // If this cpts has only one element, it will not intersect with any cpts in
    // inters, just add it into intersection set.
    inters.insert(cpts);
    return;
  } else {
    ...
    for (PointsToList::const_iterator it = toBeDeleted.begin(),
                                      eit = toBeDeleted.end();
         it != eit; ++it) {
    ...
    for (PointsToList::const_iterator it = newInters.begin(),
                                      eit = newInters.end();
         it != eit; ++it) {
      const NodeBS &temp_cpts = *it;
 ...
 * Just process as getMRsForLoad().
 */
void IntraDisjointMRG::getMRsForCallSiteRef(MRSet &aliasMRs, const NodeBS &cpts,
                                            const SVFFunction *fun) {
  getMRsForLoad(aliasMRs, cpts, fun);
}
