...
void MRGenerator::destroy() {
  ...
  for (MRSet::iterator it = memRegSet.begin(), eit = memRegSet.end(); it != eit;
       ++it) {
    delete *it;
  }
...
void MRGenerator::generateMRs() {
  ...
  DBOUT(DGENERAL,
        outs() << pasMsg("\tCollect ModRef For const CallICFGNode*\n"));
...
void MRGenerator::collectModRefForLoadStore() {
  ...
  for (SVFModule::const_iterator fi = svfModule->begin(),
                                 efi = svfModule->end();
       fi != efi; ++fi) {
    ...
    for (Function::const_iterator iter = fun.getLLVMFun()->begin(),
      ...
      for (BasicBlock::const_iterator bit = bb.begin(), ebit = bb.end();
        ...
        for (SVFStmtList::iterator bit = pagEdgeList.begin(),
                                   ebit = pagEdgeList.end();
             bit != ebit; ++bit) {
          ...
          if (const StoreStmt *st = SVFUtil::dyn_cast<StoreStmt>(inst)) {
            ...
            // TODO: change this assertion check later when we have conditional
            // points-to set
            if (cpts.empty())
              continue;
...
void MRGenerator::collectModRefForCall() {
  ...
  for (SVFIR::CallSiteSet::const_iterator
           it = pta->getPAG()->getCallSiteSet().begin(),
  ...
  while (!worklist.empty()) {
    ...
    for (NodeBS::iterator it = subNodes.begin(), eit = subNodes.end();
         it != eit; ++it) {
  ...
  for (CallSite cs : SymbolTableInfo::SymbolInfo()->getCallSiteSet()) {
    const CallICFGNode *callBlockNode =
        pta->getPAG()->getICFG()->getCallICFGNode(cs.getInstruction());
...
void MRGenerator::sortPointsTo(const NodeBS &cpts) {
  ...
  for (PointsToList::iterator it = subSetList.begin(), eit = subSetList.end();
       it != eit; ++it) {
...
void MRGenerator::partitionMRs() {
  ...
  for (FunToPointsTosMap::iterator it = getFunToPointsToList().begin(),
                                   eit = getFunToPointsToList().end();
       it != eit; ++it) {
    for (PointsToList::iterator cit = it->second.begin(),
                                ecit = it->second.end();
         cit != ecit; ++cit) {
      sortPointsTo(*cit);
    }
...
void MRGenerator::updateAliasMRs() {
  ...
  for (StoresToPointsToMap::const_iterator it = storesToPointsToMap.begin(),
                                           eit = storesToPointsToMap.end();
       it != eit; ++it) {
    MRSet aliasMRs;
    const SVFFunction *fun = getFunction(it->first);
    const NodeBS &storeCPts = it->second;
    getAliasMemRegions(aliasMRs, storeCPts, fun);
    for (MRSet::iterator ait = aliasMRs.begin(), eait = aliasMRs.end();
         ait != eait; ++ait) {
      storesToMRsMap[it->first].insert(*ait);
    }
  }

  for (LoadsToPointsToMap::const_iterator it = loadsToPointsToMap.begin(),
                                          eit = loadsToPointsToMap.end();
       it != eit; ++it) {
    MRSet aliasMRs;
    const SVFFunction *fun = getFunction(it->first);
  ...
  for (CallSiteToPointsToMap::const_iterator
           it = callsiteToModPointsToMap.begin(),
  ...
  for (CallSiteToPointsToMap::const_iterator
           it = callsiteToRefPointsToMap.begin(),
...
NodeBS &MRGenerator::CollectPtsChain(NodeID id) {
  ...
  if (it != cachedPtsChainMap.end())
    ...
    while (!worklist.empty()) {
      NodeID nodeId = worklist.pop();
      const NodeBS &tmp = pta->getPts(nodeId).toNodeBS();
      for (NodeBS::iterator it = tmp.begin(), eit = tmp.end(); it != eit;
           ++it) {
...
void MRGenerator::getEscapObjviaGlobals(NodeBS &globs,
  ...
  for (NodeBS::iterator it = calleeModRef.begin(), eit = calleeModRef.end();
       it != eit; ++it) {
    ...
    if (const Function *fun = pta->getPAG()->getGNode(id)->getFunction()) {
      ...
      if (svffun != curFun)
        ...
        return callGraphSCC->isInCycle(
            callGraph->getCallGraphNode(svffun)->getId());
    }
  }
...
bool MRGenerator::handleCallsiteModRef(NodeBS &mod, NodeBS &ref,
  ...
  /// if a callee is a heap allocator function, then its mod set of this
  /// callsite is the heap object.
  ...
  /// otherwise, we find the mod/ref sets from the callee function, who has
  /// definition and been processed
  else {
...
void MRGenerator::modRefAnalysis(PTACallGraphNode *callGraphNode,
  ...
  for (PTACallGraphNode::iterator it = callGraphNode->InEdgeBegin(),
    ...
    for (PTACallGraphEdge::CallInstSet::iterator
             cit = edge->getDirectCalls().begin(),
             ecit = edge->getDirectCalls().end();
         cit != ecit; ++cit) {
      NodeBS mod, ref;
      const CallICFGNode *cs = (*cit);
      bool modrefchanged =
          handleCallsiteModRef(mod, ref, cs, callGraphNode->getFunction());
      if (modrefchanged)
        worklist.push(edge->getSrcID());
    }
    /// handle indirect callsites
    for (PTACallGraphEdge::CallInstSet::iterator
             cit = edge->getIndirectCalls().begin(),
