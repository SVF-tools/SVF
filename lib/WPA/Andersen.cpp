...
void AndersenBase::cleanConsCG(NodeID id) {
  ...
  assert(!consCG->hasGNode(id) &&
         "this is either a rep nodeid or a sub nodeid should have already been "
         "merged to its field-insensitive base! ");
}
  ...
  if (SVFUtil::isa<VariantGepCGEdge>(edge)) {
    ...
    for (NodeID o : pts) {
      ...
      NodeID fieldSrcPtdNode =
          consCG->getGepObjVar(o, normalGepEdge->getLocationSet());
...
inline void Andersen::collapsePWCNode(NodeID nodeId) {
  ...
  // collapseNodePts() may change the points-to set of the nodes which have been
  // processed before, in this case, we may need to re-do the analysis.
...
inline void Andersen::collapseFields() {
  while (consCG->hasNodesToBeCollapsed()) {
    ...
    // collapseField() may change the points-to set of the nodes which have been
    // processed before, in this case, we may need to re-do the analysis.
  ...
  for (NodeBS::iterator fieldIt = allFields.begin(), fieldEit = allFields.end();
    ...
    if (fieldId != baseId) {
      ...
      if (fieldId != baseRepNodeId) {
        // gep node fieldId becomes redundant if it is merged to its base node
        // who is set as field-insensitive two node IDs should be different
        // otherwise this field is actually the base and should not be removed.
...
bool Andersen::mergeSrcToTgt(NodeID nodeId, NodeID newRepId) {
  ...
  /// 1. if find gep edges inside SCC cycle, the rep node will become a PWC node
  /// and its pts should be collapsed later.
  /// 2. if the node to be merged is already a PWC node, the rep node will also
  /// become a PWC node as it will have a self-cycle gep edge.
  if (node->isPWCNode())
    pwc = true;
...
void Andersen::cluster(void) const {
  ...
  PointsTo::MappingPtr nodeMapping =
      std::make_shared<std::vector<NodeID>>(NodeIDAllocator::Clusterer::cluster(
          steens, keys, candidates, "aux-steens"));
  ...
  for (OrderedNodeSet::iterator nIter = this->getAllValidPtrs().begin();
    ...
    if (getPAG()->isValidTopLevelPtr(node)) {
      ...
      if (pts.empty()) {
        ...
        for (multiset<u32_t>::const_iterator it = line.begin();
             it != line.end(); ++it)
          outs() << *it << " ";
        outs() << "}\n\n";
      }
