...
bool AndersenSFR::processGepPts(const PointsTo &pts, const GepCGEdge *edge) {
      ...
      SVFUtil::isa<NormalGepCGEdge>(edge)) // dst is in pwc
  {
    ...
    if (!srcInits.empty()) {
      ...
      s32_t offset =
          SVFUtil::dyn_cast<NormalGepCGEdge>(edge)->getConstantFieldIdx();
 ...
 *
 */
void AndersenSFR::fieldExpand(NodeSet &initials, s32_t offset, NodeBS &strides,
                              PointsTo &expandPts) {
  numOfFieldExpand++;
  ...
  while (!initials.empty()) {
    ...
    if (consCG->isBlkObjOrConstantObj(init))
      ...
      else if (SVFUtil::isa<FIObjVar>(initPN) ||
               SVFUtil::isa<DummyObjVar>(initPN))
        initOffset = 0;
      else
      ...
      for (s32_t _f : offsets) {
        ...
        initials.erase(gepId); // gep id in initials should be removed to avoid
