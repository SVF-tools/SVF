...
void TypeBasedHeapCloning::setPAG(SVFIR *pag) { ppag = pag; }

bool TypeBasedHeapCloning::isBlkObjOrConstantObj(NodeID o) const {
  if (isClone(o))
    o = cloneToOriginalObj.at(o);
  return SVFUtil::isa<ObjVar>(ppag->getGNode(o)) &&
...
const NodeBS TypeBasedHeapCloning::getGepObjClones(NodeID base,
  ...
  if (geps.empty()) {
    ...
    if (baseType->getTag() == dwarf::DW_TAG_array_type ||
      ...
      // Don't use totalOffset because we're operating on the Gep object which
      // is our parent (i.e. field of some base, not the base itself).
...
bool TypeBasedHeapCloning::init(NodeID loc, NodeID p, const DIType *tildet,
  ...
  for (NodeID o : pPt) {
    ...
    if (tp == undefType) {
      ...
      ++numInit;
      if (!pta->isHeapMemObj(o) && !SVFUtil::isa<DummyObjVar>(obj))
        ++numSGInit;
      ...
      // struct Y from a;` so we want the second instruction to be operating on
      // an object of type 'Struct S', not 'Array of S'.
      prop = cloneObject(o, tildet, false);
      ++numAgg;
      if (!pta->isHeapMemObj(o) && !SVFUtil::isa<DummyObjVar>(obj))
        ++numSGAgg;
      ...
      //  stack/globals.
      //  - isFirstField because ^ can happen because when we take the field of
      //  a
    ...
    if (prop != o) {
      // If we cloned, we want to keep o in p's PTS but filter it (ignore it
      // later).
      pNewPt.set(o);
      filterSet.set(o);
      // TODO: hack, sound but imprecise and unclean.
      // In the aggs case there is a difference between it being good for
      // arrays and structs. For now, just propagate both the clone and the
      // original object till a cleaner solution is found.
...
NodeID TypeBasedHeapCloning::cloneObject(NodeID o, const DIType *type, bool) {
  ...
  if (const GepObjVar *gepObj = SVFUtil::dyn_cast<GepObjVar>(obj)) {
    const NodeBS &clones =
        getGepObjClones(gepObj->getBaseNode(), gepObj->getConstantFieldIdx());
    ...
    // The only instance of original object of a Gep object being retrieved is
    // for IN sets and gepToSVFGRetriever in FSTBHC, so we don't care that clone
    // comes from o (we can get that by checking the base and offset).
    setOriginalObj(clone, getOriginalObj(o));
    CloneGepObjVar *cloneGepObj =
        SVFUtil::dyn_cast<CloneGepObjVar>(ppag->getGNode(clone));
...
NodeID TypeBasedHeapCloning::addCloneFIObjNode(const MemObj *mem) {
  NodeID id = NodeIDAllocator::get()->allocateObjectId();
  return ppag->addObjNode(mem->getValue(),
                          new CloneFIObjVar(mem->getValue(), id, mem), id);
}
...
void TypeBasedHeapCloning::validateTBHCTests(SVFModule *) {
  ...
  for (u32_t i = 0; i < llvmModuleSet->getModuleNum(); ++i) {
    ...
    for (const CallICFGNode *cbn : callSites) {
      ...
      // We have a test call,
      // We want the store which stores to the pointer in question (i.e. operand
      // of the store is the pointer, and the store itself is the dereference).
      ...
      // Repeat for q.
      while (currInst != nullptr) {
        // while loop, not do-while, because we need to the next instruction
        // (current instruction is the first deref()).
        prevInst = currInst;
        currInst = currInst->getNextNonDebugInstruction();
      ...
      if (fn->getName() == PointerAnalysis::aliasTestMayAlias ||
        ...
        passed =
            res == AliasResult::MayAlias || res == AliasResult::PartialAlias;
        ...
        passed = res != AliasResult::MayAlias &&
                 res != AliasResult::MustAlias &&
                 res != AliasResult::PartialAlias;
