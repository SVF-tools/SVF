...
void AndersenStat::collectCycleInfo(ConstraintGraph *consCG) {
  ...
  for (ConstraintGraph::iterator it = consCG->begin(), eit = consCG->end();
       it != eit; ++it) {
    // sub nodes have been removed from the constraint graph, only rep nodes are
    // left.
...
void AndersenStat::constraintGraphStat() {
  ...
  for (ConstraintEdge::ConstraintEdgeSetTy::iterator
           it = consCG->getDirectCGEdges().begin(),
  ...
  for (ConstraintGraph::ConstraintNodeIDToNodeMapTy::iterator
           nodeIt = consCG->begin(),
           nodeEit = consCG->end();
  ...
  PTNumStatMap["NumOfCGEdge"] = consCG->getLoadCGEdges().size() +
                                consCG->getStoreCGEdges().size() + numOfCopys +
                                numOfGeps;
...
void AndersenStat::statNullPtr() {
  ...
  _NumOfNullPtr = 0;
  for (SVFIR::iterator iter = pta->getPAG()->begin(),
    ...
    if (SVFUtil::isa<ValVar>(pagNode) == false)
      continue;
    SVFStmt::SVFStmtSetTy &inComingStore =
        pagNode->getIncomingEdges(SVFStmt::Store);
    SVFStmt::SVFStmtSetTy &outGoingLoad =
        pagNode->getOutgoingEdges(SVFStmt::Load);
