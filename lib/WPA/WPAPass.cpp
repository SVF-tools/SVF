...
void WPAPass::PrintAliasPairs(PointerAnalysis *pta) {
  ...
  for (SVFIR::iterator lit = pag->begin(), elit = pag->end(); lit != elit;
       ++lit) {
    ...
    for (SVFIR::iterator rit = lit, erit = pag->end(); rit != erit; ++rit) {
      ...
      SVFUtil::outs()
          << (result == AliasResult::NoAlias ? "NoAlias" : "MayAlias") << " var"
          << node1->getId() << "[" << node1->getValueName() << "@"
...
AliasResult WPAPass::alias(const Value *V1, const Value *V2) {
  ...
  ///       between two Values if they both have SVFIR nodes. Otherwise,
  ...
  if (pag->hasValueNode(V1) && pag->hasValueNode(V2)) {
    ...
    if (Options::AliasRule.getBits() == 0 || Options::AliasRule.isSet(Veto)) {
      ...
      for (PTAVector::const_iterator it = ptaVector.begin(),
                                     eit = ptaVector.end();
           it != eit; ++it) {
