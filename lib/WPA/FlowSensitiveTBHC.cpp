...
FlowSensitiveTBHC::FlowSensitiveTBHC(SVFIR *_pag, PTATY type)
  ...
  // doesn't use pta in the constructor so it just needs to be allocated, which
  // it is.
...
void FlowSensitiveTBHC::backPropagate(NodeID clone) {
  ...
  if (SVFUtil::isa<CloneGepObjVar>(cloneObj) ||
      SVFUtil::isa<GepObjVar>(originalObj)) {
  ...
  } else if (SVFUtil::isa<CloneFIObjVar>(cloneObj) ||
             SVFUtil::isa<CloneDummyObjVar>(cloneObj)) {
...
bool FlowSensitiveTBHC::propAlongIndirectEdge(const IndirectSVFGEdge *edge) {
  ...
  // cloning, it uses only the base objects; we want to account for clones too.
  ...
  for (NodeID o : pts) {
    ...
    if (GepObjVar *gep = SVFUtil::dyn_cast<GepObjVar>(pag->getGNode(o))) {
      // Want the geps which are at the same "level" as this one (same mem obj,
      // same offset).
      const NodeBS &geps =
          getGepObjsFromMemObj(gep->getMemObj(), gep->getConstantFieldIdx());
      for (NodeID g : geps) {
...
bool FlowSensitiveTBHC::propAlongDirectEdge(const DirectSVFGEdge *edge) {
  ...
  } else if (FormalRetSVFGNode *fp =
                 SVFUtil::dyn_cast<FormalRetSVFGNode>(src)) {
    ...
    if (TypeBasedHeapCloning::isBlkObjOrConstantObj(oq)) {
      ...
      if (gepStmt->isVariantFieldGep()) {
        ...
        if (DCHGraph::isAgg(baseType) &&
            ...
            ((u32_t)gepStmt->getConstantFieldIdx() >=
             dchg->getNumFields(baseType))) {
          // If the field offset is too high for this object, it is killed. It
          // seems that a clone was made on this GEP but this is not the base
          // (e.g. base->f1->f2), and SVF expects to operate on the base (hence
          // the large offset). The base will have been cloned at another GEP
          // and back-propagated, thus it'll reach here safe and sound. We
          // ignore arrays/pointers because those are array accesses/pointer
          // arithmetic we assume are correct. Obviously, non-aggregates cannot
          // have their fields taken so they are spurious.
          filterSet.set(oq);
        } else {
          // Operate on the field and all its clones.
          const NodeBS fieldClones =
              getGepObjClones(oq, gepStmt->getConstantFieldIdx());
          for (NodeID fc : fieldClones) {
            gepToSVFGRetrievers[getOriginalObj(fc)].set(gep->getId());
            tmpDstPts.set(fc);
          }
...
bool FlowSensitiveTBHC::processLoad(const LoadSVFGNode *load) {
  ...
  // We want to perform the initialisation for non-pointer nodes but not process
  // the load.
...
void FlowSensitiveTBHC::expandFIObjs(const PointsTo &pts,
  ...
  for (NodeID o : pts) {
    ...
    while (const GepObjVar *gepObj =
               SVFUtil::dyn_cast<GepObjVar>(pag->getGNode(o))) {
      expandedPts |= getAllFieldsObjVars(o);
