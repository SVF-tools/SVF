...
void IFDS::printRes(){
    ...
    for (ICFGNodeToDataFactsMap::iterator it = UniniVarSolution.begin(), eit = UniniVarSolution.end(); it != eit; ++it){
        ICFGNode* node = it->first;
...
IFDS::Datafact IFDS::transferFun(PathNode *pathNode) {
    ...
    if (IntraBlockNode *node = SVFUtil::dyn_cast<IntraBlockNode>(icfgNode)) {
        ...
        for (IntraBlockNode::StmtOrPHIVFGNodeVec::const_iterator it = nodes.begin(), eit = nodes.end();
             it != eit; ++it) {
