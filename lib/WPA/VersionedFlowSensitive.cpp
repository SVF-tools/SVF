...
VersionedFlowSensitive::VersionedFlowSensitive(SVFIR *_pag, PTATY type)
  ...
  for (SVFIR::const_iterator it = pag->begin(); it != pag->end(); ++it) {
    if (SVFUtil::isa<ObjVar>(it->second))
      equivalentObject[it->first] = it->first;
  }

  assert(!Options::OPTSVFG &&
...
void VersionedFlowSensitive::prelabel(void) {
  ...
  for (SVFG::iterator it = svfg->begin(); it != svfg->end(); ++it) {
    ...
    if (const StoreSVFGNode *stn = SVFUtil::dyn_cast<StoreSVFGNode>(ln)) {
      ...
      // The outgoing edges are not only what will later be propagated. SVFGOPT
      // may move around nodes such that there can be an MRSVFGNode with no
      // incoming or outgoing edges which will be added at runtime. In essence,
      // we can no longer rely on the outgoing edges of a delta node when
      // SVFGOPT is enabled.
...
void VersionedFlowSensitive::meldLabel(void) {
  ...
  assert(Options::VersioningThreads > 0 &&
         "VFS::meldLabel: number of versioning threads must be > 0!");
  ...
  // solving, unlike other nodes with which we can make do with the reliance
  // map.
  std::vector<NodeID> nodesWhichNeedVersions;
  for (SVFG::const_iterator it = svfg->begin(); it != svfg->end(); ++it) {
    const NodeID n = it->first;
    if (delta(n) || deltaSource(n) || isStore(n) || isLoad(n))
      nodesWhichNeedVersions.push_back(n);
  }
  ...
  auto meldVersionWorker = [this, &footprintOwner, &objectQueue,
                            &objectQueueMutex, &footprintOwnerMutex,
                            &versionMutexes, &prelabeledNodes, &isPrelabeled,
                            &nodesWhichNeedVersions](const unsigned thread) {
    while (true) {
      NodeID o;
      {
      ...
      // For starting nodes, we only need those which did prelabeling for o
      // specifically.
      // TODO: maybe we should move this to prelabel with a map (o -> starting
      // nodes).
      ...
      for (const SVFGNode *sn : prelabeledNodes) {
        if (const StoreSVFGNode *store = SVFUtil::dyn_cast<StoreSVFGNode>(sn)) {
          ...
          if (this->ander->getPts(p).test(o))
            osStartingNodes.push_back(sn);
        } else if (delta(sn->getId())) {
          const MRSVFGNode *mr = SVFUtil::dyn_cast<MRSVFGNode>(sn);
          if (mr != nullptr) {
            if (mr->getPointsTo().test(o))
              osStartingNodes.push_back(sn);
          }
        }
      }
      ...
      unsigned numSCCs = SCC::detectSCCs(this, this->svfg, o, osStartingNodes,
                                         partOf, footprint);

      // 2. Skip any further processing of a footprint we have seen before.
      {
        std::lock_guard<std::mutex> guard(footprintOwnerMutex);
        const Map<std::vector<const IndirectSVFGEdge *>, NodeID>::const_iterator
            canonOwner = footprintOwner.find(footprint);
        if (canonOwner == footprintOwner.end()) {
          ...
          // Same version and stmt reliance as the canonical. During solving we
          // cannot just reuse the canonical object's reliance because it may
          // change due to on-the-fly call graph construction. Something like
          // copy-on-write could be good... probably negligible.
          this->versionReliance.at(o) =
              this->versionReliance.at(canonOwner->second);
      ...
      for (NodeID n = 0; n < partOf.size(); ++n) {
        ...
        if (isPrelabeled[n]) {
          if (this->isStore(n))
            storesYieldedMeldVersion[n].set(bit);
          else
            sccToMeldVersion[partOf[n]].set(bit);
          ++bit;
        }
      ...
      // Maps SCC to the store it corresponds to or -1 if it doesn't. TODO:
      // unsigned vs signed -- nasty.
      ...
      for (size_t i = 0; i < todoList.size(); ++i) {
        ...
        // For stores, that is in storesYieldedMeldVersion, otherwise, consume
        // == yield and we can just use sccToMeldVersion.
        const MeldVersion &nMV =
            nIsStore ? storesYieldedMeldVersion[n] : sccToMeldVersion[nSCC];
        for (const SVFGEdge *e : sn->getOutEdges()) {
          ...
          // No point propagating when n's SCC == m's SCC (same meld version
          // there) except when it is a store, because we are actually
          // propagating n's yielded into m's consumed. Store nodes are in their
          // own SCCs, so it is a self loop on a store node.
      ...
      Map<Version, std::vector<Version>> &osVersionReliance =
          this->versionReliance.at(o);
      for (u32_t scc = 0; scc < numSCCs; ++scc) {
        if (sccReliance[scc].empty())
          continue;

        // Some consume relies on a yield. When it's a store, we need to pick
        // whether to use the consume or yield unlike when it is not because
        // they are the same.
        const Version version = storeSCC[scc] != -1
                                    ? storesYieldedVersion[storeSCC[scc]]
                                    : sccToVersion[scc];
      ...
      // TODO: maybe randomise iteration order for less contention? Needs
      // profiling.
      ...
      for (size_t i = 0; i < nodesWhichNeedVersions.size(); ++i) {
        ...
        if (c != invalidVersion) {
          ...
          if (this->isStore(n) || this->isLoad(n))
            osStmtReliance[c].set(n);
        }

        if (this->isStore(n)) {
          const Map<NodeID, Version>::const_iterator yIt =
              storesYieldedVersion.find(n);
          if (yIt != storesYieldedVersion.end())
            this->setYield(n, o, yIt->second);
        }
      }
    }
  };
  ...
  for (unsigned i = 0; i < Options::VersioningThreads; ++i)
    workers.push_back(std::thread(meldVersionWorker, i));
...
void VersionedFlowSensitive::buildDeltaMaps(void) {
  ...
  for (SVFG::const_iterator it = svfg->begin(); it != svfg->end(); ++it) {
    ...
    if (const CallICFGNode *cbn =
            SVFUtil::dyn_cast<CallICFGNode>(s->getICFGNode())) {
      if (deltaCBNs.find(cbn) != deltaCBNs.end())
        deltaSourceMap[l] = true;
    }

    // TODO: this is an over-approximation but it sound, marking every formal
    // out as
    //       a delta-source.
    if (SVFUtil::isa<FormalOUTSVFGNode>(s))
      deltaSourceMap[l] = true;
  }
}
...
void VersionedFlowSensitive::removeAllIndirectSVFGEdges(void) {
  for (SVFG::iterator nodeIt = svfg->begin(); nodeIt != svfg->end(); ++nodeIt) {
    ...
    for (SVFGEdge *e : inEdges) {
      if (SVFUtil::isa<IndirectSVFGEdge>(e))
        toDeleteFromIn.push_back(e);
    }

    for (SVFGEdge *e : toDeleteFromIn)
      svfg->removeSVFGEdge(e);
...
void VersionedFlowSensitive::propagateVersion(const NodeID o, const Version v,
                                              const Version vp,
                                              bool time /*=true*/) {
  ...
  if (vPtD->unionPts(dstVar, srcVar)) {
    ...
    VarToPropNodeMap::const_iterator dvpIt =
        versionedVarToPropNode.find(dstVar);
    ...
    for (NodeID s : getStmtReliance(o, vp))
      pushIntoWorklist(s);
  }
...
void VersionedFlowSensitive::processNode(NodeID n) {
  ...
  if (const DummyVersionPropSVFGNode *dvp =
          SVFUtil::dyn_cast<DummyVersionPropSVFGNode>(sn)) {
...
void VersionedFlowSensitive::updateConnectedNodes(
  ...
  for (const SVFGEdge *e : newEdges) {
    ...
    if (SVFUtil::isa<PHISVFGNode>(dstNode) ||
      ...
      assert(deltaSource(src) && "VFS::updateConnectedNodes: new indirect "
                                 "edges should be from delta source nodes!");

      const NodeBS &ept = ie->getPointsTo();
      // For every o, such that src --o--> dst, we need to set up reliance (and
      // propagate).
      for (const NodeID o : ept) {
        Version srcY = getYield(src, o);
        if (srcY == invalidVersion)
          continue;
        Version dstC = getConsume(dst, o);
        if (dstC == invalidVersion)
          continue;

        std::vector<Version> &versionsRelyingOnSrcY =
            getReliantVersions(o, srcY);
        if (std::find(versionsRelyingOnSrcY.begin(),
                      versionsRelyingOnSrcY.end(),
                      dstC) == versionsRelyingOnSrcY.end()) {
...
bool VersionedFlowSensitive::processStore(const StoreSVFGNode *store) {
  ...
  if (!changedObjects.empty()) {
    for (const NodeID o : changedObjects) {
      ...
      for (NodeID s : getStmtReliance(o, y))
        pushIntoWorklist(s);
    }
  }
...
void VersionedFlowSensitive::cluster(void) {
  ...
  for (SVFIR::iterator pit = pag->begin(); pit != pag->end(); ++pit) {
    ...
    if (Options::PredictPtOcc && pag->getObject(v) != nullptr)
      occ = stmtReliance[v].size() + 1;
    assert(occ != 0);
    keys.push_back(std::make_pair(v, occ));
  }

  PointsTo::MappingPtr nodeMapping =
      std::make_shared<std::vector<NodeID>>(NodeIDAllocator::Clusterer::cluster(
...
Version VersionedFlowSensitive::getVersion(const NodeID l, const NodeID o,
                                           const LocVersionMap &lvm) const {
  const Map<NodeID, NodeID>::const_iterator canonObjectIt =
      equivalentObject.find(o);
  const NodeID op =
      canonObjectIt == equivalentObject.end() ? o : canonObjectIt->second;
...
Version VersionedFlowSensitive::getYield(const NodeID l, const NodeID o) const {
  ...
  if (isStore(l))
    return getVersion(l, o, yield);
  else
    return getVersion(l, o, consume);
}

void VersionedFlowSensitive::setVersion(const NodeID l, const NodeID o,
                                        const Version v, LocVersionMap &lvm) {
  ObjToVersionMap &ovm = lvm[l];
  ovm[o] = v;
}

void VersionedFlowSensitive::setConsume(const NodeID l, const NodeID o,
                                        const Version v) {
  setVersion(l, o, v, consume);
}

void VersionedFlowSensitive::setYield(const NodeID l, const NodeID o,
                                      const Version v) {
  // Non-store: consume == yield.
  if (isStore(l))
    setVersion(l, o, v, yield);
  else
    setVersion(l, o, v, consume);
}

std::vector<Version> &
VersionedFlowSensitive::getReliantVersions(const NodeID o, const Version v) {
...
void VersionedFlowSensitive::dumpReliances(void) const {
  ...
  for (const Map<NodeID, Map<Version, std::vector<Version>>>::value_type &ovrv :
       versionReliance) {
    NodeID o = ovrv.first;
    SVFUtil::outs() << "  Object " << o << "\n";
    for (const Map<Version, std::vector<Version>>::value_type &vrv :
         ovrv.second) {
      Version v = vrv.first;
  ...
  for (const Map<NodeID, Map<Version, NodeBS>>::value_type &ovss :
       stmtReliance) {
    NodeID o = ovss.first;
    SVFUtil::outs() << "  Object " << o << "\n";
...
unsigned VersionedFlowSensitive::SCC::detectSCCs(
    VersionedFlowSensitive *vfs, const SVFG *svfg, const NodeID object,
  ...
  for (const SVFGNode *v : startingNodes) {
    if (nodeData[v->getId()].index == -1) {
      visit(vfs, object, partOf, footprint, nodeData, stack, index, currentSCC,
            v);
    }
  }
...
void VersionedFlowSensitive::SCC::visit(
    VersionedFlowSensitive *vfs, const NodeID object, std::vector<int> &partOf,
    std::vector<const IndirectSVFGEdge *> &footprint,
    std::vector<NodeData> &nodeData, std::stack<const SVFGNode *> &stack,
    int &index, int &currentSCC, const SVFGNode *v) {
  ...
  for (const SVFGEdge *e : v->getOutEdges()) {
    ...
    if (!ie->getPointsTo().test(object))
      continue;

    // Even if we don't count edges to stores and deltas for SCCs' sake, they
    // are relevant to the footprint as a propagation still occurs over such
    // edges.
    footprint.push_back(ie);

    // Ignore edges to delta nodes because they are prelabeled so cannot
    // be part of the SCC v is in (already in nodesTodo from the prelabeled
    // set). Similarly, store nodes.
    if (vfs->delta(wId) || vfs->isStore(wId))
      continue;

    if (nodeData[wId].index == -1) {
      visit(vfs, object, partOf, footprint, nodeData, stack, index, currentSCC,
            w);
      nodeData[vId].lowlink =
          std::min(nodeData[vId].lowlink, nodeData[wId].lowlink);
    } else if (nodeData[wId].onStack) {
      nodeData[vId].lowlink =
          std::min(nodeData[vId].lowlink, nodeData[wId].index);
    }
  }
