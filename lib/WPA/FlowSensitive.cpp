...
void FlowSensitive::initialize() {
  ...
  assert(!Options::ClusterAnder && "FlowSensitive::initialize: clustering "
                                   "auxiliary Andersen's unsupported.");
  ...
  svfg = Options::CTirAliasEval ? memSSA.buildFullSVFG(ander)
...
void FlowSensitive::finalize() {
  ...
  if (Options::ClusterFs) {
    ...
    NodeIDAllocator::Clusterer::evaluate(*PointsTo::getCurrentBestNodeMapping(),
                                         allPts, stats, true);
    NodeIDAllocator::Clusterer::printStats("post-main: best", stats);

    // Do the same for the candidates. TODO: probably temporary for eval.
    // purposes.
    for (std::pair<hclust_fast_methods, std::vector<NodeID>> &candidate :
         candidateMappings) {
      // Can reuse stats, since we're always filling it with `evaluate`, it will
      // always be overwritten.
      NodeIDAllocator::Clusterer::evaluate(candidate.second, allPts, stats,
                                           true);
      NodeIDAllocator::Clusterer::printStats(
          "post-main: candidate " +
              SVFUtil::hclustMethodToString(candidate.first),
          stats);
    }
  }
...
bool FlowSensitive::propFromSrcToDst(SVFGEdge *edge) {
  ...
  else if (IndirectSVFGEdge *indEdge =
               SVFUtil::dyn_cast<IndirectSVFGEdge>(edge))
...
bool FlowSensitive::propAlongIndirectEdge(const IndirectSVFGEdge *edge) {
  ...
  for (NodeBS::iterator ptdIt = pts.begin(), ptdEit = pts.end();
       ptdIt != ptdEit; ++ptdIt) {
    NodeID ptd = *ptdIt;
...
bool FlowSensitive::processPhi(const PHISVFGNode *phi) {
  ...
  for (PHISVFGNode::OPVers::const_iterator it = phi->opVerBegin(),
                                           eit = phi->opVerEnd();
       it != eit; ++it) {
...
bool FlowSensitive::processLoad(const LoadSVFGNode *load) {
  ...
  for (PointsTo::iterator ptdIt = srcPts.begin(); ptdIt != srcPts.end();
       ++ptdIt) {
    NodeID ptd = *ptdIt;
    ...
    if (isFieldInsensitive(ptd)) {
      /// If the ptd is a field-insensitive node, we should also get all field
      /// nodes' points-to sets and pass them to pagDst.
      const NodeBS &allFields = getAllFieldsObjVars(ptd);
      for (NodeBS::iterator fieldIt = allFields.begin(),
                            fieldEit = allFields.end();
           fieldIt != fieldEit; ++fieldIt) {
...
bool FlowSensitive::processStore(const StoreSVFGNode *store) {
  ...
  if (getPts(store->getPAGSrcNodeID()).empty() == false) {
    for (PointsTo::iterator it = dstPts.begin(), eit = dstPts.end(); it != eit;
         ++it) {
      NodeID ptd = *it;
...
bool FlowSensitive::updateCallGraph(const CallSiteToFunPtrMap &callsites) {
  ...
  for (typename CallEdgeMap::value_type &csfs : newEdges) {
    ...
    for (FunctionSet::iterator potentialFunctionIt =
             potentialFunctionSet.begin();
         potentialFunctionIt != potentialFunctionSet.end();) {
      const SVFFunction *potentialFunction = *potentialFunctionIt;
      if (andersFunctionSet.find(potentialFunction) ==
          andersFunctionSet.end()) {
...
void FlowSensitive::updateConnectedNodes(const SVFGEdgeSetTy &edges) {
  for (SVFGEdgeSetTy::const_iterator it = edges.begin(), eit = edges.end();
    ...
    if (SVFUtil::isa<PHISVFGNode>(dstNode)) {
      /// If this is a formal-param or actual-ret node, we need to solve this
      /// phi node in next iteration
...
void FlowSensitive::plainMap(void) const {
  ...
  PointsTo::MappingPtr plainMapping =
      std::make_shared<std::vector<NodeID>>(numObjects);
  PointsTo::MappingPtr reversePlainMapping =
      std::make_shared<std::vector<NodeID>>(numObjects);
...
void FlowSensitive::printCTirAliasStats(void) {
  ...
  SVFUtil::outs() << "eval-ctir-aliases " << total << " " << mayAliases << " "
                  << noAliases << " "
                  << "\n";
  SVFUtil::outs() << "  "
                  << "TOTAL : " << total << "\n"
