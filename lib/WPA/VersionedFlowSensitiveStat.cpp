...
void VersionedFlowSensitiveStat::performStat() {
  ...
  for (SVFG::iterator it = vfspta->svfg->begin(); it != vfspta->svfg->end();
       ++it) {
    numOfNode++;
...
void VersionedFlowSensitiveStat::versionStat(void) {
  ...
  for (const VersionedFlowSensitive::LocVersionMap *lvm :
    ...
    for (const VersionedFlowSensitive::ObjToVersionMap &lov : *lvm) {
      for (const VersionedFlowSensitive::ObjToVersionMap::value_type &ov :
           lov) {
  ...
  if (_NumNonEmptyVersions != 0)
    _AvgVersionPtsSize = (double)totalVersionPtsSize / (double)_NumUsedVersions;
...
void VersionedFlowSensitiveStat::ptsSizeStat() {
  ...
  for (SVFIR::iterator it = vfspta->getPAG()->begin();
       it != vfspta->getPAG()->end(); ++it) {
