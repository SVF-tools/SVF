...
void AndersenLCD::handleCopyGep(ConstraintNode *node) {
  ...
  for (ConstraintEdge *edge : node->getCopyOutEdges()) {
    ...
    // In one edge, if the pts of src node equals to that of dst node, and the
    // edge is never met, push it into 'metEdges' and push the dst node into
    // 'lcdCandidates'
...
NodeStack &AndersenLCD::SCCDetect() {
  ...
  sccCandidates.clear();
  for (NodeSet::iterator it = lcdCandidates.begin(); it != lcdCandidates.end();
       ++it)
    if (sccRepNode(*it) == *it)
