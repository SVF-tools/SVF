...
PointerAnalysis::PointerAnalysis(SVFIR *p, PTATY ty, bool alias_check)
    : svfMod(nullptr), ptaTy(ty), stat(nullptr), ptaCallGraph(nullptr),
      callGraphSCC(nullptr), icfg(nullptr), chgraph(nullptr),
      typeSystem(nullptr) {
  pag = p;
  ...
  if (obj->isStack()) {
    if (const Function *fun = pag->getGNode(id)->getFunction()) {
      ...
      return callGraphSCC->isInCycle(
          getPTACallGraph()->getCallGraphNode(svffun)->getId());
    }
  }
  return false;
}
...
void PointerAnalysis::resolveIndCalls(const CallICFGNode *cs,
  ...
  for (PointsTo::iterator ii = target.begin(), ie = target.end(); ii != ie;
       ii++) {

    if (getNumOfResolvedIndCallEdge() >= Options::IndirectCallLimit) {
      wrnMsg("Resolved Indirect Call Edges are Out-Of-Budget, please increase "
             "the limit");
      return;
    }
    ...
    if (ObjVar *objPN = SVFUtil::dyn_cast<ObjVar>(pag->getGNode(*ii))) {
      ...
      if (obj->isFunction()) {
        ...
        /// if the arg size does not match then we do not need to connect this
        /// parameter even if the callee is a variadic function (the first
        /// parameter of variadic function is its paramter number)
        ...
        if (0 == getIndCallMap()[cs].count(callee)) {
          ...
          // The indirect call is maintained by ourself, We may update llvm's
          // when we need to
 ...
 */
bool PointerAnalysis::matchArgs(const CallICFGNode *cs,
...
void PointerAnalysis::getVFnsFromPts(const CallICFGNode *cs,
  ...
  if (chgraph->csHasVtblsBasedonCHA(
    ...
    const VTableSet &chaVtbls = chgraph->getCSVtblsBasedonCHA(
        SVFUtil::getLLVMCallSite(cs->getCallSite()));
    for (PointsTo::iterator it = target.begin(), eit = target.end(); it != eit;
         ++it) {
      const PAGNode *ptdnode = pag->getGNode(*it);
      if (ptdnode->hasValue()) {
        if (const GlobalValue *vtbl =
...
void PointerAnalysis::connectVCallToVFns(const CallICFGNode *cs,
  ...
  for (VFunSet::const_iterator fit = vfns.begin(), feit = vfns.end();
       fit != feit; ++fit) {
    const SVFFunction *callee = *fit;
    ...
    if (SVFUtil::getLLVMCallSite(cs->getCallSite()).arg_size() ==
      ...
      ptaCallGraph->addIndirectCallGraphEdge(callBlockNode, cs->getCaller(),
                                             callee);
    }
  }
}
...
void PointerAnalysis::validateSuccessTests(std::string fun) {
  ...
  // check for must alias cases, whether our alias analysis produce the correct
  // results
  if (const SVFFunction *checkFun = getFunction(fun)) {
    ...
    for (Value::user_iterator i = checkFun->getLLVMFun()->user_begin(),
      ...
      if (SVFUtil::isCallSite(*i)) {
        ...
        if (checkSuccessful)
          outs() << sucMsg("\t SUCCESS :") << fun << " check <id:" << id1
                 << ", id:" << id2 << "> at (" << getSourceLoc(*i) << ")\n";
        else {
