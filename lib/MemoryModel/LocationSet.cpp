...
/// totalConstOffset = flattenOffset(value1,type1) * flattenOffset(type2,type2)
/// + flattenOffset(type3,type3) For a pointer type (e.g., t1 is PointerType),
/// we will retrieve the pointee type and times the offset, i.e.,
/// getElementNum(t1) X off1
...
/// %6 = getelementptr inbounds %struct.Student, %struct.Student* %5, i32 0, i32
/// 1
///     value1: i32 0  type1: %struct.Student*
///     value2: i32 1  type2: %struct.Student = type { %struct.inner, [10 x [3 x
///     i8]] } accumulateConstantOffset = 2
/// %7 = getelementptr inbounds [10 x [3 x i8]], [10 x [3 x i8]]* %6, i64 0, i64
/// 3
...
s32_t LocationSet::accumulateConstantOffset() const {
  ...
  for (int i = offsetValues.size() - 1; i >= 0; i--) {
    ...
    if (const PointerType *pty = SVFUtil::dyn_cast<PointerType>(type))
      totalConstOffset +=
          op->getSExtValue() * getElementNum(pty->getElementType());
    else {
      s32_t offset = op->getSExtValue();
      u32_t flattenOffset =
          SymbolTableInfo::SymbolInfo()->getFlattenedElemIdx(type, offset);
...
SVF::LocationSet::LSRelation
LocationSet::checkRelation(const LocationSet &LHS, const LocationSet &RHS) {
