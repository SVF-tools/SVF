...
ObjTypeInfo::ObjTypeInfo(const Type *t, u32_t max)
    : type(t), flags(0), maxOffsetLimit(max), elemNum(max) {
  assert(t && "no type information for this object?");
}

void ObjTypeInfo::resetTypeForHeapStaticObj(const Type *t) {
  assert((isStaticObj() || isHeap()) &&
         "can only reset the inferred type for heap and static objects!");
  type = t;
}
...
SymbolTableInfo::TypeToFieldInfoMap::iterator
SymbolTableInfo::getStructInfoIter(const Type *T) {
  assert(T);
...
void SymbolTableInfo::collectArrayInfo(const ArrayType *ty) {
  ...
  /// array without any element (this is not true in C/C++ arrays) we assume
  /// there is an empty dummy element
  if (totalElemNum == 0) {
  ...
  /// Flatten arrays, map each array element index `i` to flattened index `(i *
  /// nfE * totalElemNum)/outArrayElemNum` nfE>1 if the array element is a
  ...
  for (u32_t i = 0; i < outArrayElemNum; i++)
    stinfo->addFldWithType(0, elemTy,
                           (i * nfE * totalElemNum) / outArrayElemNum);
    ...
    if (SVFUtil::isa<StructType>(et) || SVFUtil::isa<ArrayType>(et)) {
      ...
      for (u32_t tpi = 0; tpi < subStinfo->getStride(); tpi++) {
        for (u32_t tpj = 0; tpj < nfE; tpj++) {
          stinfo->getFlattenElementTypes().push_back(
              subStinfo->getFlattenFieldTypes()[tpj]);
        }
      }
    } else // simple type
    {
...
bool SymbolTableInfo::isConstantObjSym(const Value *val) {
  if (const GlobalVariable *v = SVFUtil::dyn_cast<GlobalVariable>(val)) {
    if (cppUtil::isValVtbl(v))
      ...
      for (std::vector<const Type *>::const_iterator it = fields.begin(),
                                                     eit = fields.end();
           it != eit; ++it) {
...
MemObj *SymbolTableInfo::createBlkObj(SymID symId) {
  ...
  MemObj *obj = new MemObj(
      symId, createObjTypeInfo(IntegerType::get(
                 LLVMModuleSet::getLLVMModuleSet()->getContext(), 32)));
  objMap[symId] = obj;
  return obj;
}
...
/// Flatterned offset information of a struct or an array including its array
/// fields
u32_t SymbolTableInfo::getFlattenedElemIdx(const Type *T, u32_t origId) {
  if (Options::ModelArrays) {
    std::vector<u32_t> &so =
        getStructInfoIter(T)->second->getFlattenedElemIdxVec();
    assert((unsigned)origId <= so.size() && !so.empty() &&
           "element index out of bounds, can't get flattened index!");
    return so[origId];
  } else {
    if (SVFUtil::isa<StructType>(T)) {
      std::vector<u32_t> &so =
          getStructInfoIter(T)->second->getFlattenedFieldIdxVec();
      assert((unsigned)origId <= so.size() && !so.empty() &&
             "Struct index out of bounds, can't get flattened index!");
      return so[origId];
    } else {
      /// When Options::ModelArrays is disabled, any element index Array is
      /// modeled as the base
      assert(SVFUtil::isa<ArrayType>(T) &&
             "Only accept struct or array type if Options::ModelArrays is "
             "disabled!");
      return 0;
    }
  }
...
const Type *SymbolTableInfo::getOriginalElemType(const Type *baseType,
                                                 u32_t origId) {
  return getStructInfoIter(baseType)->second->getOriginalElemType(origId);
}

/// Return the type of a flattened element given a flattened index
const Type *SymbolTableInfo::getFlatternedElemType(const Type *baseType,
                                                   u32_t flatten_idx) {
  if (Options::ModelArrays) {
    const std::vector<const Type *> &so =
        getStructInfoIter(baseType)->second->getFlattenElementTypes();
    assert(flatten_idx <= so.size() && !so.empty() &&
           "element index out of bounds, can't get element type!");
    return so[flatten_idx];
  } else {
    const std::vector<const Type *> &so =
        getStructInfoIter(baseType)->second->getFlattenFieldTypes();
    assert(flatten_idx <= so.size() && !so.empty() &&
           "element index out of bounds, can't get element type!");
    return so[flatten_idx];
  }
...
SymbolTableInfo::getFlattenFieldTypes(const StructType *T) {
...
void SymbolTableInfo::printFlattenFields(const Type *type) {
  ...
  if (const ArrayType *at = SVFUtil::dyn_cast<ArrayType>(type)) {
    ...
    std::vector<const Type *> &finfo =
        getStructInfo(st)->getFlattenFieldTypes();
    int field_idx = 0;
    for (std::vector<const Type *>::const_iterator it = finfo.begin(),
                                                   eit = finfo.end();
         it != eit; ++it, field_idx++) {
...
bool ObjTypeInfo::isNonPtrFieldObj(const LocationSet &ls) {
  ...
  if (SVFUtil::isa<StructType>(ety) || SVFUtil::isa<ArrayType>(ety)) {
    const Type *elemTy = SymbolTableInfo::SymbolInfo()->getFlatternedElemType(
        ety, ls.accumulateConstantFieldIdx());
...
u32_t MemObj::getNumOfElements() const { return typeInfo->getNumOfElements(); }
...
/// Get obj type info
const Type *MemObj::getType() const { return typeInfo->getType(); }
/*
...
void MemObj::setFieldInsensitive() { typeInfo->setMaxFieldOffsetLimit(0); }

bool MemObj::isFunction() const { return typeInfo->isFunction(); }

bool MemObj::isGlobalObj() const { return typeInfo->isGlobalObj(); }

bool MemObj::isStaticObj() const { return typeInfo->isStaticObj(); }

bool MemObj::isStack() const { return typeInfo->isStack(); }

bool MemObj::isHeap() const { return typeInfo->isHeap(); }

bool MemObj::isStruct() const { return typeInfo->isStruct(); }

bool MemObj::isArray() const { return typeInfo->isArray(); }

bool MemObj::isVarStruct() const { return typeInfo->isVarStruct(); }

bool MemObj::isVarArray() const { return typeInfo->isVarArray(); }

bool MemObj::isConstantStruct() const { return typeInfo->isConstantStruct(); }

bool MemObj::isConstantArray() const { return typeInfo->isConstantArray(); }

bool MemObj::isConstDataOrConstGlobal() const {
  return typeInfo->isConstDataOrConstGlobal();
}

bool MemObj::isConstantData() const { return typeInfo->isConstantData(); }

bool MemObj::hasPtrObj() const { return typeInfo->hasPtrObj(); }
