 ...
 * Constructor
 */
BVDataPTAImpl::BVDataPTAImpl(SVFIR *p, PointerAnalysis::PTATY type,
                             bool alias_check)
  ...
  if (type == Andersen_BASE || type == Andersen_WPA ||
      ...
      type == AndersenSFR_WPA) {
    // Only maintain reverse points-to when the analysis is field-sensitive, as
    // objects turning field-insensitive is all it is used for.
    bool maintainRevPts = Options::MaxFieldLimit != 0;
    if (Options::ptDataBacking == PTBackingType::Mutable)
      ptD = new MutDiffPTDataTy(maintainRevPts);
    else if (Options::ptDataBacking == PTBackingType::Persistent)
      ptD = new PersDiffPTDataTy(getPtCache(), maintainRevPts);
    else
      assert(
          false &&
          "BVDataPTAImpl::BVDataPTAImpl: unexpected points-to backing type!");
  } else if (type == Steensgaard_WPA) {
    // Steensgaard is only field-insensitive (for now?), so no reverse
    // points-to.
    if (Options::ptDataBacking == PTBackingType::Mutable)
      ptD = new MutDiffPTDataTy(false);
    else if (Options::ptDataBacking == PTBackingType::Persistent)
      ptD = new PersDiffPTDataTy(getPtCache(), false);
    else
      assert(
          false &&
          "BVDataPTAImpl::BVDataPTAImpl: unexpected points-to backing type!");
    ...
    if (Options::INCDFPTData) {
      ...
      if (Options::ptDataBacking == PTBackingType::Mutable)
        ...
        ptD = new PersDFPTDataTy(getPtCache(), false);
      else
        assert(
            false &&
            "BVDataPTAImpl::BVDataPTAImpl: unexpected points-to backing type!");
    ...
    if (Options::ptDataBacking == PTBackingType::Mutable)
      ...
      ptD = new PersVersionedPTDataTy(getPtCache(), false);
    else
      assert(
          false &&
          "BVDataPTAImpl::BVDataPTAImpl: unexpected points-to backing type!");
...
void BVDataPTAImpl::finalize() {
  ...
  if (Options::ptDataBacking == PTBackingType::Persistent && Options::PStat)
    ptCache.printStats("bv-finalize");
}
...
void BVDataPTAImpl::remapPointsToSets(void) { getPTDataTy()->remapAllPts(); }
...
/*!
 * Dump all points-to including top-level (ValVar) and address-taken (ObjVar)
 * variables
 */
