...
void SVFVar::dump() const { outs() << this->toString() << "\n"; }
...
const std::string GepValVar::toString() const {
  std::string str;
  raw_string_ostream rawstr(str);
  rawstr << "GepValVar ID: " << getId()
         << " with offset_" + std::to_string(getConstantFieldIdx());
  if (Options::ShowSVFIRValue) {
    rawstr << "\n";
    rawstr << value2String(value);
  }
...
const std::string GepObjVar::toString() const {
  std::string str;
  raw_string_ostream rawstr(str);
  rawstr << "GepObjVar ID: " << getId()
         << " with offset_" + std::to_string(ls.accumulateConstantFieldIdx());
  if (Options::ShowSVFIRValue) {
    rawstr << "\n";
    rawstr << value2String(value);
  }
