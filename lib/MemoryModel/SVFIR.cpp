 ...
 * Add Phi statement
 */
SelectStmt *SVFIR::addSelectStmt(NodeID res, NodeID op1, NodeID op2,
                                 NodeID cond) {
 ...
 * Add Compare edge
 */
CmpStmt *SVFIR::addCmpStmt(NodeID op1, NodeID op2, NodeID dst,
                           u32_t predicate) {
  SVFVar *op1Node = getGNode(op1);
  SVFVar *op2Node = getGNode(op2);
  SVFVar *dstNode = getGNode(dst);
 ...
 * Add Compare edge
 */
BinaryOPStmt *SVFIR::addBinaryOPStmt(NodeID op1, NodeID op2, NodeID dst,
                                     u32_t opcode) {
  SVFVar *op1Node = getGNode(op1);
  SVFVar *op2Node = getGNode(op2);
  SVFVar *dstNode = getGNode(dst);
 ...
 */
StoreStmt *SVFIR::addStoreStmt(NodeID src, NodeID dst,
                               const IntraICFGNode *curVal) {
  SVFVar *srcNode = getGNode(src);
  SVFVar *dstNode = getGNode(dst);
 ...
 */
GepStmt *SVFIR::addGepStmt(NodeID src, NodeID dst, const LocationSet &ls,
                           bool constGep) {
 ...
 * Add normal (Gep) edge
 */
GepStmt *SVFIR::addNormalGepStmt(NodeID src, NodeID dst,
 ...
 * Find the base node id of src and connect base node to dst node
 */
GepStmt *SVFIR::addVariantGepStmt(NodeID src, NodeID dst,
                                  const LocationSet &ls) {
  const LocationSet &baseLS = getLocationSetFromBaseNode(src);
  SVFVar *baseNode = getGNode(getBaseValVar(src));
  SVFVar *dstNode = getGNode(dst);
 ...
 */
NodeID SVFIR::addGepValNode(const Value *curInst, const Value *gepVal,
...
NodeID SVFIR::getGepObjVar(const MemObj *obj, const LocationSet &ls) {
  ...
  if (Options::FirstFieldEqBase && newLS.accumulateConstantFieldIdx() == 0)
    return base;
...
NodeID SVFIR::addGepObjNode(const MemObj *obj, const LocationSet &ls) {
  ...
  NodeID gepId = NodeIDAllocator::get()->allocateGepObjectId(
      base, ls.accumulateConstantFieldIdx(), Options::MaxFieldLimit);
 ...
 */
NodeID SVFIR::getGepValVar(const Value *curInst, NodeID base,
                           const LocationSet &ls) const {
  GepValueVarMap::const_iterator iter = GepValObjMap.find(curInst);
  if (iter == GepValObjMap.end()) {
    return UINT_MAX;
  } else {
    NodeLocationSetMap::const_iterator lit =
        iter->second.find(std::make_pair(base, ls));
...
void SVFIR::print() {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = rets.begin(), eiter = rets.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = tdfks.begin(),
                                       eiter = tdfks.end();
       iter != eiter; ++iter) {
  ...
  for (SVFStmt::SVFStmtSetTy::iterator iter = tdjns.begin(),
                                       eiter = tdjns.end();
       iter != eiter; ++iter) {
...
bool SVFIR::isNonPointerObj(NodeID id) const {
  ...
  } else if (const DummyObjVar *dummyNode =
                 SVFUtil::dyn_cast<DummyObjVar>(node)) {
 ...
 */
void SVFIR::handleBlackHole(bool b) { Options::HandBlackHole = b; }
