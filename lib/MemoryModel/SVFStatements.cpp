...
SVFStmt::SVFStmt(SVFVar *s, SVFVar *d, GEdgeFlag k, bool real)
    : GenericPAGEdgeTy(s, d, k), value(nullptr), basicBlock(nullptr),
      icfgNode(nullptr), edgeId(UINT_MAX) {
  if (real) {
...
const std::string PhiStmt::toString() const {
  ...
  for (u32_t i = 0; i < getOpVarNum(); i++)
    rawstr << "[Var" << getOpVar(i)->getId() << ", ICFGNode"
           << getOpICFGNode(i)->getId() << "],";
  rawstr << ")]\t";
  if (Options::ShowSVFIRValue) {
    rawstr << "\n";
    rawstr << value2String(getValue());
...
const std::string SelectStmt::toString() const {
  std::string str;
  raw_string_ostream rawstr(str);
  rawstr << "SelectStmt: (Condition Var" << getCondition()->getId() << ") [Var"
         << getResID() << " <-- (Var";
...
const std::string CmpStmt::toString() const {
  std::string str;
  raw_string_ostream rawstr(str);
  rawstr << "CmpStmt: [Var" << getResID() << " <-- (Var" << getOpVarID(0)
         << " predicate" << getPredicate() << " Var" << getOpVarID(1) << ")]\t";
  if (Options::ShowSVFIRValue) {
    rawstr << "\n";
    rawstr << value2String(getValue());
  }
...
const std::string BinaryOPStmt::toString() const {
  std::string str;
  raw_string_ostream rawstr(str);
  rawstr << "BinaryOPStmt: [Var" << getResID() << " <-- (Var" << getOpVarID(0)
         << " opcode" << getOpcode() << " Var" << getOpVarID(1) << ")]\t";
  if (Options::ShowSVFIRValue) {
    rawstr << "\n";
    rawstr << value2String(getValue());
  }
...
NodeID MultiOpndStmt::getResID() const { return getRes()->getId(); }

NodeID UnaryOPStmt::getOpVarID() const { return getOpVar()->getId(); }
NodeID UnaryOPStmt::getResID() const { return getRes()->getId(); }

/// Return true if this is a phi at the function exit
/// to receive one or multiple return values of this function
bool PhiStmt::isFunctionRetPhi() const { return SVFUtil::isa<RetPN>(getRes()); }
