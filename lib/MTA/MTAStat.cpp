...
void MTAStat::performThreadCallGraphStat(ThreadCallGraph *tcg) {
  ...
  for (ThreadCallGraph::CallSiteSet::const_iterator it = tcg->forksitesBegin(),
    ...
    const Function *spawnee = SVFUtil::dyn_cast<Function>(
        tcg->getThreadAPI()->getForkedFun((*it)->getCallSite()));
    if (spawnee == nullptr) {
      numOfIndForksite++;
      indirectfork = true;
    }
    for (ThreadCallGraph::ForkEdgeSet::const_iterator
             cgIt = tcg->getForkEdgeBegin(*it),
  ...
  for (ThreadCallGraph::CallSiteSet::const_iterator it = tcg->joinsitesBegin(),
       ...
       it != eit; ++it) {
    for (ThreadCallGraph::JoinEdgeSet::const_iterator
             cgIt = tcg->getJoinEdgeBegin(*it),
  ...
  if (Options::AllPairMHP) {
    ...
    for (SVFModule::iterator F = mod->begin(), E = mod->end(); F != E; ++F) {
      ...
      if (!mhp->isConnectedfromMain(llvmfun))
        continue;
      for (const_inst_iterator II = inst_begin(llvmfun), E = inst_end(llvmfun);
           II != E; ++II) {
    ...
    for (InstSet::const_iterator it1 = instSet1.begin(), eit1 = instSet1.end();
         it1 != eit1; ++it1) {
      for (InstSet::const_iterator it2 = instSet2.begin(),
                                   eit2 = instSet2.end();
           it2 != eit2; ++it2) {
