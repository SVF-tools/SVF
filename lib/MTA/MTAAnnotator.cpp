...
void MTAAnnotator::collectLoadStoreInst(SVFModule *mod) {
  ...
  for (SVFModule::iterator F = mod->begin(), E = mod->end(); F != E; ++F) {
    if (SVFUtil::isExtCall(*F))
      continue;
    for (inst_iterator II = inst_begin((*F)->getLLVMFun()),
                       E = inst_end((*F)->getLLVMFun());
         II != E; ++II) {
...
void MTAAnnotator::pruneThreadLocal(PointerAnalysis *pta) {
  ...
  const SVFStmt::SVFStmtSetTy &forkedges =
      pag->getPTASVFStmtSet(SVFStmt::ThreadFork);
  for (SVFStmt::SVFStmtSetTy::const_iterator it = forkedges.begin(),
                                             eit = forkedges.end();
       it != eit; ++it) {
    PAGEdge *edge = *it;
  ...
  for (SVFIR::SVFStmtSet::const_iterator it = globaledges.begin(),
                                         eit = globaledges.end();
       it != eit; ++it) {
  ...
  while (!worklist.empty()) {
    ...
    for (NodeBS::iterator pit = fields.begin(), epit = fields.end();
         pit != epit; ++pit) {
      if (!nonlocalobjs.test(*pit))
        worklist.set(*pit);
    }
  ...
  InstSet needannost;
  InstSet needannold;
  for (InstSet::iterator it = storeset.begin(), eit = storeset.end(); it != eit;
       ++it) {
  ...
  for (InstSet::iterator it = loadset.begin(), eit = loadset.end(); it != eit;
       ++it) {
...
void MTAAnnotator::pruneAliasMHP(PointerAnalysis *pta) {
  ...
  InstSet needannost;
  InstSet needannold;
  for (InstSet::iterator it1 = storeset.begin(), eit1 = storeset.end();
       it1 != eit1; ++it1) {
    for (InstSet::iterator it2 = it1, eit2 = storeset.end(); it2 != eit2;
         ++it2) {
      ...
      if (AnnoMHP) {
        ...
        /// if it1 == it2, mhp analysis will annotate it1 that locates in loop
        /// or recursion. but alias analysis fails to determine whether it1 is
        /// in loop or recursion, that means all store instructions will be
        /// annotated by alias analysis to guarantee sound.
        needannost.insert(*it1);
        needannost.insert(*it2);
      }
    }
    for (InstSet::iterator it2 = loadset.begin(), eit2 = loadset.end();
         it2 != eit2; ++it2) {
