  ...
  for (SVFModule::iterator F = module->begin(), E = module->end(); F != E;
    ...
    if (!tct->isCandidateFun(fun->getLLVMFun()) && !isExtCall(fun)) {
      ...
      for (CxtThreadStmtSet::const_iterator it1 = tsSet.begin(),
        ...
        for (inst_iterator II = inst_begin(fun->getLLVMFun()),
                           EE = inst_end(fun->getLLVMFun());
             II != EE; ++II) {
  ...
  for (PTACallGraphNode::const_iterator nit = node->OutEdgeBegin(),
    ...
    if (!isExtCall(callee)) {
      CxtThreadStmt newCts(cts.getTid(), curCxt,
                           &(callee->getLLVMFun()->getEntryBlock().front()));
...
void MHP::handleJoin(const CxtThreadStmt &cts, NodeID rootTid) {
  ...
  if (!joinedTids.empty()) {
    if (const Loop *joinLoop = fja->getJoinLoop(call)) {
      ...
      DBOUT(DMTA, outs() << "\n\t match join site "
                         << SVFUtil::value2String(call) << " for thread "
                         << rootTid << "\n");
    }
  }
...
void MHP::handleRet(const CxtThreadStmt &cts) {
  ...
  PTACallGraphNode *curFunNode = tcg->getCallGraphNode(
      tct->getSVFFun(cts.getStmt()->getParent()->getParent()));
  for (PTACallGraphNode::const_iterator it = curFunNode->getInEdges().begin(),
    ...
    for (PTACallGraphEdge::CallInstSet::const_iterator
      ...
      if (matchCxt(newCxt, (*cit)->getCallSite(),
                   curFunNode->getFunction()->getLLVMFun())) {
        InstVec nextInsts;
...
void MHP::handleIntra(const CxtThreadStmt &cts) {
  ...
  for (InstVec::const_iterator nit = nextInsts.begin(), enit = nextInsts.end();
       nit != enit; ++nit) {
...
void MHP::printInterleaving() {
  for (ThreadStmtToThreadInterleav::const_iterator
    ...
    outs() << "( t" << it->first.getTid() << " , $"
           ...
           << SVFUtil::value2String(it->first.getStmt()) << " ) ==> [";
...
void ForkJoinAnalysis::collectSCEVInfo() {
  ...
  for (ThreadCallGraph::CallSiteSet::const_iterator
  ...
  for (FunToFJSites::const_iterator it = funToFJSites.begin(),
                                    eit = funToFJSites.end();
       it != eit; ++it) {
    // ScalarEvolution* SE = MTA::getSE(it->first);
    for (CallInstSet::const_iterator sit = it->second.begin(),
                                     esit = it->second.end();
         sit != esit; ++sit) {
      ...
      if (tct->getThreadCallGraph()->isForksite(getCBN(callInst))) {
        ...
        // SE->getSCEV(const_cast<Value*>(forkSiteTidPtr)); const SCEV
        // *baseForkTidPtrSCEV =
        // SE->getSCEV(const_cast<Value*>(getBasePtr(forkSiteTidPtr)));
        ...
        // SE->getSCEV(const_cast<Value*>(joinSiteTidPtr)); const SCEV
        // *baseJoinTidPtrSCEV =
        // SE->getSCEV(const_cast<Value*>(getBasePtr(joinSiteTidPtr)));
      ...
      while (!cxtStmtList.empty()) {
        ...
        if (isTDFork(curInst)) {
          ...
          handleJoin(cts, rootTid);
                   ...
                   tct->isCandidateFun(getCallee(curInst, callees))) {
...
void ForkJoinAnalysis::handleFork(const CxtStmt &cts, NodeID rootTid) {
  ...
  if (getTCG()->hasThreadForkEdge(cbn)) {
    for (ThreadCallGraph::ForkEdgeSet::const_iterator
             cgIt = getTCG()->getForkEdgeBegin(cbn),
             ecgIt = getTCG()->getForkEdgeEnd(cbn);
         cgIt != ecgIt; ++cgIt) {
      const Function *callee =
          (*cgIt)->getDstNode()->getFunction()->getLLVMFun();
      CallStrCxt newCxt = curCxt;
      pushCxt(newCxt, call, callee);
      CxtThread ct(newCxt, call);
      if (getMarkedFlag(cts) != TDAlive)
...
void ForkJoinAnalysis::handleCall(const CxtStmt &cts, NodeID rootTid) {
  ...
  if (getTCG()->hasCallGraphEdge(cbn)) {
    for (PTACallGraph::CallGraphEdgeSet::const_iterator
             cgIt = getTCG()->getCallEdgeBegin(cbn),
...
void ForkJoinAnalysis::handleRet(const CxtStmt &cts) {
  ...
  PTACallGraphNode *curFunNode = getTCG()->getCallGraphNode(
      tct->getSVFFun(curInst->getParent()->getParent()));
  for (PTACallGraphNode::const_iterator it = curFunNode->getInEdges().begin(),
                                        eit = curFunNode->getInEdges().end();
       it != eit; ++it) {
    PTACallGraphEdge *edge = *it;
...
NodeBS ForkJoinAnalysis::getDirAndIndJoinedTid(const CxtStmt &cs) {
  ...
  for (NodeBS::iterator it = directJoinTids.begin(), eit = directJoinTids.end();
       it != eit; ++it) {
    worklist.push(*it);
  }
...
bool ForkJoinAnalysis::isSameSCEV(const Instruction *forkSite,
  ...
  if (forkse.inloop && joinse.inloop)
    ...
    return accessSameArrayIndex(SVFUtil::cast<GetElementPtrInst>(forkse.ptr),
