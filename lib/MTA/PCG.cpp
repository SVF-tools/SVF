...
void PCG::initFromThreadAPI(SVFModule *module) {
  for (SVFModule::const_iterator fi = module->begin(), efi = module->end();
    ...
    for (inst_iterator II = inst_begin((*fi)->getLLVMFun()),
                       E = inst_end((*fi)->getLLVMFun());
         II != E; ++II) {
...
/*!
 * 	Infer spawners and spawnees from call graph. The inference are
 * recursively done spawners: procedures may create a thread and return with the
 * created thread still running spawnees: procedures may be executed as a
 * spawned thread followers: procedures may be invoked by a thread after the
 * thread returns from a spawner (procedure may be called after pthread_creat is
 * called).
 */
...
void PCG::collectSpawners() {
  ...
  for (FunSet::iterator it = spawners.begin(), eit = spawners.end(); it != eit;
       ++it) {
    worklist.push(*it);
  }
  while (!worklist.empty()) {
    ...
    for (PTACallGraphNode::const_iterator it = funNode->InEdgeBegin(),
      ...
      const Function *caller =
          callEdge->getSrcNode()->getFunction()->getLLVMFun();
      if (isSpawnerFun(caller) == false) {
        worklist.push(caller);
        addSpawnerFun(caller);
      }
      /// add all the callsites from callers to callee (spawner) as a spawn
      /// site.
...
void PCG::collectSpawnees() {
  ...
  FunWorkList worklist;
  for (FunSet::iterator it = spawnees.begin(), eit = spawnees.end(); it != eit;
       ++it) {
    worklist.push(*it);
  }
  while (!worklist.empty()) {
...
void PCG::identifyFollowers() {
  ...
  for (CallInstSet::const_iterator sit = spawnSitesBegin(),
                                   esit = spawnSitesEnd();
       sit != esit; ++sit) {
        ...
        if ((SVFUtil::isa<CallBase>(inst)) && !isSpawnsite(inst)) {
          ...
          if (callgraph->hasCallGraphEdge(cbn)) {
            for (PTACallGraph::CallGraphEdgeSet::const_iterator
                     cgIt = callgraph->getCallEdgeBegin(cbn),
...
void PCG::printTDFuns() {
  ...
  for (SVFModule::const_iterator fi = mod->begin(), efi = mod->end(); fi != efi;
    ...
    outs() << fun->getName().str() << " [" << isSpawner << isSpawnee
           << isFollower << "]\n";
  }
}
