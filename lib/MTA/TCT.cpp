...
bool TCT::isInLoopInstruction(const Instruction *inst) {
  ...
  while (!worklist.empty()) {
    ...
    PTACallGraphNode *cgnode =
        tcg->getCallGraphNode(getSVFFun(inst->getParent()->getParent()));
  ...
  for (InstSet::const_iterator it = insts.begin(), eit = insts.end(); it != eit;
       ++it) {
    ...
    for (PTACallGraphNode::const_iterator nit = cgnode->InEdgeBegin(),
      ...
      for (PTACallGraphEdge::CallInstSet::const_iterator
        ...
        const Function *caller =
            (*cit)->getCallSite()->getParent()->getParent();
...
void TCT::collectEntryFunInCallGraph() {
  for (SVFModule::const_iterator it = getSVFModule()->begin(),
                                 eit = getSVFModule()->end();
       it != eit; ++it) {
  ...
  if (cgEdge->getEdgeKind() == PTACallGraphEdge::CallRetEdge) {
    ...
    if (pushToCTPWorkList(newctp)) {
      ...
      if (addTCTEdge(this->getGNode(ctp.getTid()), spawneeNode)) {
        DBOUT(DMTA,
              outs() << "Add TCT Edge from thread " << ctp.getTid() << "  ";
...
void TCT::getNextInsts(const Instruction *curInst, InstVec &instList) {
  ...
  if (!curInst->isTerminator()) {
    ...
    for (succ_const_iterator it = succ_begin(BB), ie = succ_end(BB); it != ie;
         ++it) {
      /// if we are sitting at the loop header, then go inside the loop but
      /// ignore loop exit
...
void TCT::pushCxt(CallStrCxt &cxt, const Instruction *call,
  ...
  if (inSameCallGraphSCC(tcg->getCallGraphNode(getSVFFun(caller)),
                         tcg->getCallGraphNode(getSVFFun(callee))) == false) {
    pushCxt(cxt, csId);
...
bool TCT::matchCxt(CallStrCxt &cxt, const Instruction *call,
  ...
  if (inSameCallGraphSCC(tcg->getCallGraphNode(getSVFFun(caller)),
                         tcg->getCallGraphNode(svfcallee)) == false) {
    if (cxt.back() == csId)
      cxt.pop_back();
    else
      return false;
...
void TCT::dumpCxt(CallStrCxt &cxt) {
  ...
  rawstr << "[:";
  for (CallStrCxt::const_iterator it = cxt.begin(), eit = cxt.end(); it != eit;
       ++it) {
    rawstr << " ' " << *it << " ' ";
...
TCTEdge *TCT::getGraphEdge(TCTNode *src, TCTNode *dst, TCTEdge::CEDGEK kind) {
  for (TCTEdge::ThreadCreateEdgeSet::const_iterator iter = src->OutEdgeBegin();
...
template <> struct DOTGraphTraits<TCT *> : public DefaultDOTGraphTraits {
  ...
  static std::string getEdgeAttributes(TCTNode *node, EdgeIter EI,
    ...
    /// black edge for direct call while two functions contain indirect calls
    /// will be label with red color
    return "color=black";
  }
};
} // End namespace llvm
