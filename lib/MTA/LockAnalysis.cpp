...
void LockAnalysis::collectLockUnlocksites() {
  ...
  for (SVFModule::iterator F = tct->getSVFModule()->begin(),
                           E = tct->getSVFModule()->end();
       F != E; ++F) {
    for (inst_iterator II = inst_begin((*F)->getLLVMFun()),
                       EE = inst_end((*F)->getLLVMFun());
         II != EE; ++II) {
...
void LockAnalysis::printLocks(const CxtStmt &cts) {
  ...
  for (CxtLockSet::const_iterator it = lockset.begin(), eit = lockset.end();
       it != eit; ++it) {
    (*it).dump();
  }
}
...
void LockAnalysis::pushCxt(CallStrCxt &cxt, const Instruction *call,
  ...
  if (tct->inSameCallGraphSCC(getTCG()->getCallGraphNode(svfcaller),
                              getTCG()->getCallGraphNode(svfcallee)) == false) {
...
bool LockAnalysis::isProtectedByCommonCxtLock(const Instruction *i1,
  ...
  for (CxtStmtSet::const_iterator cts1 = ctsset1.begin(), ects1 = ctsset1.end();
       cts1 != ects1; cts1++) {
