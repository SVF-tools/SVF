...
MTAResultValidator::getStringArg(const Instruction *inst,
  ...
  const Constant *arrayinst =
      SVFUtil::dyn_cast<Constant>(gepinst->getOperand(0));
  const ConstantDataArray *cxtarray =
...
bool MTAResultValidator::collectCallsiteTargets() {
  for (SVFModule::const_iterator fi = getModule()->begin(),
                                 efi = getModule()->end();
       fi != efi; ++fi) {
    for (Function::const_iterator bi = ((*fi)->getLLVMFun())->begin(),
                                  ebi = ((*fi)->getLLVMFun())->end();
         bi != ebi; ++bi) {
...
bool MTAResultValidator::collectCxtThreadTargets() {
  ...
  for (auto it = getModule()->llvmFunBegin(); it != getModule()->llvmFunEnd();
       it++) {
  ...
  for (MHP::InstToThreadStmtSetMap::iterator seti = instToTSMap.begin(),
    ...
    if ((*seti).second.size() != tsSet.size()) {
      if (Options::PrintValidRes) {
        ...
        outs() << "\n----Given threads:\n";
        for (MHP::CxtThreadStmtSet::iterator thdlevi = (*seti).second.begin(),
                                             ethdlevi = (*seti).second.end();
             thdlevi != ethdlevi; ++thdlevi) {
          outs() << "TID " << rthdTovthd[(*thdlevi).getTid()] << ": ";
          dumpCxt((*thdlevi).getContext());
        }
        outs() << "\n----Analysis threads:\n";
        for (MHP::CxtThreadStmtSet::const_iterator it = tsSet.begin(),
                                                   eit = tsSet.end();
             it != eit; ++it) {
    ...
    for (MHP::CxtThreadStmtSet::const_iterator it = tsSet.begin(),
      ...
      bool matched = false;
      for (MHP::CxtThreadStmtSet::iterator it2 = (*seti).second.begin(),
                                           eit2 = (*seti).second.end();
           it2 != eit2; ++it2) {
        ...
        if (ts2.getTid() == ts.getTid() &&
          ...
          if (lev != lev2) {
            ...
            if (lev.count() >= lev2.count()) {
              ...
              for (NodeBS::iterator it = lev2.begin(), eit = lev2.end();
                   it != eit; it++) {
                if (!lev.test(*it)) {
      ...
      if (!matched) {
        if (Options::PrintValidRes) {
          outs() << errMsg("\nValidate Interleaving: Wrong at:")
                 << SVFUtil::getSourceLoc(inst) << "\n";
          outs() << "Reason: analysis thread cxt is not matched by given "
                    "thread cxt\n";
          dumpCxt(ts.getContext());
          NodeBS lev = mhp->getInterleavingThreads(ts);
