...
inline std::string LockResultValidator::getOutput(const char *scenario,
  ...
  for (Value::const_use_iterator it = F->use_begin(), ie = F->use_end();
    ...
    if (const StoreInst *store = SVFUtil::dyn_cast<StoreInst>(memInst)) {
      if (const BinaryOperator *bop =
              SVFUtil::dyn_cast<BinaryOperator>(store->getValueOperand())) {
  ...
  for (LockAnalysis::CxtStmtToCxtLockSet::iterator it = analyedLS.begin(),
    ...
    if (LS.size() != (*it).second.size()) {
      if (Options::PrintValidRes) {
        outs() << errMsg("\nValidate Stmt's Lock : Wrong at: ")
               << SVFUtil::value2String(inst) << "\n";
        outs() << "Reason: The number of lock on current stmt is wrong\n";
        outs() << "\n----Given locks:\n";
        for (CxtLockSetStr::iterator it1 = LS.begin(), eit1 = LS.end();
             it1 != eit1; it++) {
          outs() << "Lock  " << *it1 << " ";
        }
        outs() << "\n----Analysis locks:\n";
    ...
    for (LockAnalysis::CxtLockSet::iterator it3 = LSA.begin(), eit3 = LSA.end();
         it3 != eit3; it3++) {
