...
static llvm::RegisterPass<MTA> RACEDETECOR("mta",
...
void MTA::detect(SVFModule *module) {
  ...
  for (LoadSet::const_iterator lit = loads.begin(), elit = loads.end();
       lit != elit; ++lit) {
    const LoadInst *load = *lit;
    bool loadneedcheck = false;
    for (StoreSet::const_iterator sit = stores.begin(), esit = stores.end();
         sit != esit; ++sit) {
