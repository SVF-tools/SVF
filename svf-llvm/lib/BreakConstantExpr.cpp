...
bool BreakConstantGEPs::runOnModule(Module& module)
{
    ...
    for (Module::iterator F = module.begin(), E = module.end(); F != E; ++F)
    {
        ...
        while (Worklist.size())
        {
            ...
            {
                for (u32_t index = 0; index < PHI->getNumIncomingValues();
                     ++index)
                {
                    //
                            ...
                            hasConstantExpr(PHI->getIncomingValue(index)))
                    {
                        ...
                        for (u32_t i2 = index; i2 < PHI->getNumIncomingValues();
                             ++i2)
                        {
            ...
            else
            {
                for (u32_t index = 0; index < I->getNumOperands(); ++index)
                {
                    ...
                    // replacing constant expressions immediately before the
                    // instruction using the constant expression.
                    //
                    if (ConstantExpr* CE =
                            hasConstantExpr(I->getOperand(index)))
                    {
