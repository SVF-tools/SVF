...
SVFIR* SVFIRBuilder::build()
{
    ...
    if (pag->getNodeNumAfterPAGBuild() > 1) return pag;
    ...
    // dump SVFIR
    if (Options::PAGDotGraph()) pag->dump("svfir_initial");

    // print to command line of the SVFIR graph
    if (Options::PAGPrint()) pag->print();

    // dump ICFG
    if (Options::DumpICFG()) pag->getICFG()->dump("icfg_initial");
...
void SVFIRBuilder::initialiseNodes()
{
    ...
    for (SymbolTableInfo::ValueToIDMapTy::iterator iter = symTable->valSyms().begin();
         iter != symTable->valSyms().end(); ++iter)
    {
        DBOUT(DPAGBuild, outs() << "add val node " << iter->second << "\n");
        if (iter->second == symTable->blkPtrSymID() || iter->second == symTable->nullPtrSymID()) continue;
        pag->addValNode(iter->first, iter->second);
    }

    for (SymbolTableInfo::ValueToIDMapTy::iterator iter = symTable->objSyms().begin();
         iter != symTable->objSyms().end(); ++iter)
    {
        DBOUT(DPAGBuild, outs() << "add obj node " << iter->second << "\n");
        if (iter->second == symTable->blackholeSymID() || iter->second == symTable->constantSymID()) continue;
        pag->addObjNode(iter->first, iter->second);
    }

    for (SymbolTableInfo::FunToIDMapTy::iterator iter = symTable->retSyms().begin(); iter != symTable->retSyms().end();
         ++iter)
    {
    ...
    for (SymbolTableInfo::FunToIDMapTy::iterator iter = symTable->varargSyms().begin();
    ...
    assert(pag->getTotalNodeNum() >= symTable->getTotalSymNum() && "not all node have been initialized!!!");
}
...
*/
u32_t SVFIRBuilder::inferFieldIdxFromByteOffset(const llvm::GEPOperator* gepOp, DataLayout* dl, AccessPath& ap,
                                                APOffset idx)
{
    return 0;
}
...
bool SVFIRBuilder::computeGepOffset(const User* V, AccessPath& ap)
{
    ...
    for (bridge_gep_iterator gi = bridge_gep_begin(*V), ge = bridge_gep_end(*V); gi != ge; ++gi)
    {
        ...
        {
            if (!op || (arrTy->getArrayNumElements() <= (u32_t)op->getSExtValue())) continue;
            APOffset idx = op->getSExtValue();
            u32_t offset =
                pag->getSymbolInfo()->getFlattenedElemIdx(LLVMModuleSet::getLLVMModuleSet()->getSVFType(arrTy), idx);
        ...
        else if (const StructType* ST = SVFUtil::dyn_cast<StructType>(gepTy))
        {
            ...
            APOffset idx = op->getSExtValue();
            u32_t offset =
                pag->getSymbolInfo()->getFlattenedElemIdx(LLVMModuleSet::getLLVMModuleSet()->getSVFType(ST), idx);
            ap.setFldIdx(ap.getConstantStructFldIdx() + offset);
        }
        else if (gepTy->isSingleValueType())
        {
            // If it's a non-constant offset access
            // If its point-to target is struct or array, it's likely an array accessing (%result = gep %struct.A* %a,
            // i32 %non-const-index) If its point-to target is single value (pointer arithmetic), then it's a variant
            // gep (%result = gep i8* %p, i32 %non-const-index)
...
void SVFIRBuilder::processCE(const Value* val)
{
    if (const Constant* ref = SVFUtil::dyn_cast<Constant>(val))
    {
        if (const ConstantExpr* gepce = isGepConstantExpr(ref))
        {
             ...
             */
            addGepEdge(pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                       pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(gepce)), ap, constGep);
        ...
        else if (const ConstantExpr* castce = isCastConstantExpr(ref))
        {
            ...
            addCopyEdge(pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                        pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(castce)), CopyStmt::BITCAST);
            setCurrentLocation(cval, cbb);
        }
        ...
        else if (const ConstantExpr* int2Ptrce = isInt2PtrConstantExpr(ref))
        {
            ...
            addCopyEdge(pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(opnd)),
                        pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(int2Ptrce)),
                        CopyStmt::INTTOPTR);
            setCurrentLocation(cval, cbb);
        }
        ...
        else if (SVFUtil::isa<BlockAddress>(ref))
        {
            ...
            // is treated as constant data object for now, see LLVMUtil.h:397, SymbolTableInfo.cpp:674 and
            // SVFIRBuilder.cpp:183-194
            const SVFValue* cval = getCurrentValue();
            const SVFBasicBlock* cbb = getCurrentBB();
            setCurrentLocation(ref, nullptr);
            NodeID dst = pag->getValueNode(LLVMModuleSet::getLLVMModuleSet()->getSVFValue(ref));
...
{
    DBOUT(DPAGBuild,
          outs() << "global " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(gvar)->toString()
                 << " constant initializer: " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(C)->toString() << "\n");
    ...
    {
        if (cppUtil::isValVtbl(gvar) && !Options::VtableInSVFIR()) return;
        for (u32_t i = 0, e = C->getNumOperands(); i != e; i++)
        {
            u32_t off = pag->getSymbolInfo()->getFlattenedElemIdx(
                LLVMModuleSet::getLLVMModuleSet()->getSVFType(C->getType()), i);
...
void SVFIRBuilder::visitGlobal(SVFModule* svfModule)
{
    ...
    for (Module& M : LLVMModuleSet::getLLVMModuleSet()->getLLVMModules())
    {
        for (Module::global_iterator I = M.global_begin(), E = M.global_end(); I != E; ++I)
        {
            ...
            if (gvar->hasInitializer())
            {
                ...
                DBOUT(DPAGBuild, outs() << "add global var node "
                                        << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(gvar)->toString() << "\n");
        ...
        // Handle global aliases (due to linkage of multiple bc files), e.g., @x = internal alias @y. We need to add a
        // copy from y to x.
...
void SVFIRBuilder::visitAllocaInst(AllocaInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process alloca  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitPHINode(PHINode& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process phi " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << "  \n");
    ...
    for (u32_t i = 0; i < inst.getNumIncomingValues(); ++i)
    {
        ...
        bool matched = (incomingInst == nullptr || incomingInst->getFunction() == inst.getFunction());
...
{
    DBOUT(DPAGBuild,
          outs() << "process load  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitStoreInst(StoreInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process store " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitGetElementPtrInst(GetElementPtrInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process gep  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitCastInst(CastInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process cast  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
    NodeID dst = getValueNode(&inst);
...
void SVFIRBuilder::visitSelectInst(SelectInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process select  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitCallSite(CallBase* cs)
{
    ...
    if (!cs->getType()->isVoidTy()) pag->addCallSiteRets(retBlockNode, pag->getGNode(getValueNode(cs)));
...
void SVFIRBuilder::visitReturnInst(ReturnInst& inst)
{
    ...
    DBOUT(DPAGBuild,
          outs() << "process return  " << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(&inst)->toString() << " \n");
...
void SVFIRBuilder::visitBranchInst(BranchInst& inst)
{
    ...
    NodeID cond;
    if (inst.isConditional()) cond = getValueNode(inst.getCondition());
    else
        cond = pag->getNullPtr();
...
void SVFIRBuilder::visitSwitchInst(SwitchInst& inst)
{
    ...
    for (const Instruction* succInst : nextInsts)
    {
        ...
        s64_t val = -1;
        if (condVal && condVal->getBitWidth() <= 64) val = condVal->getSExtValue();
        const SVFInstruction* svfSuccInst = LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(succInst);
        const ICFGNode* icfgNode = pag->getICFG()->getICFGNode(svfSuccInst);
...
/// TODO: for now, create a copy edge from %ap2 to %tmp, we assume here %tmp should point to the n-th argument of the
/// var_args
...
void SVFIRBuilder::handleDirectCall(CallBase* cs, const Function* F)
{
    ...
    for (; itF != ieF; ++itA, ++itF)
    {
        ...
        DBOUT(DPAGBuild, outs() << "process actual parm  "
                                << LLVMModuleSet::getLLVMModuleSet()->getSVFValue(AA)->toString() << " \n");
...
const Value* SVFIRBuilder::getBaseValueForExtArg(const Value* V)
{
    ...
    if (const GetElementPtrInst* gep = SVFUtil::dyn_cast<GetElementPtrInst>(value))
    {
        ...
        {
            if (const ConstantInt* op = SVFUtil::dyn_cast<ConstantInt>(gi.getOperand())) totalidx += op->getSExtValue();
        }
        if (totalidx == 0 && !SVFUtil::isa<StructType>(value->getType())) value = gep->getPointerOperand();
    }
    return value;
}
...
void SVFIRBuilder::updateCallGraph(CallGraph* callgraph)
{
    ...
    for (; iter != eiter; iter++)
    {
        ...
        const CallBase* callbase =
            SVFUtil::cast<CallBase>(LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(callBlock->getCallSite()));
        assert(callBlock->isIndirectCall() && "this is not an indirect call?");
        const CallGraph::FunctionSet& functions = iter->second;
        for (CallGraph::FunctionSet::const_iterator func_iter = functions.begin(); func_iter != functions.end();
             func_iter++)
        {
            const Function* callee =
                SVFUtil::cast<Function>(LLVMModuleSet::getLLVMModuleSet()->getLLVMValue(*func_iter));
    ...
    // dump SVFIR
    if (Options::PAGDotGraph()) pag->dump("svfir_final");
}
...
NodeID SVFIRBuilder::getGepValVar(const Value* val, const AccessPath& ap, const SVFType* elementType)
{
    ...
    if (gepval == UINT_MAX)
    {
         ...
         * 2. GlobalVariable
         */
        assert((SVFUtil::isa<SVFInstruction, SVFGlobalValue>(curVal)) &&
        ...
        NodeID gepNode =
            pag->addGepValNode(curVal, llvmmodule->getSVFValue(val), ap, NodeIDAllocator::get()->allocateValueId(),
...
void SVFIRBuilder::setCurrentBBAndValueForPAGEdge(PAGEdge* edge)
{
    ...
    if (const SVFInstruction* curInst = SVFUtil::dyn_cast<SVFInstruction>(curVal))
    {
        ...
        if (srcFun != nullptr && !SVFUtil::isa<RetPE>(edge) &&
            !SVFUtil::isa<SVFFunction>(edge->getSrcNode()->getValue()))
        {
        ...
        if (dstFun != nullptr && !SVFUtil::isa<CallPE>(edge) &&
            !SVFUtil::isa<SVFFunction>(edge->getDstNode()->getValue()))
        {
        ...
        {
            if (SVFUtil::isa<RetPE>(edge)) icfgNode = pag->getICFG()->getRetICFGNode(curInst);
            else
    ...
    else if (SVFUtil::isa<SVFConstant>(curVal) || SVFUtil::isa<SVFFunction>(curVal) ||
             SVFUtil::isa<SVFMetadataAsValue>(curVal))
    {
        if (!curBB) pag->addGlobalPAGEdge(edge);
        else
        {
...
AccessPath SVFIRBuilder::getAccessPathFromBaseNode(NodeID nodeId)
{
    ...
    if (geps.empty()) return AccessPath(0);
    ...
    if (gepEdge->isVariantFieldGep()) return AccessPath(0);
    else
