...
bool LLVMUtil::isObject(const Value* ref)
{
    if (SVFUtil::isa<Instruction>(ref) &&
        SVFUtil::isHeapAllocExtCallViaRet(
            LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(SVFUtil::cast<Instruction>(ref))))
        return true;
    if (SVFUtil::isa<GlobalVariable>(ref)) return true;
    if (SVFUtil::isa<Function, AllocaInst>(ref)) return true;
...
{
    assert(!SVFUtil::isExtCall(LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(fun)) &&
    ...
    while (!bbVec.empty())
    {
        ...
        if (DomTreeNode* dtNode = dt.getNode(const_cast<BasicBlock*>(bb)))
        {
            for (DomTreeNode::iterator DI = dtNode->begin(), DE = dtNode->end(); DI != DE; ++DI)
            {
                ...
                if (visited.find(succbb) == visited.end()) visited.insert(succbb);
                else
                    continue;
...
bool LLVMUtil::basicBlockHasRetInst(const BasicBlock* bb)
{
    ...
    {
        if (SVFUtil::isa<ReturnInst>(*it)) return true;
    }
    return false;
}
...
{
    if (fun->hasAddressTaken()) return false;
    if (LLVMUtil::isProgEntryFunction(fun)) return false;
    for (Value::const_user_iterator i = fun->user_begin(), e = fun->user_end(); i != e; ++i)
    {
        if (LLVMUtil::isCallSite(*i)) return false;
    }
    if (LLVMModuleSet::getLLVMModuleSet()->hasDeclaration(fun))
    {
        ...
        for (LLVMModuleSet::FunctionSetType::const_iterator it = decls.begin(), eit = decls.end(); it != eit; ++it)
        {
            const Function* decl = *it;
            if (decl->hasAddressTaken()) return false;
...
bool LLVMUtil::isPtrInUncalledFunction(const Value* value)
{
    ...
    {
        if (isUncalledFunction(inst->getParent()->getParent())) return true;
    }
    else if (const Argument* arg = SVFUtil::dyn_cast<Argument>(value))
    {
        if (isUncalledFunction(arg->getParent())) return true;
    }
    return false;
}
...
bool LLVMUtil::isIntrinsicFun(const Function* func)
{
    if (func &&
        (func->getIntrinsicID() == llvm::Intrinsic::donothing ||
         ...
         func->getIntrinsicID() == llvm::Intrinsic::dbg_label || func->getIntrinsicID() == llvm::Intrinsic::dbg_value))
    {
        return true;
    }
    return false;
...
{
    if (SVFUtil::isa<GlobalValue>(val) || isInt2PtrConstantExpr(val)) return val;
    else if (const ConstantExpr* CE = SVFUtil::dyn_cast<ConstantExpr>(val))
    {
        if (Instruction::isCast(CE->getOpcode())) return stripConstantCasts(CE->getOperand(0));
    }
    return val;
}
...
const Value* LLVMUtil::stripAllCasts(const Value* val)
{
    while (true)
    {
        ...
        {
            if (ce->isCast()) val = ce->getOperand(0);
            else
                return val;
        }
        else
...
void LLVMUtil::getNextInsts(const Instruction* curInst, std::vector<const SVFInstruction*>& instList)
{
    if (!curInst->isTerminator())
    {
        ...
        if (LLVMUtil::isIntrinsicInst(nextInst)) getNextInsts(nextInst, instList);
        else
...
const Value* LLVMUtil::getFirstUseViaCastInst(const Value* val)
{
    ...
    {
        if (SVFUtil::isa<BitCastInst>(it.getUser())) latestUse = it.getUser();
        else
            latestUse = nullptr;
    }
    return latestUse;
...
{
    u32_t num = 0;
    for (const_pred_iterator it = pred_begin(BB), et = pred_end(BB); it != et; ++it) num++;
    return num;
}
...
{
    if (removedFuncList.empty()) return; // No functions to remove annotations in extapi.bc module
    ...
    for (unsigned i = 0; i < ca->getNumOperands(); ++i)
    {
        ...
        if (annotatedFunc &&
            std::find(removedFuncList.begin(), removedFuncList.end(), annotatedFunc) == removedFuncList.end())
        {
            newAnnotations.push_back(structAn);
        }
    }

    if (newAnnotations.size() == ca->getNumOperands()) return; // No annotations to remove
    ...
    GlobalVariable* GV =
        new GlobalVariable(newCA->getType(), glob->isConstant(), glob->getLinkage(), newCA, "llvm.global.annotations");
    ...
    for (const Instruction& I : instructions(F))
    {
        if (const CallBase* callInst = SVFUtil::dyn_cast<CallBase>(&I))
        {
            ...
            if (calledFunction)
            {
                ...
                calledFunctions.insert(calledFunctions.end(), nestedCalledFunctions.begin(),
...
bool LLVMUtil::isUnusedGlobalVariable(const GlobalVariable& global)
{
    ...
    else
    {
        ...
        for (auto& use : global.uses())
            if (use.getUser()->getNumUses() != 0) return false;
    }
    return true;
}
...
void LLVMUtil::removeUnusedGlobalVariables(Module* module)
{
    ...
    for (GlobalVariable* global : unusedGlobals) global->eraseFromParent();
}
...
{
    if (removedFuncList.empty()) return;

    Module* mod = (*removedFuncList.begin())->getParent();
    if (mod->getName().str() != ExtAPI::getExtAPI()->getExtBcPath()) return;
    ...
    auto isCalledFunction = [](llvm::Function* F) {
        ...
        for (auto& use : F->uses())
        {
            ...
            if (llvm::isa<llvm::CallBase>(user)) return true;
        }
        return false;
    };
...
std::string LLVMUtil::restoreFuncName(std::string funcName)
{
    ...
    // Since C function names cannot include '.', change the function name from llvm.memcpy.p0i8.p0i8.i64 to
    // llvm_memcpy_p0i8_p0i8_i64."
    bool hasSpecialPrefix = funcName[0] == '\01';
    bool hasDot = funcName.find('.') != std::string::npos;

    if (!hasDot && !hasSpecialPrefix) return funcName;
    ...
    if (hasSpecialPrefix)
    {
        ...
        if (funcName.substr(0, prefix1.length()) == prefix1) funcName = funcName.substr(prefix1.length());
        else if (funcName.substr(0, prefix2.length()) == prefix2)
            funcName = funcName.substr(prefix2.length());
    }
    // Replace '.' with '_'
    if (hasDot) std::replace(funcName.begin(), funcName.end(), '.', '_');
...
u32_t LLVMUtil::getTypeSizeInBytes(const Type* type)
{
    ...
    if (type->isSized())
        return getDataLayout(LLVMModuleSet::getLLVMModuleSet()->getMainLLVMModule())
...
u32_t LLVMUtil::getTypeSizeInBytes(const StructType* sty, u32_t field_idx)
{
    ...
    const StructLayout* stTySL = getDataLayout(LLVMModuleSet::getLLVMModuleSet()->getMainLLVMModule())
...
const std::string LLVMUtil::getSourceLoc(const Value* val)
{
    ...
    {
        if (argument->getArgNo() % 10 == 1) rawstr << argument->getArgNo() << "st";
...
std::string LLVMUtil::dumpValueAndDbgInfo(const Value* val)
{
    std::string str;
    llvm::raw_string_ostream rawstr(str);
    if (val) rawstr << dumpValue(val) << getSourceLoc(val);
    else
        rawstr << " llvm Value is null";
    return rawstr.str();
}
