...
bool LLVMUtil::isUncalledFunction(const Function* fun)
{
    ...
    {
        const LLVMModuleSet::FunctionSetType& decls =
                            ...
                            std::vector<const SVFInstruction*>& instList)
{
    if (!curInst->isTerminator())
    {
        ...
        const SVFInstruction* svfNextInst =
            LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(nextInst);
...
void LLVMUtil::getPrevInsts(const Instruction* curInst,
                            std::vector<const SVFInstruction*>& instList)
{
    ...
    if (curInst != &(curInst->getParent()->front()))
    {
        ...
        const SVFInstruction* svfPrevInst =
            LLVMModuleSet::getLLVMModuleSet()->getSVFInstruction(prevInst);
...
void LLVMUtil::removeFunAnnotations(Set<Function*>& removedFuncList)
{
    ...
    ConstantArray* ca =
        SVFUtil::dyn_cast<ConstantArray>(glob->getInitializer());
    if (ca == nullptr)
        return;
    ...
    {
        ConstantStruct* structAn =
            SVFUtil::dyn_cast<ConstantStruct>(ca->getOperand(i));
        if (structAn == nullptr)
            continue;
        ...
        if (ConstantExpr* expr =
                SVFUtil::dyn_cast<ConstantExpr>(structAn->getOperand(0)))
        {
            if (expr->getOpcode() == Instruction::BitCast)
            {
                annotatedFunc =
                    SVFUtil::dyn_cast<Function>(expr->getOperand(0));
            }
        }

        // Opague pointer, If the above method didn't work, try casting directly
        // to Function
        if (!annotatedFunc)
        {
            annotatedFunc =
                SVFUtil::dyn_cast<Function>(structAn->getOperand(0));
        }

        // Process the annotated function if it's not in the removed list
        if (annotatedFunc &&
            std::find(removedFuncList.begin(), removedFuncList.end(),
                      annotatedFunc) == removedFuncList.end())
        {
    ...
    ArrayType* annotationsType =
        ArrayType::get(ca->getType()->getElementType(), newAnnotations.size());
    Constant* newCA = ConstantArray::get(annotationsType, newAnnotations);

    // Check if a global variable with the name llvm.global.annotations already
    // exists
    GlobalVariable* existingGlobal =
        module->getGlobalVariable("llvm.global.annotations");
    if (existingGlobal)
        // Rename the existing llvm.global.annotations to
        // llvm.global.annotations_old
        existingGlobal->setName("llvm.global.annotations_old");

    // Create a new global variable with the updated annotations
    GlobalVariable* newGlobal = new GlobalVariable(
        *module, newCA->getType(), glob->isConstant(), glob->getLinkage(),
        newCA, "llvm.global.annotations", glob, glob->getThreadLocalMode());
...
bool LLVMUtil::isUnusedGlobalVariable(const GlobalVariable& global)
{
    ...
    if (global.getName() == "llvm.global.annotations" &&
        SVFUtil::isa<ConstantArray>(global.getInitializer()))
        return false;
    else
    {
...
void LLVMUtil::removeUnusedFuncsAndAnnotationsAndGlobalVariables(
    Set<Function*> removedFuncList)
{
    ...
    /// The functions to be deleted from extapi.bc can be categorized into two
    /// types. The first type includes functions do not contain any invocation
    /// statements, The second type includes functions whose contain invocation
    /// statements. It is necessary to delete functions of the second type first
    /// before deleting those of the first type; Otherwise, errors may occur
    /// when calling eraseFromParent().
    ...
    for (Function* func : removedFuncList)
    {
        if (isCalledFunction(func))
            // Record first kind function(which does not contain any invocation
            // statements)
...
void LLVMUtil::getSuccBBandCondValPairVec(const SwitchInst& switchInst,
                                          SuccBBAndCondValPairVec& vec)
{
...
s64_t LLVMUtil::getCaseValue(const SwitchInst& switchInst,
                             SuccBBAndCondValPair& succBB2CondVal)
{
