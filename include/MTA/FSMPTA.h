...
public:
  SVFGNodeLockSpan(const StmtSVFGNode *SVFGnode,
  ...
  inline const StmtSVFGNode *getSVFGNode() const { return SVFGNode; }
  inline const LockAnalysis::LockSpan getLockSpan() const { return lockSpan; }
...
private:
  ...
  void handleStoreLoadNonSparse(const StmtSVFGNode *n1, const StmtSVFGNode *n2,
                                PointerAnalysis *pta);
  void handleStoreStoreNonSparse(const StmtSVFGNode *n1, const StmtSVFGNode *n2,
                                 PointerAnalysis *pta);
...
public:
  ...
  FSMPTA(MHP *m, LockAnalysis *la)
      : FlowSensitive(m->getTCT()->getPTA()->getPAG()), mhp(m), lockana(la) {}
  ...
  inline SVFIR *getPAG() { return mhp->getTCT()->getPTA()->getPAG(); }
