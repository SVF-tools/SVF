...
public:
  ...
  // Analysis
  void analyze();
  inline SVFModule *getModule() const { return mod; }
...
protected:
   ...
   * basic blocks. The collected targets are stored in csnumToInstMap that maps
   * label "cs1" to CallInst.
   */
...
private:
  ...
  MHP::ThreadStmtToThreadInterleav
      threadStmtToInterLeaving; /// Map a statement to its thread interleavings
...
protected:
  ...
  void collectValidationTargets() {
    ...
    for (int i = 0, e = csInsts.size(); i != e;) {
      ...
      assert(I1 && I2 &&
             "RC_ACCESS should be placed immediately after the target memory "
             "access.");
