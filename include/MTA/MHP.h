...
public:
  ...
  //    /// Interface to query whether two instructions are protected by common
  //    locks virtual bool isProtectedByACommonLock(const Instruction* i1, const
  //    Instruction* i2); virtual bool isAllCxtInSameLockSpan(const Instruction
  //    *I1, const Instruction *I2); virtual bool isOneCxtInSameLockSpan(const
  //    Instruction *I1, const Instruction *I2);
  //
...
private:
  inline const PTACallGraph::FunctionSet &
  getCallee(const Instruction *inst, PTACallGraph::FunctionSet &callees) {
  ...
  inline void rmInterleavingThread(const CxtThreadStmt &tgr, const NodeBS &tids,
...
public:
  ...
  /// to its corresponding thread ids (2) never happen-in-parallel thread pairs
  ...
  /// Get directly and indirectly joined threadIDs based on a context-sensitive
  /// join site
...
private:
  ...
  void markCxtStmtFlag(const CxtStmt &tgr, const CxtStmt &src) {
    ...
    if (flag_tgr == Empty) {
      ...
      /// alive is at the bottom of the semilattice, nothing needs to be done
      /// here
    }
    if (flag_tgr != getMarkedFlag(tgr)) {
      pushToCTSWorkList(tgr);
  ...
  CxtStmtToTIDMap
      dirAndIndJoinMap; ///< maps a context-sensitive join site to directly and
                        ///< indirectly joined thread ids
  CxtStmtToLoopMap
