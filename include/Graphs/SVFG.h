...
public:
  ...
  typedef Map<const CallICFGNode *, ActualINSVFGNodeSet>
      CallSiteToActualINsMapTy;
  typedef Map<const CallICFGNode *, ActualOUTSVFGNodeSet>
      CallSiteToActualOUTsMapTy;
  typedef Map<const SVFFunction *, FormalINSVFGNodeSet>
      FunctionToFormalINsMapTy;
  typedef Map<const SVFFunction *, FormalOUTSVFGNodeSet>
...
public:
  ...
  void getInterVFEdgesForIndirectCallSite(const CallICFGNode *cs,
  ...
  inline const DummyVersionPropSVFGNode *
  addDummyVersionPropSVFGNode(const NodeID object, const NodeID version) {
    DummyVersionPropSVFGNode *dvpNode =
        new DummyVersionPropSVFGNode(totalVFGNode++, object, version);
...
protected:
  ...
  SVFGEdge *addRetIndirectVFEdge(NodeID srcId, NodeID dstId, const NodeBS &cpts,
                                 CallSiteID csId);
  ...
  virtual inline void getInterVFEdgeAtIndCSFromAPToFP(const PAGNode *cs_arg,
    ...
    SVFGEdge *edge =
        hasInterVFGEdge(actualParam, formalParam, SVFGEdge::CallDirVF, csId);
    assert(edge != nullptr &&
  ...
  getInterVFEdgeAtIndCSFromAInToFIn(ActualINSVFGNode *actualIn,
                                    const SVFFunction *callee,
                                    SVFGEdgeSetTy &edges) {
    for (SVFGNode::const_iterator outIt = actualIn->OutEdgeBegin(),
  ...
  getInterVFEdgeAtIndCSFromFOutToAOut(ActualOUTSVFGNode *actualOut,
                                      const SVFFunction *callee,
                                      SVFGEdgeSetTy &edges) {
    for (SVFGNode::const_iterator inIt = actualOut->InEdgeBegin(),
  ...
  inline void addFormalINSVFGNode(const FunEntryICFGNode *funEntry,
                                  const MRVer *resVer, const NodeID nodeId) {
  ...
  inline void addFormalOUTSVFGNode(const FunExitICFGNode *funExit,
                                   const MRVer *ver, const NodeID nodeId) {
  ...
  inline void addActualINSVFGNode(const CallICFGNode *callsite,
                                  const MRVer *ver, const NodeID nodeId) {
    ActualINSVFGNode *sNode = new ActualINSVFGNode(nodeId, callsite, ver);
    addSVFGNode(sNode,
                pag->getICFG()->getCallICFGNode(callsite->getCallSite()));
    callSiteToActualINMap[callsite].set(sNode->getId());
  }

  /// Add memory callsite chi SVFG node
  inline void addActualOUTSVFGNode(const CallICFGNode *callsite,
                                   const MRVer *resVer, const NodeID nodeId) {
  ...
  inline void addIntraMSSAPHISVFGNode(
      ICFGNode *BlockICFGNode,
