...
/*
 * Graph representation of SVF IR. It can be seen as a program assignment graph
 * (PAG)
 */
...
protected:
  SVFStmt::KindToSVFStmtMapTy
      KindToSVFStmtSetMap; // < SVFIR edge map containing all PAGEdges
  SVFStmt::KindToSVFStmtMapTy
      KindToPTASVFStmtSetMap; // < SVFIR edge map containing only
                              // pointer-related edges, i.e, both RHS and RHS
                              // are of pointer type
  bool fromFile; ///< Whether the SVFIR is built according to user specified
                 ///< data from a txt file
  NodeID
      nodeNumAfterPAGBuild; // initial node number after building SVFIR,
                            // excluding later added nodes, e.g., gepobj nodes
  u32_t totalPTAPAGEdge;
  ValueToEdgeMap
      valueToEdgeMap; ///< Map llvm::Values to all corresponding PAGEdges
  ...
  SVFStmt *hasLabeledEdge(SVFVar *src, SVFVar *dst, SVFStmt::PEDGEK kind,
                          const ICFGNode *cs);
  /// Return MultiOpndStmt since it has more than one operands (we use operand 2
  /// here to make the flag)
...
public:
  ...
  inline NodeID getObjectNode(const Value *V) { return symInfo->getObjSym(V); }
  /// GetReturnNode - Return the unique node representing the return value of a
  /// function
  inline NodeID getReturnNode(const SVFFunction *func) const {
    return symInfo->getRetSym(func->getLLVMFun());
  }
  /// getVarargNode - Return the unique node representing the variadic argument
  ...
  inline const MemObj *getBlackHoleObj() const { return symInfo->getBlkObj(); }
  inline const MemObj *getConstantObj() const {
    return symInfo->getConstantObj();
  }

  inline u32_t getValueNodeNum() const { return symInfo->valSyms().size(); }
  inline u32_t getObjectNodeNum() const { return symInfo->idToObjMap().size(); }
  inline u32_t getNodeNumAfterPAGBuild() const { return nodeNumAfterPAGBuild; }
  inline void setNodeNumAfterPAGBuild(u32_t num) { nodeNumAfterPAGBuild = num; }

  inline u32_t getPAGNodeNum() const { return nodeNum; }
  inline u32_t getPAGEdgeNum() const { return edgeNum; }
  inline u32_t getPTAPAGEdgeNum() const { return totalPTAPAGEdge; }
  /// Return graph name
  inline std::string getGraphName() const { return "SVFIR"; }
...
/* !
 * GraphTraits specializations of SVFIR to be used for the generic graph
 * algorithms. Provide graph traits for tranversing from a SVFIR node using
 * standard graph traversals.
 */
template <>
struct GraphTraits<SVF::SVFVar *>
    : public GraphTraits<SVF::GenericNode<SVF::SVFVar, SVF::SVFStmt> *> {};

/// Inverse GraphTraits specializations for SVFIR node, it is used for inverse
/// traversal.
template <>
struct GraphTraits<Inverse<SVF::SVFVar *>>
    : public GraphTraits<
          Inverse<SVF::GenericNode<SVF::SVFVar, SVF::SVFStmt> *>> {};

template <>
struct GraphTraits<SVF::IRGraph *>
    : public GraphTraits<SVF::GenericGraph<SVF::SVFVar, SVF::SVFStmt> *> {
