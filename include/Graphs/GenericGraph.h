...
struct GraphTraits<SVF::GenericNode<NodeTy, EdgeTy> *> {
  ...
  typedef mapped_iterator<typename SVF::GenericNode<NodeTy, EdgeTy>::iterator,
                          decltype(&edge_dest)>
      ChildIteratorType;
...
struct GraphTraits<SVF::GenericGraph<NodeTy, EdgeTy> *>
  ...
  static inline NodeType *deref_val(PairTy P) { return P.second; }

  // nodes_iterator/begin/end - Allow iteration over all nodes in the graph
  typedef mapped_iterator<typename GenericGraphTy::iterator,
                          decltype(&deref_val)>
      nodes_iterator;
  ...
  static unsigned graphSize(GenericGraphTy *G) { return G->getTotalNodeNum(); }
