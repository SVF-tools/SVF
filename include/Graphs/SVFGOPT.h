...
protected:
  ...
  //@{
  virtual inline void connectAParamAndFParam(const PAGNode *cs_arg,
  ...
  virtual inline void connectFRetAndARet(const PAGNode *fun_ret,
                                         const PAGNode *cs_ret, CallSiteID csId,
                                         SVFGEdgeSetTy &edges) {
    NodeID phiId = getDef(cs_ret);
    NodeID retdef = getDef(fun_ret);
    /// If a function does not have any return instruction. The def of a
    /// FormalRetVFGNode is itself (see VFG.h: addFormalRetVFGNode). Therefore,
    /// we do not connect return edge from a function without any return
    /// instruction (i.e., pag->isPhiNode(fun_ret)==false) because unique
    /// fun_ret PAGNode was not collected as a PhiNode in
    /// SVFIRBuilder::visitReturnInst
  ...
  virtual inline void connectAInAndFIn(const ActualINSVFGNode *actualIn,
    ...
    if (intersection.empty() == false) {
      ...
      SVFGEdge *edge =
          addCallIndirectSVFGEdge(aiDef, formalIn->getId(), csId, intersection);
      if (edge != nullptr)
        edges.insert(edge);
    }
  }
  ...
  virtual inline void connectFOutAndAOut(const FormalOUTSVFGNode *formalOut,
    ...
    if (intersection.empty() == false) {
      ...
      SVFGEdge *edge =
          addRetIndirectSVFGEdge(foDef, actualOut->getId(), csId, intersection);
      if (edge != nullptr)
        edges.insert(edge);
    }
  }
...
private:
  ...
  SVFGEdge *addCallIndirectSVFGEdge(NodeID srcId, NodeID dstId, CallSiteID csid,
                                    const NodeBS &cpts);
  /// Add indirect ret edge from src to dst with one call site ID.
  SVFGEdge *addRetIndirectSVFGEdge(NodeID srcId, NodeID dstId, CallSiteID csid,
                                   const NodeBS &cpts);
  //@}
  ...
  inline InterPHISVFGNode *addInterPHIForAR(const ActualRetSVFGNode *ar) {
    ...
    addSVFGNode(sNode, pag->getICFG()->getRetICFGNode(
  ...
  inline void resetDef(const PAGNode *pagNode, const SVFGNode *node) {
    ...
    assert(it != PAGNodeToDefMap.end() &&
           "a SVFIR node doesn't have definition before");
  ...
  /// Return TRUE if this node has both incoming call/ret and outgoing call/ret
  /// edges.
