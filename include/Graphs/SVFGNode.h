...
public:
  ...
  inline const NodeBS &getPointsTo() const { return cpts; }
  ...
  static inline bool classof(const VFGNode *node) {
    return node->getNodeKind() == FPIN || node->getNodeKind() == FPOUT ||
           ...
           node->getNodeKind() == MPhi || node->getNodeKind() == MIntraPhi ||
...
public:
  /// Constructor
  FormalINSVFGNode(NodeID id, const MRVer *resVer,
                   const FunEntryICFGNode *funEntry)
      : MRSVFGNode(id, FPIN) {
    cpts = resVer->getMR()->getPointsTo();
    ver = resVer;
    funEntryNode = funEntry;
  }
  inline const MRVer *getMRVer() const { return ver; }
...
public:
  ...
  inline const FunExitICFGNode *getFunExitNode() const { return funExitNode; }
...
public:
  ...
  /// Callsite
  inline const CallICFGNode *getCallSite() const { return cs; }
...
public:
  ...
  /// Ver
  inline const MRVer *getResVer() const { return ver; }
  /// MSSA phi operands
  //@{
  ...
  static inline bool classof(const MRSVFGNode *node) {
    return (node->getNodeKind() == MPhi || node->getNodeKind() == MIntraPhi ||
            node->getNodeKind() == MInterPhi);
  }
  static inline bool classof(const VFGNode *node) {
    return (node->getNodeKind() == MPhi || node->getNodeKind() == MIntraPhi ||
...
public:
  ...
  IntraMSSAPHISVFGNode(NodeID id, const MRVer *resVer)
      : MSSAPHISVFGNode(id, resVer, MIntraPhi) {}

  inline const MRVer *getMRVer() const { return getResVer(); }
...
public:
  ...
  InterMSSAPHISVFGNode(NodeID id, const FormalINSVFGNode *fi)
      : MSSAPHISVFGNode(id, fi->getMRVer(), MInterPhi), fun(fi->getFun()),
        callInst(nullptr) {}
  /// Constructor interPHI for actual return
  InterMSSAPHISVFGNode(NodeID id, const ActualOUTSVFGNode *ao)
      : MSSAPHISVFGNode(id, ao->getMRVer(), MInterPhi), fun(nullptr),
...
public:
  ...
  //@{
  static inline bool classof(const DummyVersionPropSVFGNode *) { return true; }
