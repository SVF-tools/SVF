...
public:
  ...
  typedef Map<std::pair<NodeID, const CallICFGNode *>, ActualParmVFGNode *>
...
protected:
  ...
  PAGNodeToBranchVFGNodeMapTy
      PAGNodeToBranchVFGNodeMap; ///< map a PAGNode to its BranchVFGNode
...
public:
  ...
  virtual void connectCallerAndCallee(const CallICFGNode *cs,
                                      const SVFFunction *callee,
                                      VFGEdgeSetTy &edges);

  /// Get callsite given a callsiteID
  //@{
  inline CallSiteID getCallSiteID(const CallICFGNode *cs,
  ...
  inline IntraPHIVFGNode *getIntraPHIVFGNode(const PAGNode *pagNode) const {
    PAGNodeToPHIVFGNodeMapTy::const_iterator it =
        PAGNodeToIntraPHIVFGNodeMap.find(pagNode);
  ...
  inline BinaryOPVFGNode *getBinaryOPVFGNode(const PAGNode *pagNode) const {
    PAGNodeToBinaryOPVFGNodeMapTy::const_iterator it =
        PAGNodeToBinaryOPVFGNodeMap.find(pagNode);
  ...
  inline UnaryOPVFGNode *getUnaryOPVFGNode(const PAGNode *pagNode) const {
    PAGNodeToUnaryOPVFGNodeMapTy::const_iterator it =
        PAGNodeToUnaryOPVFGNodeMap.find(pagNode);
  ...
  inline BranchVFGNode *getBranchVFGNode(const PAGNode *pagNode) const {
    PAGNodeToBranchVFGNodeMapTy::const_iterator it =
        PAGNodeToBranchVFGNodeMap.find(pagNode);
    assert(it != PAGNodeToBranchVFGNodeMap.end() &&
           "BranchVFGNode can not be found??");
    return it->second;
  }
  inline CmpVFGNode *getCmpVFGNode(const PAGNode *pagNode) const {
    PAGNodeToCmpVFGNodeMapTy::const_iterator it =
        PAGNodeToCmpVFGNodeMap.find(pagNode);
    assert(it != PAGNodeToCmpVFGNodeMap.end() &&
           "CmpVFGNode can not be found??");
    return it->second;
  }
  inline ActualParmVFGNode *getActualParmVFGNode(const PAGNode *aparm,
                                                 const CallICFGNode *cs) const {
    PAGNodeToActualParmMapTy::const_iterator it =
        PAGNodeToActualParmMap.find(std::make_pair(aparm->getId(), cs));
    assert(it != PAGNodeToActualParmMap.end() &&
           "actual parameter VFG node can not be found??");
    return it->second;
  }
  inline ActualRetVFGNode *getActualRetVFGNode(const PAGNode *aret) const {
    PAGNodeToActualRetMapTy::const_iterator it =
        PAGNodeToActualRetMap.find(aret);
  ...
  inline FormalParmVFGNode *getFormalParmVFGNode(const PAGNode *fparm) const {
    PAGNodeToFormalParmMapTy::const_iterator it =
        PAGNodeToFormalParmMap.find(fparm);
  ...
  inline FormalRetVFGNode *getFormalRetVFGNode(const PAGNode *fret) const {
    PAGNodeToFormalRetMapTy::const_iterator it =
        PAGNodeToFormalRetMap.find(fret);
  ...
  inline bool hasBlackHoleConstObjAddrAsDef(const PAGNode *pagNode) const {
    if (hasDef(pagNode)) {
      ...
      if (const AddrVFGNode *addr = SVFUtil::dyn_cast<AddrVFGNode>(defNode)) {
        if (SVFIR::getPAG()->isBlkObjOrConstantObj(
                addr->getPAGEdge()->getSrcID()))
          return true;
      } else if (const CopyVFGNode *copy =
...
protected:
  ...
  inline void checkIntraEdgeParents(const VFGNode *srcNode,
    ...
    if (srcfun != nullptr && dstfun != nullptr) {
      assert((srcfun == dstfun) && "src and dst nodes of an intra VFG edge are "
  ...
  /// To be noted store does not create a new pointer, we do not set def for any
  /// SVFIR node
  ...
  /// So we need to make a pair <PAGNodeID,CallSiteID> to find the right
  /// VFGParmNode
  ...
  inline void addFormalParmVFGNode(const PAGNode *fparm, const SVFFunction *fun,
                                   CallPESet &callPEs) {
    FormalParmVFGNode *sNode =
        new FormalParmVFGNode(totalVFGNode++, fparm, fun);
  ...
  inline void addFormalRetVFGNode(const PAGNode *uniqueFunRet,
    ...
    FormalRetVFGNode *sNode =
        new FormalRetVFGNode(totalVFGNode++, uniqueFunRet, fun);
    ...
    /// if this uniqueFunRet is a phi node, which means it will receive values
    /// from multiple return instructions of fun we will set this phi node's def
    /// later Ideally, every function uniqueFunRet should be a PhiNode
  ...
  inline void addIntraPHIVFGNode(const MultiOpndStmt *edge) {
    IntraPHIVFGNode *sNode =
        new IntraPHIVFGNode(totalVFGNode++, edge->getRes());
    u32_t pos = 0;
  ...
  inline void addBinaryOPVFGNode(const BinaryOPStmt *edge) {
    BinaryOPVFGNode *sNode =
        new BinaryOPVFGNode(totalVFGNode++, edge->getRes());
    u32_t pos = 0;
    for (auto var : edge->getOpndVars()) {
      sNode->setOpVer(pos, var);
      pos++;
