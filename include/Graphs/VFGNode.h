...
public:
  ...
  virtual const Value *getValue() const { return nullptr; }
...
public:
  ...
  /// Whether this node is used for pointer analysis. Both src and dst PAGNodes
  /// are of ptr type.
...
public:
  ...
  BranchVFGNode(NodeID id, const BranchStmt *r)
      : VFGNode(id, Branch), brstmt(r) {}
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const BranchVFGNode *) { return true; }
  static inline bool classof(const VFGNode *node) {
  ...
  const BranchStmt *getBranchStmt() const { return brstmt; }
  /// Successors of this branch statement
  ///@{
  u32_t getNumSuccessors() const { return brstmt->getNumSuccessors(); }
...
public:
  ...
  static inline bool classof(const VFGNode *node) {
    return (node->getNodeKind() == TPhi || node->getNodeKind() == TIntraPhi ||
...
public:
  ...
  /// RetPE iterators
  inline RetPESet::const_iterator retPEBegin() const { return retPEs.begin(); }
