...
public:
  /// Constructor
  ConstraintGraph(SVFIR *p) : pag(p), edgeIndex(0) { buildCG(); }
  /// Destructor
  ...
  inline void removeConstraintNode(ConstraintNode *node) { removeGNode(node); }
  //@}
  ...
  inline NodeBS &getSubs(NodeID node) { return nodeToSubsMap[node]; }
  inline NodeID getRep(NodeID node) { return nodeToRepMap[node]; }
  inline void resetRep(NodeID node) { nodeToRepMap.erase(node); }
  //@}
  ...
  /// rep node Remove outgoing direct edges of sub node which is inside the SCC
  /// from its rep node Return TRUE if there's a gep edge inside this SCC (PWC).
