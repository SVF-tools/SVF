...
public:
  ...
  inline bool isCallDirectVFGEdge() const { return getEdgeKind() == CallDirVF; }
  ...
  inline bool isRetIndirectVFGEdge() const { return getEdgeKind() == RetIndVF; }
...
public:
  ...
  static inline bool classof(const VFGEdge *edge) {
    return edge->getEdgeKind() == IntraDirectVF ||
           edge->getEdgeKind() == CallDirVF || edge->getEdgeKind() == RetDirVF;
  }
