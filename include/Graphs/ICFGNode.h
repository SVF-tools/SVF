...
public:
  ...
  /// Set/Get methods of VFGNodes
  ///@{
  inline void addSVFStmt(const SVFStmt *edge) { pagEdges.push_back(edge); }

  inline const SVFStmtList &getSVFStmts() const { return pagEdges; }
  ///@}
...
public:
  GlobalICFGNode(NodeID id) : ICFGNode(id, GlobalBlock) { bb = nullptr; }

  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const GlobalICFGNode *) { return true; }
...
public:
  IntraICFGNode(NodeID id, const Instruction *i)
      : ICFGNode(id, IntraBlock), inst(i) {
    fun =
        LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(inst->getFunction());
    bb = inst->getParent();
  }
  ...
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const IntraICFGNode *) { return true; }
...
public:
  ...
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const InterICFGNode *) { return true; }
...
public:
  ...
  inline void addFormalParms(const SVFVar *fp) { FPNodes.push_back(fp); }

  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const FunEntryICFGNode *) { return true; }
...
public:
  ...
  inline const SVFVar *getFormalRet() const { return formalRet; }

  /// Add actual return parameter
  inline void addFormalRet(const SVFVar *fr) { formalRet = fr; }
...
public:
  CallICFGNode(NodeID id, const Instruction *c)
  ...
  /// Return callsite
  inline void setRetICFGNode(const RetICFGNode *r) { ret = r; }
  ...
  inline const ActualParmNodeVec &getActualParms() const { return APNodes; }

  /// Add actual parameters
  inline void addActualParms(const SVFVar *ap) { APNodes.push_back(ap); }

  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const CallICFGNode *) { return true; }
...
public:
  RetICFGNode(NodeID id, const Instruction *c, CallICFGNode *cb)
      : InterICFGNode(id, FunRetBlock), cs(c), actualRet(nullptr),
        callBlockNode(cb) {
    fun = LLVMModuleSet::getLLVMModuleSet()->getSVFFunction(cs->getFunction());
    bb = cs->getParent();
  }
  ...
  inline const CallICFGNode *getCallICFGNode() const { return callBlockNode; }
  /// Return actual return parameter
  inline const SVFVar *getActualRet() const { return actualRet; }

  /// Add actual return parameter
  inline void addActualRet(const SVFVar *ar) { actualRet = ar; }

  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const RetICFGNode *) { return true; }
