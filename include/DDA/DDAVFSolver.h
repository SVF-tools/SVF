...
public:
  ...
  /// Constructor
  DDAVFSolver()
      : outOfBudgetQuery(false), _pag(nullptr), _svfg(nullptr), _ander(nullptr),
        _callGraph(nullptr), _callGraphSCC(nullptr), _svfgSCC(nullptr),
        ddaStat(nullptr) {}
  /// Destructor
  virtual ~DDAVFSolver() {
    if (_ander != nullptr) {
...
protected:
        ...
        for (typename CPtSet::iterator it = storepts.begin(),
          ...
          if (propagateViaObj(*it, getLoadCVar(dpm))) {
            ...
            if (isStrongUpdate(storepts, SVFUtil::cast<StoreSVFGNode>(node))) {
              DBOUT(DDDA, SVFUtil::outs() << "backward strong update for obj "
                                          << dpm.getCurNodeID() << "\n");
  ...
  inline void startNewPTCompFromStoreDst(CPtSet &pts, const DPIm &oldDpm) {
    ...
    const SVFGEdge *edge =
        getSVFG()->getIntraVFGEdge(storeDst, store, SVFGEdge::IntraDirectVF);
    assert(edge && "Edge not found!!");
  ...
  inline void backtraceToStoreSrc(CPtSet &pts, const DPIm &oldDpm) {
    ...
    const SVFGEdge *edge =
        getSVFG()->getIntraVFGEdge(storeSrc, store, SVFGEdge::IntraDirectVF);
    assert(edge && "Edge not found!!");
    ...
    if (obj->isStack()) {
      if (const Function *fun = _pag->getGNode(id)->getFunction()) {
        ...
        return _callGraphSCC->isInCycle(
            _callGraph->getCallGraphNode(svffun)->getId());
      }
    }
    return false;
  }
  ...
  void resolveFunPtr(const DPIm &dpm) {
    if (const CallICFGNode *cbn =
      ...
      if (_pag->isIndirectCallSites(cbn)) {
        ...
        DPIm funPtrDpm(dpm);
        funPtrDpm.setLocVar(getSVFG()->getDefSVFGNode(_pag->getGNode(funPtr)),
                            funPtr);
        findPT(funPtrDpm);
      }
  ...
  /// Update call graph
  virtual inline void updateCallGraphAndSVFG(const DPIm &, const CallICFGNode *,
                                             SVFGEdgeSet &) {}
  //@}
