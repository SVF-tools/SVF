...
public:
  ...
  /// Finalize analysis
  inline virtual void finalize() override { BVDataPTAImpl::finalize(); }
  ...
  virtual bool isHeapCondMemObj(const NodeID &var,
  ...
  virtual inline void handleAddr(PointsTo &pts, const LocDPItem &dpm,
  ...
  /// processGep node
  virtual PointsTo processGepPts(const GepSVFGNode *gep,
                                 const PointsTo &srcPts) override;

  /// Update call graph.
  //@{
  virtual void updateCallGraphAndSVFG(const LocDPItem &dpm,
  ...
  /// PAGNode ID
  //@{
  virtual inline const PointsTo &
  getCachedTLPointsTo(const LocDPItem &dpm) override {
