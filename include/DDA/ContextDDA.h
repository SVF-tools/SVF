...
public:
  ...
  virtual bool handleBKCondition(CxtLocDPItem &dpm,
                                 const SVFGEdge *edge) override;
  ...
  virtual bool isHeapCondMemObj(const CxtVar &var,
  ...
  /// Update call graph.
  //@{
  virtual void updateCallGraphAndSVFG(const CxtLocDPItem &dpm,
                                      const CallICFGNode *cs,
  ...
  /// processGep node
  virtual CxtPtSet processGepPts(const GepSVFGNode *gep,
                                 const CxtPtSet &srcPts) override;

  /// Handle Address SVFGNode to add proper conditional points-to
  virtual void handleAddr(CxtPtSet &pts, const CxtLocDPItem &dpm,
  ...
  virtual inline bool isCondCompatible(const ContextCond &cxt1,
