...
public:
  ...
  enum Strategy {
    ...
    /// implementation of allocateGepObjectId). The purpose of this allocation
    /// strategy is human readability.
    DEBUG,
  };
...
public:
  /// Perform clustering given points-to sets with nodes allocated according to
  /// the DENSE strategy.
  class Clusterer {
  private:
  ...
  public:
    /// Returns vector mapping previously allocated node IDs to a smarter
    /// allocation based on the points-to sets in pta accessed through keys. The
    /// second part of the keys pairs are the number of (potential) occurrences
    /// of that points-to set or a subset, depending on the client's wish.
    /// TODO: interfaces are getting unwieldy, an initialised object may be
    /// better.
    /// TODO: kind of sucks pta can't be const here because getPts isn't.
    static std::vector<NodeID>
    cluster(BVDataPTAImpl *pta,
    ...
    static std::vector<NodeID>
    getReverseNodeMapping(const std::vector<NodeID> &nodeMapping);

    /// Fills in *NumWords statistics in stats..
    static void evaluate(const std::vector<NodeID> &nodeMap,
    ...
    static void printStats(std::string title,
                           Map<std::string, std::string> &stats);

  private:
    /// Returns an index into a condensed matrix (upper triangle, excluding
    /// diagonals) corresponding to an nxn matrix.
    static inline size_t condensedIndex(size_t n, size_t i, size_t j);

    /// Returns the minimum number of bits required to represent pts in a
    /// perfect world.
    static inline unsigned requiredBits(const PointsTo &pts);

    /// Returns the minimum number of bits required to represent n items in a
    /// perfect world.
    ...
    static inline double *getDistanceMatrix(
    ...
    /// Traverses the dendrogram produced by fastcluster, making node o, where o
    /// is the nth leaf (per recursive DFS) map to n. index is the dendrogram
    /// node to work off. The traversal should start at the top, which is the
    /// "last" (consider that it is 2D) element of the dendrogram, numObjects
    /// - 1.
    static inline void
    traverseDendrogram(std::vector<NodeID> &nodeMap, const int *dendrogram,
                       const size_t numObjects, unsigned &allocCounter,
                       Set<int> &visited, const int index,
                       const std::vector<NodeID> &regionNodeMap);

    /// Returns a vector mapping object IDs to a label such that if two objects
    /// appear in the same points-to set, they have the same label. The "appear
    /// in the same points-to set" is encoded by graph which is an adjacency
    /// list ensuring that x in pt(p) and y in pt(p) -> x is reachable from y.
    static inline std::vector<unsigned>
    regionObjects(const Map<NodeID, Set<NodeID>> &graph, size_t numObjects,
                  size_t &numLabels);

    // From all the candidates, returns the best mapping for pointsToSets
    // (points-to set -> # occurences).
    static inline std::pair<hclust_fast_methods, std::vector<NodeID>>
