...
public:
  ...
  void intersectWithComplement(const CoreBitVector &lhs,
...
public:
  ...
  public:
    ...
    operator=(const CoreBitVectorIterator &cbv) = default;
...
template <> struct Hash<CoreBitVector> {
  size_t operator()(const CoreBitVector &cbv) const { return cbv.hash(); }
};
