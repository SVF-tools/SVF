...
public:
  ...
  inline std::string toString() const {
    ...
    rawstr << "[:";
    for (CallStrCxt::const_iterator it = context.begin(), eit = context.end();
         it != eit; ++it) {
      rawstr << *it << " ";
    }
    rawstr << " ]";
...
public:
  ...
  /// Push context
  inline bool pushContext(NodeID cxt) { return this->context.pushContext(cxt); }
...
public:
  ...
  /// Overloading Operator!=
  inline bool operator!=(const CxtDPItem &rhs) const { return !(*this == rhs); }
};
} // namespace SVF
...
template <class LocCond> struct std::hash<SVF::CxtStmtDPItem<LocCond>> {
  size_t operator()(const SVF::CxtStmtDPItem<LocCond> &csdpi) const {
    SVF::Hash<
        std::pair<SVF::NodeID, std::pair<const LocCond *, SVF::ContextCond>>>
        h;
