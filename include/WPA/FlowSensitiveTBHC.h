...
public:
  ...
  /// Get PTA name
  virtual const std::string PTAName() const override { return "FSTBHC"; }
  ...
  virtual void expandFIObjs(const PointsTo &pts,
                            PointsTo &expandedPts) override;
...
protected:
  ...
  virtual void countAliases(Set<std::pair<NodeID, NodeID>> cmp,
                            unsigned *mayAliases, unsigned *noAliases) override;
