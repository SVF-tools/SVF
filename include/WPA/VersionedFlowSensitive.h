...
private:
  ...
  /// Propagates version v of o to version vp of o. time indicates whether it
  /// should record time taken itself.
  void propagateVersion(const NodeID o, const Version v, const Version vp,
                        bool time = true);
  ...
  /// Returns true if l is a delta-source node, i.e., may get a new outgoing
  /// indirect edge to a delta node due to on-the-fly callgraph construction.
  virtual bool deltaSource(const NodeID l) const;

  /// Shared code for getConsume and getYield. They wrap this function.
  Version getVersion(const NodeID l, const NodeID o,
                     const LocVersionMap &lvm) const;

  /// Returns the consumed version of o at l. If no such version exists, returns
  /// invalidVersion.
  Version getConsume(const NodeID l, const NodeID o) const;

  /// Returns the yielded version of o at l. If no such version exists, returns
  /// invalidVersion.
  Version getYield(const NodeID l, const NodeID o) const;

  /// Shared code for setConsume and setYield. They wrap this function.
  void setVersion(const NodeID l, const NodeID o, const Version v,
                  LocVersionMap &lvm);
  ...
  double versionPropTime; ///< Time to propagate versions to versions which rely
                          ///< on them.
  //@}
  ...
  public:
    ...
    static unsigned
    detectSCCs(VersionedFlowSensitive *vfs, const SVFG *svfg,
               const NodeID object,
  ...
  private:
    ...
    static void visit(VersionedFlowSensitive *vfs, const NodeID object,
                      std::vector<int> &partOf,
                      std::vector<const IndirectSVFGEdge *> &footprint,
                      std::vector<NodeData> &nodeData,
                      std::stack<const SVFGNode *> &stack, int &index,
