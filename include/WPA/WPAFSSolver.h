...
protected:
  ...
  inline bool isInCurrentSCC(NodeID node) {
    return (const_cast<NodeBS &>(this->getSCCDetector()->subNodes(curSCCID)))
        .test(node);
  }
...
protected:
  virtual void solve() {
    ...
    while (!nodeStack.empty()) {
      ...
      if (solveAll == false)
        sccNodes &= getCandidates(); /// get nodes which need to be processed in
                                     /// this SCC cycle
      ...
      removeCandidates(sccNodes); /// remove nodes which have been processed
