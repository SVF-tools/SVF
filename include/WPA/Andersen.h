...
protected:
  CallSite2DummyValPN
      callsite2DummyValPN; ///< Map an instruction to a dummy obj which created
                           ///< at an indirect callsite, which invokes a heap
                           ///< allocator
  ...
  void sanitizePts() {
    for (ConstraintGraph::iterator it = consCG->begin(), eit = consCG->end();
         it != eit; ++it) {
  ...
  virtual void addTypeForGepObjNode(NodeID, const NormalGepCGEdge *) { return; }
...
public:
  AndersenWaveDiff(SVFIR *_pag, PTATY type = AndersenWaveDiff_WPA,
...
protected:
  ...
  inline void addLCDCandidate(NodeID nodeId) { lcdCandidates.insert(nodeId); };
  //@}

  // 'metEdges' is used to collect edges met by AndersenLCD, to avoid redundant
  // visit
  //@{
  ...
  void addMetEdge(ConstraintEdge *edge) { metEdges.insert(edge->getEdgeID()); };
  //@}
...
protected:
  ...
  inline NodeID getOfflineRep(NodeID nodeId) { return oCG->getOCGRep(nodeId); }
  //@}

  // The set 'mergedNodes' is used to record the merged node, therefore avoiding
  // re-merge nodes
  //@{
