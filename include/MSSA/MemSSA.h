...
private:
  ...
  //@{
  inline void AddLoadMU(const BasicBlock *bb, const LoadStmt *load,
                        const MRSet &mrSet) {
    for (MRSet::iterator iter = mrSet.begin(), eiter = mrSet.end();
         iter != eiter; ++iter)
      AddLoadMU(bb, load, *iter);
  }
  inline void AddStoreCHI(const BasicBlock *bb, const StoreStmt *store,
                          const MRSet &mrSet) {
    for (MRSet::iterator iter = mrSet.begin(), eiter = mrSet.end();
         iter != eiter; ++iter)
...
public:
  ...
  //@{
  inline MUSet &getMUSet(const LoadStmt *ld) { return load2MuSetMap[ld]; }
  inline CHISet &getCHISet(const StoreStmt *st) { return store2ChiSetMap[st]; }
