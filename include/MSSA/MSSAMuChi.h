...
public:
  ...
  /// Get MRVERID
  inline MRVERID getID() const { return vid; }
};
...
public:
  /// Constructor/Destructor for MU
  //@{
  LoadMU(const BasicBlock *b, const LoadStmt *i, const MemRegion *m,
         Cond c = true)
  ...
  inline const LoadStmt *getLoadInst() const { return inst; }
...
public:
  ...
  /// Return callsite
  inline const CallICFGNode *getCallSite() const { return callsite; }
...
public:
  ...
  //@{
  StoreCHI(const BasicBlock *b, const StoreStmt *i, const MemRegion *m,
           Cond c = true)
  ...
  inline const StoreStmt *getStoreInst() const { return inst; }
...
public:
  ...
  virtual void dump() {
    ...
    for (OPVers::iterator it = opVers.begin(), eit = opVers.end(); it != eit;
         ++it)
      SVFUtil::outs() << "MR_" << this->getMR()->getMRID() << "V_"
