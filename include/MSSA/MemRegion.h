...
public:
  ...
  /// Return points-to
  inline const NodeBS &getPointsTo() const { return cptsSet; }
  /// Operator== overriding
  ...
  inline std::string dumpStr() const {
    std::string str;
    str += "pts{";
    for (NodeBS::iterator ii = cptsSet.begin(), ie = cptsSet.end(); ii != ie;
         ii++) {
      char int2str[16];
...
private:
  ...
  /// Map a load SVFIR Edge to its memory regions sets in order for inserting
  /// mus in Memory SSA
  LoadsToMRsMap loadsToMRsMap;
  /// Map a store SVFIR Edge to its memory regions sets in order for inserting
  /// chis in Memory SSA
...
protected:
  ...
  virtual inline void getAliasMemRegions(MRSet &aliasMRs, const NodeBS &cpts,
                                         const SVFFunction *fun) {
    for (MRSet::const_iterator it = funToMRsMap[fun].begin(),
                               eit = funToMRsMap[fun].end();
         it != eit; ++it) {
  ...
  virtual inline void getMRsForCallSiteRef(MRSet &aliasMRs, const NodeBS &cpts,
                                           const SVFFunction *) {
    const MemRegion *mr = getMR(cpts);
    aliasMRs.insert(mr);
  }
  ...
  inline void addCPtsToLoad(NodeBS &cpts, const LoadStmt *ld,
                            const SVFFunction *fun) {
  ...
  inline FunToPointsTosMap &getFunToPointsToList() { return funToPointsToMap; }
  //@}
...
public:
  inline u32_t getMRNum() const { return memRegSet.size(); }
  ...
  const SVFFunction *getFunction(const PAGEdge *pagEdge) const {
    ...
    assert(it != pagEdgeToFunMap.end() &&
           "can not find its function, it is a global SVFIR edge");
    return it->second;
  }
  /// Get Memory Region set
  //@{
  inline MRSet &getFunMRSet(const SVFFunction *fun) { return funToMRsMap[fun]; }
