...
public:
  /// Get ICFG
  inline ICFG *getICFG() const { return pag->getICFG(); }
  ...
  /// Initialization of a pointer analysis, including building symbol table and
  /// SVFIR etc.
...
public:
  ...
  inline NodeID getBaseObjVar(NodeID id) { return pag->getBaseObjVar(id); }
  inline NodeID getFIObjVar(NodeID id) { return pag->getFIObjVar(id); }
  ...
  virtual void resolveIndCalls(const CallICFGNode *cs, const PointsTo &target,
  ...
  /// Return TRUE if this edge is inside a CallGraph SCC, i.e., src node and dst
  ...
  void getVFnsFromPts(const CallICFGNode *cs, const PointsTo &target,
                      VFunSet &vfns);
  void connectVCallToVFns(const CallICFGNode *cs, const VFunSet &vfns,
                          CallEdgeMap &newEdges);
