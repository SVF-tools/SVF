...
public:
  ...
  /// local (%) and global (@) identifiers are pointer types which have a value
  /// node id.
...
protected:
  ...
  SymbolTableInfo(void)
      : mod(nullptr), modelConstants(false), totalSymNum(0), maxStruct(nullptr),
        maxStSize(0) {}
...
public:
  ...
  /// Module
  inline void setModule(SVFModule *m) { mod = m; }
  ...
  inline SymID getObjSym(const Value *val) const {
    ValueToIDMapTy::const_iterator iter =
        objSymMap.find(SVFUtil::getGlobalRep(val));
  ...
  /// Statistics
  //@{
  inline u32_t getTotalSymNum() const { return totalSymNum; }
  ...
  ///  struct A { int id; int salary; }; struct B { char name[20]; struct A a;}
  ///  B b; OriginalElemType of b with field_idx 1 : Struct A FlatternedElemType
...
protected:
  ...
  /// Create an objectInfo based on LLVM type (value is null, and type could be
  /// null, representing a dummy object)
  ObjTypeInfo *createObjTypeInfo(const Type *type);

  /// Every type T is mapped to StInfo
  /// which contains size (fsize) , offset(foffset)
  /// fsize[i] is the number of fields in the largest such struct, else fsize[i]
  /// = 1. fsize[0] is always the size of the expanded struct.
...
/*!
 * Memory object symbols or MemObj (address-taken variables in LLVM-based
 * languages)
 */
class MemObj {
...
public:
  ...
  /// Destructor
  virtual ~MemObj() { destroy(); }

  virtual const std::string toString() const;

  /// Get the reference value to this object
  inline const Value *getValue() const { return refVal; }

  /// Get the memory object id
  inline SymID getId() const { return symId; }
...
public:
  ...
  ///  struct A { int id; int salary; }; struct B { char name[20]; struct A a;}
  ///  B b; OriginalFieldType of b with field_idx 1 : Struct A
  ///  FlatternedFieldType of b with field_idx 1 : int
  //{@
  const Type *getOriginalElemType(u32_t fldIdx);

  inline std::vector<u32_t> &getFlattenedFieldIdxVec() { return fldIdxVec; }
  inline std::vector<u32_t> &getFlattenedElemIdxVec() { return elemIdxVec; }
  inline std::vector<const Type *> &getFlattenElementTypes() {
    return flattenElementTypes;
  }
  inline std::vector<const Type *> &getFlattenFieldTypes() { return finfo; }
  //@}
  ...
  inline u32_t getNumOfFlattenElements() const { return numOfFlattenElements; }

  /// Return the number of fields after flattenning (ignoring array elements)
  inline u32_t getNumOfFlattenFields() const { return numOfFlattenFields; }
  /// Return the stride
  inline u32_t getStride() const { return stride; }
};
...
public:
  ...
  /// Get max field offset limit
  inline u32_t getMaxFieldOffsetLimit() { return maxOffsetLimit; }
  ...
  /// Get the number of elements of this object
  inline u32_t getNumOfElements() const { return elemNum; }

  /// Flag for this object type
  //@{
  inline void setFlag(MEMTYPE mask) { flags |= mask; }
  inline bool hasFlag(MEMTYPE mask) { return (flags & mask) == mask; }
  //@}

  /// Object attributes
  //@{
  inline bool isFunction() { return hasFlag(FUNCTION_OBJ); }
  inline bool isGlobalObj() { return hasFlag(GLOBVAR_OBJ); }
  inline bool isStaticObj() { return hasFlag(STATIC_OBJ); }
  inline bool isStack() { return hasFlag(STACK_OBJ); }
  inline bool isHeap() { return hasFlag(HEAP_OBJ); }
  //@}

  /// Object attributes (noted that an object can be a nested compound types)
  /// e.g. both isStruct and isArray can return true
  //@{
  inline bool isVarStruct() { return hasFlag(VAR_STRUCT_OBJ); }
  inline bool isConstantStruct() { return hasFlag(CONST_STRUCT_OBJ); }
  inline bool isStruct() {
    return hasFlag(VAR_STRUCT_OBJ) || hasFlag(CONST_STRUCT_OBJ);
  }
  inline bool isVarArray() { return hasFlag(VAR_ARRAY_OBJ); }
  inline bool isConstantArray() { return hasFlag(CONST_ARRAY_OBJ); }
  inline bool isArray() {
  ...
  inline bool isConstantData() { return hasFlag(CONST_DATA); }
  inline bool hasPtrObj() { return hasFlag(HASPTR_OBJ); }
