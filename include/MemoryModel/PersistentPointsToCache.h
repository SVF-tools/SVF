...
/// Persistent points-to set store. Can be used as a backing for points-to data
/// structures like PointsToDS and PointsToDFDS. Hides points-to sets and union
/// operations from users and hands out PointsToIDs. Points-to sets are
/// interned, and union operations are lazy and hash-consed.
...
public:
  ...
  /// Resets the cache removing everything except the emptyData it was
  /// initialised with.
  void reset(void) {
    clear();
  ...
  void remapAllPts(void) {
    ...
    ptsToId.clear();
    for (PointsToID i = 0; i < idToPts.size(); ++i)
      ptsToId[*idToPts[i]] = i;
  }

  /// If pts is not in the PersistentPointsToCache, inserts it, assigns an ID,
  /// and returns that ID. If it is, then the ID is returned.
  PointsToID emplacePts(const Data &pts) {
    // Is it already in the cache?
    typename PTSToIDMap::const_iterator foundId = ptsToId.find(pts);
    if (foundId != ptsToId.end())
  ...
  PointsToID complementPts(PointsToID lhs, PointsToID rhs) {
    ...
    bool opPerformed = false;
    const PointsToID result =
        opPts(lhs, rhs, complementOp, complementCache, false, opPerformed);
  ...
  PointsToID intersectPts(PointsToID lhs, PointsToID rhs) {
    ...
    bool opPerformed = false;
    const PointsToID result =
        opPts(lhs, rhs, intersectionOp, intersectionCache, true, opPerformed);
    if (opPerformed) {
      ++uniqueIntersections;
  ...
  void printStats(const std::string subtitle) const {
    ...
    SVFUtil::outs() << std::setw(fieldWidth) << "UniquePointsToSets"
                    << idToPts.size() << "\n";

    SVFUtil::outs() << std::setw(fieldWidth) << "TotalUnions" << totalUnions
                    << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PropertyUnions"
                    << propertyUnions << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "UniqueUnions" << uniqueUnions
                    << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "LookupUnions" << lookupUnions
                    << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PreemptiveUnions"
                    << preemptiveUnions << "\n";

    SVFUtil::outs() << std::setw(fieldWidth) << "TotalComplements"
                    << totalComplements << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PropertyComplements"
                    << propertyComplements << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "UniqueComplements"
                    << uniqueComplements << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "LookupComplements"
                    << lookupComplements << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PreemptiveComplements"
                    << preemptiveComplements << "\n";

    SVFUtil::outs() << std::setw(fieldWidth) << "TotalIntersections"
                    << totalIntersections << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PropertyIntersections"
                    << propertyIntersections << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "UniqueIntersections"
                    << uniqueIntersections << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "LookupIntersections"
                    << lookupIntersections << "\n";
    SVFUtil::outs() << std::setw(fieldWidth) << "PreemptiveIntersections"
                    << preemptiveIntersections << "\n";
...
private:
  PointsToID newPointsToId(void) {
    ...
    assert(idCounter != emptyPointsToId() &&
           "PPTC::newPointsToId: PointsToIDs exhausted! Try a larger type.");
    return idCounter++;
  }

  /// Performs dataOp on lhs and rhs, checking the opCache first and updating it
  /// afterwards. commutative indicates whether the operation in question is
  /// commutative or not. opPerformed is set to true if the operation was *not*
  /// cached and thus performed, false otherwise.
  inline PointsToID opPts(PointsToID lhs, PointsToID rhs, const DataOp &dataOp,
                          OpCache &opCache, bool commutative,
                          bool &opPerformed) {
    std::pair<PointsToID, PointsToID> operands;
    // If we're commutative, we want to always perform the same operation: x op
    // y. Performing x op y sometimes and y op x other times is a waste of time.
    if (commutative)
      operands = std::minmax(lhs, rhs);
    else
