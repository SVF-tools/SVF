...
/*!
 * SVF Intermediate representation, representing variables and statements as a
 * Program Assignment Graph (PAG) Variables as nodes and statements as edges.
...
private:
  ...
  ICFGNode2SVFStmtsMap
      icfgNode2SVFStmtsMap; ///< Map an ICFGNode to its SVFStmts
  ICFGNode2SVFStmtsMap
      icfgNode2PTASVFStmtsMap; ///< Map an ICFGNode to its PointerAnalysis
                               ///< related SVFStmts
  GepValueVarMap GepValObjMap; ///< Map a pair<base,off> to a gep value node id
  NodeLocationSetMap
      GepObjVarMap; ///< Map a pair<base,off> to a gep obj node id
  MemObjToFieldsMap memToFieldsMap; ///< Map a mem object id to all its fields
  SVFStmtSet
      globSVFStmtSet;    ///< Global PAGEdges without control flow information
  PHINodeMap phiNodeMap; ///< A set of phi copy edges
  FunToArgsListMap
      funArgsListMap; ///< Map a function to a list of all its formal parameters
  CSToArgsListMap callSiteArgsListMap; ///< Map a callsite to a list of all its
                                       ///< actual parameters
  CSToRetMap
  ...
  static SVFIR *pag; ///< Singleton pattern here to enable instance of SVFIR can
                     ///< only be created once.
  CallSiteToFunPtrMap indCallSiteToFunPtrMap; ///< Map an indirect callsite to
                                              ///< its function pointer
  FunPtrToCallSitesMap funPtrToCallSitesMap;  ///< Map a function pointer to the
                                              ///< callsites where it is used
  /// Valid pointers for pointer analysis resolution connected by SVFIR edges
...
public:
  ...
  inline MemObjToFieldsMap &getMemToFieldsMap() { return memToFieldsMap; }
  /// Return GepObjVarMap
  inline NodeLocationSetMap &getGepObjNodeMap() { return GepObjVarMap; }
  /// Return ICFG
  ...
  inline SVFStmtSet &getGlobalSVFStmtSet() { return globSVFStmtSet; }
  /// Get all callsites
  ...
  //@{
  inline u32_t getFieldValNodeNum() const { return GepValObjMap.size(); }
  inline u32_t getFieldObjNodeNum() const { return GepObjVarMap.size(); }
  //@}

  /// Due to constaint expression, curInst is used to distinguish different
  /// instructions (e.g., memorycpy) when creating GepValVar.
  NodeID getGepValVar(const Value *curInst, NodeID base,
  ...
  //@{
  inline SVFStmt *getIntraPAGEdge(NodeID src, NodeID dst,
                                  SVFStmt::PEDGEK kind) {
    return getIntraPAGEdge(getGNode(src), getGNode(dst), kind);
  }
  inline SVFStmt *getIntraPAGEdge(SVFVar *src, SVFVar *dst,
                                  SVFStmt::PEDGEK kind) {
  ...
  //@{
  inline NodeID getFIObjVar(const MemObj *obj) const { return obj->getId(); }
  ...
  /// Whether an object can point to any other object or any of its fields is a
  /// pointer type.
...
private:
  ...
  inline NodeID addBlackholeObjNode() {
    return addObjNode(nullptr,
                      new DummyObjVar(getBlackHoleNode(), getBlackHoleObj()),
                      getBlackHoleNode());
  }
  ...
  inline NodeID addValNode(const Value *, SVFVar *node, NodeID i) {
    assert(hasGNode(i) == false &&
           ...
           "Strategy::DEBUG to SEQ or DENSE");
    return addNode(node, i);
  }
  /// Add a memory obj node
