...
public:
  ...
  virtual inline void dumpPTData() override { dumpPts(ptsMap); }
  ...
  virtual inline Map<DataSet, unsigned>
  getAllPts(bool liveOnly) const override {
...
protected:
  virtual inline void dumpPts(const PtsMap &ptsSet,
                              OutStream &O = SVFUtil::outs()) const {
...
public:
  ...
  virtual inline void clear() override { mutPTData.clear(); }
  ...
  virtual void remapAllPts(void) override {
    ...
    for (typename PtsMap::value_type &ppt : diffPtsMap)
      ppt.second.checkAndRemap();
    for (typename PtsMap::value_type &ppt : propaPtsMap)
      ppt.second.checkAndRemap();
  }

  virtual inline void dumpPTData() override { mutPTData.dumpPTData(); }
...
public:
  ...
  virtual void remapAllPts(void) override {
    ...
    for (typename DFPtsMap::value_type &lopt : dfInPtsMap) {
      for (typename PtsMap::value_type &opt : lopt.second)
...
public:
  ...
  virtual inline void dumpPTData() override {
    ...
    if (f.good()) {
      NodeBS locs;
      for (DFPtsMapconstIter it = dfInPtsMap.begin(), eit = dfInPtsMap.end();
           it != eit; ++it)
        locs.set(it->first);

      for (DFPtsMapconstIter it = dfOutPtsMap.begin(), eit = dfOutPtsMap.end();
           it != eit; ++it)
        locs.set(it->first);

      for (NodeBS::iterator it = locs.begin(), eit = locs.end(); it != eit;
           it++) {
        LocID loc = *it;
        if (this->hasDFInSet(loc)) {
          f << "Loc:" << loc << " IN:{";
...
public:
  ...
  typedef typename BaseDFPTData::LocID LocID;
  typedef Map<LocID, DataSet> UpdatedVarMap; ///< for propagating only newly
...
public:
  /// Constructor
  MutableIncDFPTData(bool reversePT = true,
  ...
  virtual inline bool updateAllDFOutFromIn(LocID loc, const Key &singleton,
    ...
    if (this->hasDFInSet(loc)) {
      ...
      for (DataIter ptsIt = pts.begin(), ptsEit = pts.end(); ptsIt != ptsEit;
           ++ptsIt) {
        const Key var = *ptsIt;
        /// Enable strong updates if it is required to do so
        if (strongUpdates && var == singleton)
  ...
  virtual inline bool updateTLVPts(LocID srcLoc, const Key &srcVar,
    ...
    if (varHasNewDFInPts(srcLoc, srcVar)) {
      removeVarFromDFInUpdatedSet(srcLoc, srcVar);
      return this->mutPTData.unionPts(dstVar,
                                      this->getDFInPtsSet(srcLoc, srcVar));
    }
    return false;
  }
  ...
  ///@{
  static inline bool
  classof(const MutableIncDFPTData<Key, KeySet, Data, DataSet> *) {
    return true;
  }
...
public:
  ...
  getVersionedKeyRevPts(const Data &datum) override {
  ...
  ///@{
  static inline bool
  classof(const MutableVersionedPTData<Key, KeySet, Data, DataSet, VersionedKey,
                                       VersionedKeySet> *) {
    return true;
  }
