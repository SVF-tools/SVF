...
public:
  ...
  /// Put intersection of lhs with complemenet of rhs into this set
  /// (overwrites).
  ...
  static void
  setCurrentBestNodeMapping(MappingPtr newCurrentBestNodeMapping,
...
template <> struct std::hash<SVF::PointsTo> {
  size_t operator()(const SVF::PointsTo &pt) const { return pt.hash(); }
};
