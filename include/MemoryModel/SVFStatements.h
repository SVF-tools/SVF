...
public:
  ...
  /// ClassOf
  //@{
  static inline bool classof(const SVFStmt *) { return true; }
  ...
  static inline GEdgeFlag makeEdgeFlagWithAddionalOpnd(GEdgeKind k,
    ...
    u64_t label =
        (iter != var2LabelMap.end()) ? iter->second : multiOpndLabelCounter++;
  ...
  static inline GEdgeFlag makeEdgeFlagWithCallInst(GEdgeKind k,
    ...
    u64_t label =
        (iter != inst2LabelMap.end()) ? iter->second : callEdgeLabelCounter++;
    return (label << EdgeKindMaskBits) | k;
  }
  ...
  static inline GEdgeFlag makeEdgeFlagWithStoreInst(GEdgeKind k,
    ...
    u64_t label =
        (iter != inst2LabelMap.end()) ? iter->second : storeEdgeLabelCounter++;
    return (label << EdgeKindMaskBits) | k;
  }
...
private:
  ...
  static Var2LabelMap
      var2LabelMap; ///< Second operand of MultiOpndStmt to label map
...
/*
 Parent class of Addr, Copy, Store, Load, Call, Ret, NormalGep, VariantGep,
 ThreadFork, ThreadJoin connecting RHS expression and LHS expression with an
 assignment  (e.g., LHSExpr = RHSExpr) Only one operand on the right handside of
 an assignment
*/
...
public:
  ...
  //@{
  static inline bool classof(const AssignStmt *) { return true; }
  ...
  inline SVFVar *getRHSVar() const { return SVFStmt::getSrcNode(); }
  inline SVFVar *getLHSVar() const { return SVFStmt::getDstNode(); }
  inline NodeID getRHSVarID() const { return SVFStmt::getSrcID(); }
  inline NodeID getLHSVarID() const { return SVFStmt::getDstID(); }
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const AddrStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
    return edge->getEdgeKind() == SVFStmt::Addr;
  }
  static inline bool classof(const GenericPAGEdgeTy *edge) {
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const CopyStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const StoreStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const LoadStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
...
private:
  ...
  bool variantField; ///< Gep statement with a variant field index (pointer
                     ///< arithmetic) for struct field access (e.g., p = &(q +
                     ///< f), where f is a variable)
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const GepStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  /// Return accumulated constant offset (when accessing array or struct) if
  /// this offset is a constant.
  ...
  /// Gep statement with a variant field index (pointer arithmetic) for struct
  /// field access
  inline bool isVariantFieldGep() const { return variantField; }
...
public:
  ...
  /// constructor
  CallPE(SVFVar *s, SVFVar *d, const CallICFGNode *i, const FunEntryICFGNode *e,
         GEdgeKind k = SVFStmt::Call)
      : AssignStmt(s, d, makeEdgeFlagWithCallInst(k, i)), call(i), entry(e) {}

  /// Get method for the call instruction
  //@{
  inline const CallICFGNode *getCallInst() const { return call; }
  inline const CallICFGNode *getCallSite() const { return call; }
  inline const FunEntryICFGNode *getFunEntryICFGNode() const { return entry; }
  //@}
...
public:
  ...
  inline const CallICFGNode *getCallInst() const { return call; }
  inline const CallICFGNode *getCallSite() const { return call; }
  inline const FunExitICFGNode *getFunExitICFGNode() const { return exit; }
  //@}
...
/*
 * Program statements with multiple operands including BinaryOPStmt, CmpStmt and
 * PhiStmt
 */
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const MultiOpndStmt *) { return true; }
  ...
  //@}
  /// Operands and result at a BinaryNode e.g., p = q + r, `p` is resVar and `r`
  /// is OpVar
  //@{
  /// Operand SVFVars
  inline const SVFVar *getOpVar(u32_t pos) const { return opVars.at(pos); }
  /// Result SVFVar
  inline const SVFVar *getRes() const { return SVFStmt::getDstNode(); }

  NodeID getOpVarID(u32_t pos) const;
  NodeID getResID() const;

  inline u32_t getOpVarNum() const { return opVars.size(); }
  inline const OPVars &getOpndVars() const { return opVars; }
  inline OPVars::const_iterator opVarBegin() const { return opVars.begin(); }
  inline OPVars::const_iterator opVerEnd() const { return opVars.end(); }
  //@}
};

/*!
 * Phi statement (e.g., p = phi(q,r) which receives values from variables q and
 * r from different paths) it is typically at a joint point of the control-flow
 * graph
 */
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const PhiStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
...
/*!
 * Select statement (e.g., p ? q: r which receives values from variables q and r
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const SelectStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  SelectStmt(SVFVar *s, const OPVars &opnds, const SVFVar *cond)
      : MultiOpndStmt(s, opnds, SVFStmt::Select), condition(cond) {
    assert(opnds.size() == 2 && "SelectStmt can only have two operands!");
  }
  virtual const std::string toString() const override;

  inline const SVFVar *getCondition() const { return condition; }
  inline const SVFVar *getTrueValue() const { return getOpVar(0); }
  inline const SVFVar *getFalseValue() const { return getOpVar(1); }
};
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const CmpStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  CmpStmt(SVFVar *s, const OPVars &opnds, u32_t pre)
      : MultiOpndStmt(s, opnds, SVFStmt::Cmp), predicate(pre) {}

  u32_t getPredicate() const { return predicate; }
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const BinaryOPStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  BinaryOPStmt(SVFVar *s, const OPVars &opnds, u32_t oc)
      : MultiOpndStmt(s, opnds, SVFStmt::BinaryOp), opcode(oc) {}

  u32_t getOpcode() const { return opcode; }
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const UnaryOPStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  u32_t getOpcode() const { return opcode; }
  inline const SVFVar *getOpVar() const { return SVFStmt::getSrcNode(); }
  inline const SVFVar *getRes() const { return SVFStmt::getDstNode(); }
...
public:
  /// Methods for support type inquiry through isa, cast, and dyn_cast:
  //@{
  static inline bool classof(const BranchStmt *) { return true; }
  static inline bool classof(const SVFStmt *edge) {
  ...
  const SVFVar *getBranchInst() const { return brInst; }
  ...
  ///@{
  u32_t getNumSuccessors() const { return successors.size(); }
  const SuccAndCondPairVec &getSuccessors() const { return successors; }
  const ICFGNode *getSuccessor(u32_t i) const { return successors.at(i).first; }
  s32_t getSuccessorCondValue(u32_t i) const { return successors.at(i).second; }
  //@}
  virtual const std::string toString() const override;
};
