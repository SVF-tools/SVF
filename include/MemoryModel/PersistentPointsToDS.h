...
/// Each Key is given a cheap points-to ID which refers to some real points-to
/// set.
...
class PersistentPTData : public PTData<Key, KeySet, Data, DataSet> {
  template <typename K, typename KS, typename D, typename DS, typename VK,
            typename VKS>
...
public:
  ...
  /// Constructor
  PersistentPTData(PersistentPointsToCache<DataSet> &cache,
                   bool reversePT = true, PTDataTy ty = PTDataTy::PersBase)
  ...
  virtual inline bool unionPts(const Key &dstKey,
                               const DataSet &srcData) override {
  ...
  virtual void remapAllPts(void) override { ptCache.remapAllPts(); }
  ...
  ///@{
  static inline bool
  classof(const PersistentPTData<Key, KeySet, Data, DataSet> *) {
    return true;
  }
...
private:
  ...
  inline bool unionPtsFromId(const Key &dstKey, PointsToID srcId) {
    ...
    if (changed) {
      ...
      if (this->rev) {
        ...
        for (const Data &d : srcPts)
          SVFUtil::insertKey(dstKey, revPtsMap[d]);
      }
    }
  ...
  inline void clearRevPts(const DataSet &pts, const Key &k) {
    if (this->rev) {
      for (const Data &d : pts)
        clearSingleRevPts(revPtsMap[d], k);
    }
  }
...
public:
  ...
  /// Constructor
  PersistentDiffPTData(PersistentPointsToCache<DataSet> &cache,
                       bool reversePT = true, PTDataTy ty = PTDataTy::PersDiff)
      : BaseDiffPTData(reversePT, ty), ptCache(cache),
        persPTData(cache, reversePT) {}
  ...
  virtual inline const KeySet &getRevPts(const Data &data) override {
    assert(this->rev && "PersistentDiffPTData::getRevPts: constructed without "
                        "reverse PT support!");
  ...
  virtual inline bool computeDiffPts(Key &var, const DataSet &all) override {
    ...
    // Diff is made up of the entire points-to set minus what has been
    // propagated.
  ...
  ///@{
  static inline bool
  classof(const PersistentDiffPTData<Key, KeySet, Data, DataSet> *) {
    return true;
  }
...
private:
  PersistentPointsToCache<DataSet> &ptCache;
  /// Backing to implement basic PTData methods. Allows us to avoid multiple
  /// inheritance.
...
public:
  ...
  PersistentDFPTData(PersistentPointsToCache<DataSet> &cache,
                     bool reversePT = true,
                     PTDataTy ty = PTDataTy::PersDataFlow)
      : BaseDFPTData(reversePT, ty), ptCache(cache),
        persPTData(cache, reversePT) {}
  ...
  virtual inline void dumpPTData() override { persPTData.dumpPTData(); }
  ...
  virtual bool hasDFInSet(LocID loc, const Key &var) const override {
    ...
    if (foundInKeyToId == dfInPtsMap.end())
      return false;
  ...
  virtual bool hasDFOutSet(LocID loc, const Key &var) const override {
    typename DFKeyToIDMap::const_iterator foundOutKeyToId =
        dfOutPtsMap.find(loc);
    if (foundOutKeyToId == dfOutPtsMap.end())
      return false;
  ...
  virtual bool updateDFInFromIn(LocID srcLoc, const Key &srcVar, LocID dstLoc,
    ...
    return unionPtsThroughIds(getDFInPtIdRef(dstLoc, dstVar),
  ...
  virtual bool updateDFInFromOut(LocID srcLoc, const Key &srcVar, LocID dstLoc,
                                 const Key &dstVar) override {
    return unionPtsThroughIds(getDFInPtIdRef(dstLoc, dstVar),
                              getDFOutPtIdRef(srcLoc, srcVar));
  }
  ...
  virtual bool updateDFOutFromIn(LocID srcLoc, const Key &srcVar, LocID dstLoc,
                                 const Key &dstVar) override {
  ...
  virtual bool updateTLVPts(LocID srcLoc, const Key &srcVar,
                            const Key &dstVar) override {
    return unionPtsThroughIds(persPTData.ptsMap[dstVar],
                              getDFInPtIdRef(srcLoc, srcVar));
  }
  ...
  virtual Map<DataSet, unsigned> getAllPts(bool liveOnly) const override {
    ...
    if (!liveOnly) {
      ...
      // In essence, we want the ptCache.getAllPts() to just add the unused,
      // non-GC'd points-to sets to allPts.
      for (typename Map<DataSet, unsigned>::value_type pto : allPts)
        pto.second -= 1;
  ...
  ///@{
  static inline bool
  classof(const PersistentDFPTData<Key, KeySet, Data, DataSet> *) {
    return true;
  }
...
protected:
  ...
  /// PTData for top-level pointers. We will also use its cache for
  /// address-taken pointers.
  PersistentPTData<Key, KeySet, Data, DataSet> persPTData;
...
template <typename Key, typename KeySet, typename Data, typename DataSet>
class PersistentIncDFPTData
    : public PersistentDFPTData<Key, KeySet, Data, DataSet> {
public:
  typedef PTData<Key, KeySet, Data, DataSet> BasePTData;
  typedef PersistentPTData<Key, KeySet, Data, DataSet> BasePersPTData;
  typedef DFPTData<Key, KeySet, Data, DataSet> BaseDFPTData;
...
public:
  /// Constructor
  PersistentIncDFPTData(PersistentPointsToCache<DataSet> &cache,
                        bool reversePT = true,
  ...
  virtual inline bool updateDFInFromIn(LocID srcLoc, const Key &srcVar,
                                       LocID dstLoc,
                                       const Key &dstVar) override {
    if (varHasNewDFInPts(srcLoc, srcVar) &&
        this->unionPtsThroughIds(this->getDFInPtIdRef(dstLoc, dstVar),
  ...
  virtual inline bool updateDFInFromOut(LocID srcLoc, const Key &srcVar,
                                        LocID dstLoc,
                                        const Key &dstVar) override {
    if (varHasNewDFOutPts(srcLoc, srcVar) &&
        this->unionPtsThroughIds(this->getDFInPtIdRef(dstLoc, dstVar),
                                 this->getDFOutPtIdRef(srcLoc, srcVar))) {
      setVarDFInSetUpdated(dstLoc, dstVar);
      return true;
    }
  ...
  virtual inline bool updateDFOutFromIn(LocID srcLoc, const Key &srcVar,
                                        LocID dstLoc,
                                        const Key &dstVar) override {
    if (varHasNewDFInPts(srcLoc, srcVar)) {
  ...
  virtual inline bool updateAllDFInFromOut(LocID srcLoc, const Key &srcVar,
                                           LocID dstLoc,
                                           const Key &dstVar) override {
    if (this->unionPtsThroughIds(this->getDFInPtIdRef(dstLoc, dstVar),
                                 this->getDFOutPtIdRef(srcLoc, srcVar))) {
      setVarDFInSetUpdated(dstLoc, dstVar);
      return true;
    }
  ...
  virtual inline bool updateAllDFInFromIn(LocID srcLoc, const Key &srcVar,
                                          LocID dstLoc,
                                          const Key &dstVar) override {
    if (this->unionPtsThroughIds(this->getDFInPtIdRef(dstLoc, dstVar),
                                 this->getDFInPtIdRef(srcLoc, srcVar))) {
  ...
  virtual inline bool updateAllDFOutFromIn(LocID loc, const Key &singleton,
                                           bool strongUpdates) override {
    bool changed = false;
    if (this->hasDFInSet(loc)) {
  ...
  virtual inline bool updateTLVPts(LocID srcLoc, const Key &srcVar,
                                   const Key &dstVar) override {
    if (varHasNewDFInPts(srcLoc, srcVar)) {
      removeVarFromDFInUpdatedSet(srcLoc, srcVar);
      return this->unionPtsThroughIds(this->persPTData.ptsMap[dstVar],
                                      this->getDFInPtIdRef(srcLoc, srcVar));
    }
  ...
  virtual inline bool updateATVPts(const Key &srcVar, LocID dstLoc,
                                   const Key &dstVar) override {
    if (this->unionPtsThroughIds(this->getDFOutPtIdRef(dstLoc, dstVar),
                                 this->persPTData.ptsMap[srcVar])) {
      setVarDFOutSetUpdated(dstLoc, dstVar);
      return true;
    }
  ...
  ///@{
  static inline bool
  classof(const PersistentIncDFPTData<Key, KeySet, Data, DataSet> *) {
    return true;
  }
...
private:
  ...
  ///@{
  /// Add var into loc's OUT updated set. Called when var's pts in loc's OUT set
  /// changed
...
public:
  typedef PTData<Key, KeySet, Data, DataSet> BasePTData;
  typedef VersionedPTData<Key, KeySet, Data, DataSet, VersionedKey,
                          VersionedKeySet>
      BaseVersionedPTData;
  typedef typename BasePTData::PTDataTy PTDataTy;

  typedef typename PersistentPTData<Key, KeySet, Data, DataSet>::KeyToIDMap
      KeyToIDMap;
  typedef typename PersistentPTData<VersionedKey, VersionedKeySet, Data,
                                    DataSet>::KeyToIDMap VersionedKeyToIDMap;

  PersistentVersionedPTData(PersistentPointsToCache<DataSet> &cache,
                            bool reversePT = true,
                            PTDataTy ty = PTDataTy::PersVersioned)
      : BaseVersionedPTData(reversePT, ty), tlPTData(cache, reversePT),
        atPTData(cache, reversePT) {}
  ...
  virtual const KeySet &getRevPts(const Data &data) override {
    assert(this->rev && "PersistentVersionedPTData::getRevPts: constructed "
                        "without reverse PT support!");
    return tlPTData.getRevPts(data);
  }
  virtual const VersionedKeySet &
  getVersionedKeyRevPts(const Data &data) override {
    assert(this->rev && "PersistentVersionedPTData::getVersionedKeyRevPts: "
  ...
  virtual bool unionPts(const VersionedKey &dstVar,
                        const Key &srcVar) override {
    return atPTData.unionPtsFromId(dstVar, tlPTData.ptsMap[srcVar]);
  }
  virtual bool unionPts(const Key &dstVar,
                        const VersionedKey &srcVar) override {
  ...
  virtual void clearFullPts(const Key &k) override { tlPTData.clearFullPts(k); }
  ...
  ///@{
  static inline bool
  classof(const PersistentVersionedPTData<Key, KeySet, Data, DataSet,
