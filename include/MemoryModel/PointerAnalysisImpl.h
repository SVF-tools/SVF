...
public:
  ...
  typedef VersionedPTData<NodeID, NodeSet, NodeID, PointsTo, VersionedVar,
                          Set<VersionedVar>>
      VersionedPTDataTy;
  ...
  typedef MutableIncDFPTData<NodeID, NodeSet, NodeID, PointsTo>
      MutIncDFPTDataTy;
  typedef MutableVersionedPTData<NodeID, NodeSet, NodeID, PointsTo,
                                 VersionedVar, Set<VersionedVar>>
      MutVersionedPTDataTy;

  typedef PersistentPTData<NodeID, NodeSet, NodeID, PointsTo> PersPTDataTy;
  typedef PersistentDiffPTData<NodeID, NodeSet, NodeID, PointsTo>
      PersDiffPTDataTy;
  typedef PersistentDFPTData<NodeID, NodeSet, NodeID, PointsTo> PersDFPTDataTy;
  typedef PersistentIncDFPTData<NodeID, NodeSet, NodeID, PointsTo>
      PersIncDFPTDataTy;
  typedef PersistentVersionedPTData<NodeID, NodeSet, NodeID, PointsTo,
                                    VersionedVar, Set<VersionedVar>>
  ...
  /// Constructor
  BVDataPTAImpl(SVFIR *pag, PointerAnalysis::PTATY type,
                bool alias_check = true);

  /// Destructor
  virtual ~BVDataPTAImpl() { destroy(); }

  inline PersistentPointsToCache<PointsTo> &getPtCache(void) { return ptCache; }
  ...
  ///@{
  virtual inline const PointsTo &getPts(NodeID id) { return ptD->getPts(id); }
  ...
  /// To be noted that adding reverse pts might incur 10% total overhead during
  /// solving
  //@{
...
public:
  ...
  typedef Map<NodeID, PointsTo>
      PtrToBVPtsMap; /// map a pointer to its BitVector points-to representation
  ...
  void expandFIObjs(const CPtSet &cpts, CPtSet &expandedCpts) {
    expandedCpts = cpts;
    ;
    for (typename CPtSet::const_iterator cit = cpts.begin(), ecit = cpts.end();
         cit != ecit; ++cit) {
...
protected:
  ...
  /// purpose To be noted that adding reverse pts might incur 10% total overhead
  /// during solving
  //@{
  ...
  virtual inline bool addPts(CVar id, CVar ptd) { return ptD->addPts(id, ptd); }
  //@}
  ...
  virtual void normalizePointsTo() {
    ...
    if (hasPtsMap()) {
      ...
      for (typename MutPTDataTy::PtsMap::const_iterator it = ptsMap.begin(),
                                                        eit = ptsMap.end();
           it != eit; ++it) {
      ...
      assert(
          false &&
          "CondPTAImpl::NormalizePointsTo: could not normalize points-to sets");
    }
  }
  /// Points-to data
  PTDataTy *ptD;
...
public:
  ...
  virtual inline PointsTo &getPts(NodeID ptr) {
    assert(normalized && "Pts of all context-var have to be merged/normalized. "
                         "Want to use getPts(CVar cvar)??");
  ...
  virtual inline const CPtSet &getCondPointsTo(NodeID ptr) {
    assert(normalized && "Pts of all context-vars have to be "
                         "merged/normalized. Want to use getPts(CVar cvar)??");
