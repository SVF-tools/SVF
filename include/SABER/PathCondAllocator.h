...
public:
  ...
  typedef Map<const BasicBlock *, CondPosMap>
      BBCondMap; // map bb to a Condition
  typedef Set<const BasicBlock *> BasicBlockSet;
  typedef Map<const Function *, BasicBlockSet>
      FunToExitBBsMap; ///< map a function to all its basic blocks calling
                       ///< program exit
  typedef Map<const BasicBlock *, Condition *>
      BBToCondMap; ///< map a basic block to its condition during control-flow
                   ///< guard computation
  typedef FIFOWorkList<const BasicBlock *>
  ...
  /// Statistics
  //@{
  inline std::string getMemUsage() { return condMgr->getMemUsage(); }
  inline u32_t getCondNum() { return condMgr->getCondNumber(); }
  //@}
  ...
  inline Condition *condNeg(Condition *cond) { return condMgr->NEG(cond); }
  inline Condition *getTrueCond() const { return condMgr->getTrueCond(); }
  inline Condition *getFalseCond() const { return condMgr->getFalseCond(); }
  ...
  inline Condition *getCond(u32_t i) const { return condMgr->getCond(i); }
  ...
  /// Get complement condition (from B1 to B0) according to a complementBB (BB2)
  /// at a phi e.g., B0: dstBB; B1:incomingBB; B2:complementBB
  ...
  inline const SVFGNode *getCurEvalSVFGNode() const { return curEvalSVFGNode; }
  //@}
  ...
  inline ICFG *getICFG() const { return PAG::getPAG()->getICFG(); }
...
private:
  ...
  Condition *evaluateTestNullLikeExpr(const BranchStmt *branchStmt,
                                      const BasicBlock *succ);
  /// Return condition when there is a branch calls program exit
  Condition *evaluateProgExit(const BranchStmt *branchStmt,
                              const BasicBlock *succ);
  ...
  BBToCondMap bbToCondMap; ///< map a basic block to its path condition starting
                           ///< from root
  const SVFGNode *curEvalSVFGNode{}; ///< current llvm value to evaluate branch
                                     ///< condition when computing guards

protected:
  BDDExprManager *condMgr; ///< z3 manager
  BBCondMap bbConds; ///< map basic block to its successors/predecessors branch
                     ///< conditions
};
