...
public:
  ...
  u32_t getConstantFieldIdx(void) const { return offset; }

  void setOffset(u32_t offset) { this->offset = offset; }
...
public:
  ...
  inline bool isMultiInheritance() const { return hasFlag(MULTI_INHERITANCE); }
  ...
  const Set<const DIDerivedType *> &getTypedefs(void) const { return typedefs; }
...
private:
  ...
  /// inheritance, 0 would be the primary base + this classes virtual functions,
  /// 1 would be the second parent, 2 would be third parent, etc.
