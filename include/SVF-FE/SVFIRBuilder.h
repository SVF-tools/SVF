...
private:
  SVFIR *pag;
  SVFModule *svfMod;
  const BasicBlock *curBB; ///< Current basic block during SVFIR construction
                           ///< when visiting the module
  const Value *curVal;     ///< Current Value during SVFIR construction when
                           ///< visiting the module

public:
  /// Constructor
  SVFIRBuilder()
      : pag(SVFIR::getPAG()), svfMod(nullptr), curBB(nullptr), curVal(nullptr) {
  }
  /// Destructor
  ...
  void addEdge(NodeID src, NodeID dst, SVFStmt::PEDGEK kind, s32_t offset = 0,
  ...
  /// GetObject - Return the object node (stack/global/heap/function) according
  /// to a LLVM Value
  inline NodeID getObjectNode(const Value *V) { return pag->getObjectNode(V); }

  /// getReturnNode - Return the node representing the unique return value of a
  /// function.
  inline NodeID getReturnNode(const SVFFunction *func) {
    return pag->getReturnNode(func);
  }

  /// getVarargNode - Return the node representing the unique variadic argument
  /// of a function.
  inline NodeID getVarargNode(const SVFFunction *func) {
    return pag->getVarargNode(func);
  }
  //@}
  /// Update SVFIR given a call graph (creating new CallPE and RetPE for
  ...
  /// If <val> is undef or poison, ‘freeze’ returns an arbitrary, but fixed
...
protected:
  ...
  u32_t inferFieldIdxFromByteOffset(const llvm::GEPOperator *gepOp,
                                    DataLayout *dl, LocationSet &ls, s32_t idx);

  /// Compute offset of a gep instruction or gep constant expression
  bool computeGepOffset(const User *V, LocationSet &ls);

  /// Get the base value of (i8* src and i8* dst) for external argument (e.g.
  /// memcpy(i8* dst, i8* src, int size))
  ...
  NodeID getGepValVar(const Value *val, const LocationSet &ls,
                      const Type *baseType);
  ...
  /// Add Copy edge
  inline void addBinaryOPEdge(NodeID op1, NodeID op2, NodeID dst,
                              u32_t opcode) {
  ...
  inline void addBranchStmt(NodeID br, NodeID cond,
  ...
  /// Add Call edge
  inline void addCallEdge(NodeID src, NodeID dst, const CallICFGNode *cs,
                          const FunEntryICFGNode *entry) {
    if (CallPE *edge = pag->addCallPE(src, dst, cs, entry))
      setCurrentBBAndValueForPAGEdge(edge);
  }
  /// Add Return edge
  inline void addRetEdge(NodeID src, NodeID dst, const CallICFGNode *cs,
                         const FunExitICFGNode *exit) {
    if (RetPE *edge = pag->addRetPE(src, dst, cs, exit))
      setCurrentBBAndValueForPAGEdge(edge);
  }
  /// Add Gep edge
  inline void addGepEdge(NodeID src, NodeID dst, const LocationSet &ls,
                         bool constGep) {
  ...
  inline void addThreadForkEdge(NodeID src, NodeID dst, const CallICFGNode *cs,
                                const FunEntryICFGNode *entry) {
    if (TDForkPE *edge = pag->addThreadForkPE(src, dst, cs, entry))
      setCurrentBBAndValueForPAGEdge(edge);
  }
  /// Add Thread join edge for parameter passing
  inline void addThreadJoinEdge(NodeID src, NodeID dst, const CallICFGNode *cs,
                                const FunExitICFGNode *exit) {
