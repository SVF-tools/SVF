...
class IRAnnotator {
public:
  IRAnnotator() : mainModule(nullptr), ptsTo(nullptr) {}
  ~IRAnnotator() {}

  void processAndersenResults(SVFIR *pag, BVDataPTAImpl *ptsTo,
                              bool writeFlag) {
    this->ptsTo = ptsTo;
...
private:
  // Write the PAGgepNode to the IR such that metadata name is the SVFIR node id
  // and the operands are its base node's id and location offset
  void writePAGgepNode(SVF::NodeID nodeId, GepObjVar *gepNode) {
    ...
    operands.push_back(
        llvm::MDString::get(context, std::to_string(baseNodeId)));
    operands.push_back(
        llvm::MDString::get(context, std::to_string(locationSetOffset)));
  ...
  // Reads the PAGGepNodes in the annotated IR and creates a new SVFIR Node
  // based on the data contained in the operands of the metadata node
  void readPAGgepNodes(SVFIR *pag) {
    for (auto it = mainModule->named_metadata_begin();
         it != mainModule->named_metadata_end(); ++it) {
      ...
      SVF::NodeID baseNodeId = std::stoi(
          llvm::cast<llvm::MDString>(mdNode->getOperand(0))->getString().str());
      SVF::s32_t locationSetOffset = std::stoi(
          llvm::cast<llvm::MDString>(mdNode->getOperand(1))->getString().str());
  ...
  void processPAGNode(const Value *value, SVF::NodeID nodeId, bool writeFlag) {
    if (auto instruction =
            const_cast<Instruction *>(SVFUtil::dyn_cast<Instruction>(value))) {
      processInstructionNode(instruction, nodeId, writeFlag);
    } else if (auto argument =
                   const_cast<Argument *>(SVFUtil::dyn_cast<Argument>(value))) {
      processArgumentNode(argument, nodeId, writeFlag);
    } else if (auto function =
                   const_cast<Function *>(SVFUtil::dyn_cast<Function>(value))) {
      processFunctionNode(function, nodeId, writeFlag);
    } else if (auto globalVar = const_cast<GlobalVariable *>(
                   SVFUtil::dyn_cast<GlobalVariable>(value))) {
      processGlobalVarNode(globalVar, nodeId, writeFlag);
    } else if (auto basicBlock = const_cast<BasicBlock *>(
                   SVFUtil::dyn_cast<BasicBlock>(value))) {
      processBasicBlockNode(basicBlock, nodeId, writeFlag);
    } else if (SVFUtil::isa<Constant>(value)) {
      processConstantNode(nodeId, writeFlag);
    } else if (auto inlineAsm = const_cast<llvm::InlineAsm *>(
                   SVFUtil::dyn_cast<llvm::InlineAsm>(value))) {
      processInlineAsmNode(inlineAsm, nodeId, writeFlag);
    } else {
      SVFUtil::outs() << "Value is NOT a Instruction, Argument, Function, "
                         "GlobalVariable, BasicBlock, Constant or InlineAsm"
                      << std::endl;
      SVFUtil::outs() << SVFUtil::value2String(value) << "\n";
    }
  }

  void processInstructionNode(Instruction *instruction,
                              const SVF::NodeID &nodeId, bool writeFlag) {
    ...
    if (writeFlag) {
      ...
      assert(mdNode != nullptr &&
             "Failed to retrive valid node id at instruction");
      addAndersenMetadata(nodeId, mdNode);
    }
  }

  void processArgumentNode(Argument *argument, const SVF::NodeID &nodeId,
                           bool writeFlag) {
    ...
    if (writeFlag) {
      ...
      assert(mdNode != nullptr &&
             "Failed to retrive valid node id at argument");
      addAndersenMetadata(nodeId, mdNode);
    }
  }

  void processFunctionNode(Function *function, const SVF::NodeID &nodeId,
                           bool writeFlag) {
    ...
    if (writeFlag) {
      ...
      assert(mdNode != nullptr &&
             "Failed to retrive valid node id at function");
      addAndersenMetadata(nodeId, mdNode);
    }
  }

  void processGlobalVarNode(GlobalVariable *globalVar,
                            const SVF::NodeID &nodeId, bool writeFlag) {
  ...
  void processBasicBlockNode(BasicBlock *basicBlock, const SVF::NodeID &nodeId,
                             bool writeFlag) {
    ...
    if (writeFlag) {
      ...
      assert(mdNode != nullptr &&
             "Failed to retrive valid node id at basicBlock");
      addAndersenMetadata(nodeId, mdNode);
    }
  }
  ...
  void processInlineAsmNode(llvm::InlineAsm *inlineAsm,
                            const SVF::NodeID &nodeId, bool writeFlag) {
    ...
    if (writeFlag) {
      ...
      auto mdNode = mainModule->getNamedMetadata(label);
      assert(mdNode != nullptr &&
             "Failed to retrive valid node id at inlineAsm");
      addAndersenNamedMetdata(nodeId, mdNode);
    }
  }
  ...
  void addAndersenMetadata(const SVF::NodeID &nodeId, MDNode *mdNode) {
    if (mdNode) {
      for (unsigned int i = 0; i < mdNode->getNumOperands(); i++) {
        auto anderNodeIdStr = llvm::cast<llvm::MDString>(mdNode->getOperand(i))
                                  ->getString()
                                  .str();
    ...
    if (mdNode) {
      for (unsigned int i = 0; i < mdNode->getNumOperands(); i++) {
        ...
        for (unsigned int j = 0; j < node->getNumOperands(); j++) {
          auto anderNodeIdStr = llvm::cast<llvm::MDString>(node->getOperand(j))
                                    ->getString()
                                    .str();
          auto anderNodeId = SVF::NodeID(std::stoul(anderNodeIdStr));
          ptsTo->addPts(nodeId, anderNodeId);
  ...
  inline llvm::MDTuple *getMdNodePts(const SVF::NodeID &nodeId,
                                     LLVMContext &context) {
    llvm::SmallVector<llvm::Metadata *, 32> operands;
