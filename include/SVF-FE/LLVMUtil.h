...
inline bool isConstantData(const Value *val) {
  return SVFUtil::isa<ConstantData>(val) ||
         ...
         SVFUtil::isa<MetadataAsValue>(val) || SVFUtil::isa<BlockAddress>(val);
}
...
inline const SVFFunction *getProgFunction(SVFModule *svfModule,
                                          const std::string &funName) {
  for (SVFModule::const_iterator it = svfModule->begin(),
                                 eit = svfModule->end();
       it != eit; ++it) {
